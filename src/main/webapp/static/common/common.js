...
$(function(){
	...
	$(".mainMenu").click(function(){
		var $this = $(this);
		if(!$this.hasClass("mainMenu-on")){
			$(".mainMenu-ul").slideUp(600);
			$(".mainMenu").removeClass("mainMenu-on");
			if(0 != $this.next(".mainMenu-ul").length){
				$this.next(".mainMenu-ul").slideDown(600);
			}
			$this.addClass("mainMenu-on");
		}else{
			$this.removeClass("mainMenu-on");
			$this.next(".mainMenu-ul").slideUp(600);
		}
	});
	//最后一个没有背景图
	$('.mainMenu-ul').find(' li:last').css({'background':'none'});
	//IE6设置最小宽度
	if(!-[1,]&&!window.XMLHttpRequest){
		//左侧菜单鼠标移入效果
		$(".mainMenu").hover(function(){
			$(this).addClass("mainMenu-hover");
		},function(){
			$(this).removeClass("mainMenu-hover");
		});
	}
...
function yd1AddPeople(obj){
	...
	$('#ordercontact').append('<ul class="ydbz_qd min-height" name="orderpersonMes">'+
		'<li><label><span class="xing">*</span>渠道联系人<font>' + (contactPeopleNum+1) +'</font>：</label><span name="channelConcat"></span> </li>'+
...
//	var middle = 'onkeyup="this.value=this.value.replace(/[^\\d\\+\\-]/g,\'\')" onafterpaste="this.value=this.value.replace(/[^\\d\\+\\-]/g,\'\')"';
	var middle = 'onkeyup="isMoney(this)" onafterpaste="isMoney(this)"';
	$(element).parent().parent().append('<p><label class="transport_city_label"><span class="xing">*</span>联运城市分区：</label><input id="intermodalGroupPart' + (index+1) + '" name="intermodalGroupPart" maxlength="50" type="text"><label>联运价格：</label>' + html_selectCurrency +'&nbsp;<input id="intermodalGroupPrice" name="intermodalGroupPrice" maxlength="16" type="text" ' + middle + ' /><span class="currency">元</span><a class="ydbz_s gray transportDel">删除</a></p>');
...
//地接社、渠道商切换
function switchSupplierAndAgent(){
	//点击地接社
	$(".activitylist_bodyer_table").delegate(".supplier","click",function(){
        $(this).addClass("on").siblings().removeClass('on');
        $('.agent_cen').removeClass('onshow');
        $('.supplier_cen').addClass('onshow');
    });
    //点击渠道商
    $(".activitylist_bodyer_table").delegate(".agent","click",function(){
         $(this).addClass("on").siblings().removeClass('on');
         $('.supplier_cen').removeClass('onshow');
         $('.agent_cen').addClass('onshow');
    });
}
function switchYingshouAndWeishou() {
	//应收金额
    $(".activitylist_bodyer_table").on("click", ".order-yingshou-title", function() {
        $(this).addClass("on").siblings().removeClass('on');
        var $table=$(this).parents('table:first');
        $table.find('.order-yingshou').addClass('onshow');
        $table.find('.order-weishou').removeClass('onshow');
    });
  //未收余额
    $(".activitylist_bodyer_table").on("click", ".order-weishou-title", function() {
        $(this).addClass("on").siblings().removeClass('on');
        var $table=$(this).parents('table:first');
        $table.find('.order-yingshou').removeClass('onshow');
        $table.find('.order-weishou').addClass('onshow');
    });
}
...
function selectChildMenu(href, element){
    ...
    var param = {_m : fId, _mc : childId,random:Math.random()};
    href = appendParam(href, param);
    window.location.href = href;
}
...
//上传的图片加水印
function uploadFilesAddFlag(ctx, inputId, obj, isSimple) {
	var fls=flashChecker();
	var s="";
	if(fls.f) {
	} else {
	...
	$.jBox("iframe:"+ ctx +"/MulUploadFile/uploadFilesPage1?isSimple=" + isSimple, {
	//$.jBox("iframe:"+ ctx, {
	    title: "文件上传",
		width: 340,
   		height: 365,
 ...
 */
function uploadFiles4SpecilDeman(inputId, obj, isSimple) {
	var ctx = $("#ctx").val();
	var fls=flashChecker();
	var s="";
	if(!fls.f) {
	...
	$.jBox("iframe:"+ ctx +"/MulUploadFile/uploadFilesPage?isSimple=" + isSimple, {
		...
		submit: function (v, h, f) {
			...
			commenFunction4SpecialDeman(obj,fileIDList,fileNameList,filePathList);
			$("#uploadPathDiv").remove();
			$(".clickBtn",window.parent.document).removeClass("clickBtn");
			fileNameList = "";
		}
...
/**
 * 输入框onblur时，及时对title更新
 */
 function updataInputTitle(obj){
 	$(obj).attr("title", $(obj).val());
 }

//299-start
/**
 * 检查最大字符长度限制。可用以给textarea模仿添加input拥有的maxlength功能
 * @param obj 本对象
 * @param maxLength 最大长度限制
 */
function checkMaxLength(obj, maxLength){
	if ($(obj).val().length > maxLength){
		$(obj).val($(obj).val().substring(0, maxLength)); // 超过了便截取之
		top.$.jBox.tip('最大长度50！', 'warn');
	} 
}

function pricePlan(obj) {

	var $tbody = $(obj).parents('tr:first').next('.pricePlanContainer').find("tbody");

	if (!$(obj).parents('tr:first').next('.pricePlanContainer').is(':hidden')) {
		$(obj).parents('tr:first').next('.pricePlanContainer').hide();
		$(obj).html('展开价格方案');
	}else {
		if($tbody.html().trim() == "") {		//防止多次点击append重复数据
			var priceJsonVal = $(obj).attr("data");
			parsePriceJson(priceJsonVal);
		}
		$(obj).parents('tr:first').next('.pricePlanContainer').show();
		$(obj).html('收起价格方案');
	}
}
 ...
 * 299产品列表
 */
function expandPriceJson(obj, activityKind) {
	var $tbody = $(obj).parents("tr:first").siblings(".pricePlanContainer").find("tbody");
	...
	if (!$(obj).parents("tr:first").siblings(".pricePlanContainer").is(':hidden')) {
		...
		if($tbody.html().trim() == "") {		//防止多次点击append重复数据
			var priceJsonVal = $(obj).attr("data");
			parsePriceJson4Activity(priceJsonVal, obj, activityKind);
		}
 ...
 * 解析价格方案字符串，填充到table中
 */
function parsePriceJson(pJson){
	if(pJson == undefined || pJson == null || pJson == ''){
		return;
	}
	var json = eval(pJson);
	// json数组个数
 ...
 * 解析价格方案字符串，填充到table中
 */
function parsePriceJson4Activity(pJson, obj, activityKind){
	var $table = $(obj).parents("tr:first").siblings(".pricePlanContainer").find("table");
	if(pJson == undefined || pJson == null || pJson == ''){
		return;
	}
	var json = eval(pJson);
 ...
 * 因为不能使用上面uploadFiles方法，这里再添加一个。（特殊需求里的附件上传）
 */
function uploadConfirmFiles(inputId, obj,orderId,uploadSave) {
	var title = '上传确认单';
	var flag = false;
	if("openNoticeFile" == inputId){
		title = '上传出团通知';
		flag = true;
	}
	var ctx = $("#ctx").val();
	...
	//上传确认单为多文件上传 出团通知为单文件上传
	$.jBox("iframe:"+ ctx +"/MulUploadFile/uploadFilesPage?isSimple="+flag, {
		title: title,
		width: 340,
		height: 'auto',
		buttons: {'完成上传':true},
		...
		submit: function (v, h, f) {
			...
			if(fileIDList != ""){
				$.ajax({
					...
					data : {
						"orderId" : orderId,
						"docFileIds":fileIDList,
						"inputId" : inputId
					},
					success : function (msg) {
						if (msg) {
							if (msg.success) {
								top.$.jBox.tip(msg.success, 'warning');
...
//清空所有条件按钮无法清空搜索后的输入框问题 added by tlw at20170309
