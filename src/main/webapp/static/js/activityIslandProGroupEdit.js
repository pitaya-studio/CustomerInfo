...
var addGroup_box = (function () {
    function initEvent(options) {
        ...
        //币种自动选择
        $table.find("select[name=peoplePrice]").val(options.v_currency);
        var $table = $("#jbox_tq table.table_product_info");
        // 房型模板 餐型模板 升级餐型模板
        ...
        $table.on('click', 'a.addHouseType', function () {
            ...
            $.ajax({
        		type: "POST",
        	   	url: $ctx+"/activityIsland/getOptions?"+new Date(),
        	   	data:{
        			type:"baseMeal",
        			uuid:roomUuid
        		},
            ...
            /*if ($(this).prop("selectedIndex") <= 0) {
            	...
            	if(!res_1){
                  ...
                  $(res_1.space_level).each(function(){
              		var tdNode6_2 = $("<td/>").text(0);
              		$(trNode6).append(tdNode6_2);
              	 });
                  var tdNode6_3 = $("<td/>").text(0);
    ...
    function initData(data,trId) {
        ...
        for (var i = 0; i < prices.length; i++) {
            ...
            // 6.29 价格结构修改
        	for (var j = 0; j < islandprice.length; j++) {
        		var $td = $trs.find("tr[name='"+islandprice[j].typeUuid+"']").find('td[name='+(islandprice[j].typeUuid+j)+']');
	        	$td.find("select:eq(0)").val(islandprice[j].currency.value);
	        	$td.find("input").val(islandprice[j].price);
            }
            // end
    ...
    function formSubmitAdd(jsoninput,showType){
    	...
    	$.post(
    		...
    		function(data){
    			if(data.message=="1"){
    				$.jBox.tip("添加成功!");
    				setTimeout(function(){window.location.href=$ctx+"/activityIsland/islandProductList?status=0&showType="+showType;},500);
    ...
    function formSubmitEdit(jsoninput){
    	...
    	$.post(
    		...
    		function(data){
    			if(data.message=="1"){
    				$.jBox.tip("添加成功!");
    				setTimeout(function(){window.location.href=$ctx+"/activityIsland/islandProductList?status=0&showType=" + "2"},500);
    			}else if(data.message=="2"){
    				setTimeout(function(){window.location.href=$ctx+"/activityIsland/islandProductList?status=0&showType=" + "2"},500);
    				$.jBox.tip("修改成功!");
    			}else if(data.message=="3"){
    	   	...
    	   	success: function(item){
    	   		...
    	   		$.jBox(boxHtml, {
    	            ...
    	            submit: function (v, h, f) {
    	            	...
    	            	var groupCodeOnlyFlag = false;
    	            	if(options.groupCode==no && !options.isAdd){
    	            		groupCodeOnlyFlag = true;
    	            	}
    	            	//团号唯一性验证（和数据库中进行比对）
						$.ajax({
							type:"post",
							...
							success:function(data){
							if(data && data.message=="true" && !groupCodeOnlyFlag){
								$.jBox.tip("该团号已存在！！！");
							} else {
	    	            	...
	    	            	//---团期uuid赋值 开始---
	    	            	//海岛游产品发布（新增、复制新增）、海岛游产品列表（复制并新增）、海岛游团期列表（复制并新增）json串没有团期uuid
	    	                if(options.tag=="IslandProductListProduct_addGroup" || options.tag=="islandProductListProduct_copyLink"){//新增
	    	                	data.uuid = '';
	    	                	data.status = v;
	    	                	if(data.status ==0){
	    	                		data.status = 3;
	    	                ...
	    	                if(options.tag=="IslandProductListProduct_addGroup" || options.tag=="islandProductListProduct_copyLink"){
	    	                	//列表项中的团期新增 增加
    	        ...
    	        if(options.tag=="IslandProductListProduct_addGroup"){
    	        	$("select.w50_30").val(options.v_currency);
    	        }
    	   	}
    	});
