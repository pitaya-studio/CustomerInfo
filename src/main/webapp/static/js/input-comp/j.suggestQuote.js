...
//						html+='<li rel="'+options.hot_list[h][0]+'"><a href="#'+h+'"><span>'+options.hot_list[h][2]+'</span>'+options.hot_list[h][1]+'</a></li>';
						html+='<li rel="'+options.hot_list[h][0]+'" title="'+options.hot_list[h][1]+'" style="text-align:left"><a href="#'+h+'"><span> </span>'+options.hot_list[h][1]+'</a></li>';
					}
					html='<div class="gray ac_result_tip">请输入名称</div><ul>'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
...
//							html += '<li rel="' + options.source[i][0] + '"><a href="#' + i + '"><span>' + options.source[i][2] + '</span>' + options.source[i][1] + '</a></li>';
							html += '<li rel="' + options.source[i][0] + '"  title="'+options.source[i][1]+'" style="text-align:left"><a href="#' + i + '"><span> </span>' + options.source[i][1] + '</a></li>';
						}
					}
					if (html == '') {
						suggest_tip = '<div class="gray ac_result_tip">' + items + '</div>';
						$(options.dataContainer).val(items);
					}
					else {
						suggest_tip = '<div class="gray ac_result_tip">' + items + '</div>';
					}
			...
			function selectCurrentResult() {
				...
				if ($currentResult) {
					var resultVal = $currentResult.children('a').html().replace(/<span>.+?<\/span>/i,'');
					$input.val(resultVal.replace(/<span><\/span>/i,''));
					$input.attr('title',resultVal.replace(/<span><\/span>/i,''));
					$results.hide();
	...
	function initSuggestClass(url,requestField,id,jsondata) {
			var i = 0;
			var j = 0;
			if(url){
				$.ajax({
					...
					success : function(data) {
						if (data) {
							$.each(data, function() {
									...
									commoncountrys[i++] = new Array(this.id, this.agentName);
								
								countrys[j++] = new Array(this.id, this.agentName);
							});
						}
					}
				});
