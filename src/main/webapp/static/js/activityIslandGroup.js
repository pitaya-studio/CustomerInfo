    	...
    	$.post(
    		...
    		function(data){
    			if(data.message=="1"){
    				...
    				$.jBox.tip('团期复制并新增异常，请重新操作!','warning');
    				$(obj).attr("disabled", false);
    			}
    		});
    }
    ...
    function formSubmitEdit(jsoninput){
    	...
    	$.post(
    		...
    		function(data){
    			if(data.message=="1"){
    				$.jBox.tip("添加成功!");
    				setTimeout(function(){window.location.href=$ctx+"/activityIsland/islandProductList?status=0&showType=" + "1"},500);
    			}else if(data.message=="2"){
    				setTimeout(function(){window.location.href=$ctx+"/activityIsland/islandProductList?status=0&showType=" + "1"},500);
    				$.jBox.tip("修改成功!");
    			}else if(data.message=="3"){
    	            ...
    	            submit: function (v, h, f) {
						...
						$.ajax({
	    	                ...
	    	                if(options.tag=="toSaveActivityIslandProduct" 
	    	                	|| options.tag=="toSaveActivityIslandProduct_copyLink" 
	    	                		|| options.tag=="islandProductListGroup_copyLink" || options.tag=="islandProductListProduct_copyLink"){//新增
	    	                	data.uuid = '';
	    	                	data.status = v;
	    	                	if(data.status ==0){
	    	                		data.status = 3;
	    	                ...
	    	                if(options.tag=="islandProductListGroup_copyLink" || options.tag=="islandProductListProduct_copyLink"){
	    	                	//列表项中的团期新增 增加
