...
//			alert(this.trafficWaysval);
		...
		// 同行价
		$td = $dataTrs.eq(rIndex).find("td:eq(1)");
		$.each(data.tradePrices, function(i) {
	...
	return function(options) {
		...
		$.jBox(boxHtml, {
			...
			submit: function(v, h, f) {
				//团号非空验证
				var igroupcode = h.find("input[id=igroupcode]").val();
				var tdate = h.find("#tdate").val();
				if(tdate==''){
			 		$.jBox.tip("日期不能为空！！！");
			 		return false;
			 	}else if(options.updateg == "updateg"){
		...
		var obj = $("select[name=hotelUuid]");
		var obj_island = $("select[name=islandUuid]");
//		同步异步原因
//	 	var obf_room = $("select[name=roomtype]");
//    	alert(obf_room);
//		getAjaxSelect('foodtype',obf_room); 
		getAjaxSelect('roomtype',obj); 
		getAjaxSelect('islandway',obj_island);
       	getAjaxSelect('hotelrank',$("#hotelUuid"));
    	//加载酒店下绑定的住客类型
   		getAjaxSelect('travelerTypeRelations',$("#hotelUuid"));
		initEvent();
