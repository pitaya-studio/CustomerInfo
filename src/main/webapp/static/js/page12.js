 ...
 *
 * @param page1 分页相关信息
 * @param webVersion  对应的版本，t1,t2
 * @param fuc  分页函数
 * @returns {string}
 */
function doPage(page1,webVersion,fuc){
    ...
    if(webVersion=="t1"){
        ...
        pageHtml+="<li><span class=\"skip\" href=\"javascript:page(2,10);\">跳至 <input type=\"text\" value=\""+pageNo+"\" onkeydown=\"var e=window.event||event||this;var c=e.keyCode||e.which;if(c==13) goPage(this.value,"+pageSize+");\" onkeypress=\"var e=window.event||event||this;var c=e.keyCode||e.which;if(c==13) goPage(this.value,"+pageSize+");\"/>页</span></li>"
        ...
        return pageHtml;
    }else if(webVersion=="t2"){
        pageHtml="";
        pageHtml+="<ul>";
        if(pageNo == first){//如果当前页是首页
            pageHtml+="<li class=\"disabled\"><a href=\"javascript:\"> « 上一页</a></li>";
        }else{
            pageHtml+="<li><a onclick=\"javascript:goPage("+prev+","+pageSize+");\"> « 上一页</a></li>";
        }
        //中间显示的页码 显示的开始页
        var begin = pageNo - (length / 2);
        ...
        if (begin > first) {
            var i = 0;
            for (i = first; i < first + slider && i < begin; i++) {
                pageHtml+="<li><a href=\"javascript:goPage("+i+","+pageSize+");\">"
                    + (i + 1 - first) + "</a></li>";
            }
            if (i < begin) {
                pageHtml+="<li class=\"disabled\"><a onclick=\"javascript:\">...</a></li>\n";
            }
        }

        for ( j = begin; j <= end; j++) {
            if (j == pageNo) {
                pageHtml+="<li class=\"active\"><a href=\"javascript:\">" + (j + 1 - first)
                    + "</a></li>\n";
            } else {
                pageHtml+="<li><a onclick=\"javascript:goPage("+j+","+pageSize+");\">"
                    + (j + 1 - first) + "</a></li>\n";
            }
        }
        ...
        for ( k = end + 1; k <= last; k++) {
            pageHtml+="<li><a href=\"javascript:goPage("+k+","+pageSize+");\">"
                + (k + 1 - first) + "</a></li>\n";
        }
        if (pageNo == last) {
            pageHtml+="<li class=\"disabled\"><a href=\"javascript:\">下一页 »</a></li>\n";
        } else {
            pageHtml+="<li><a href=\"javascript:goPage("+next+","+pageSize+");\">下一页 »</a></li>";
        }
        pageHtml+="<li class=\"disabled controls\"><a href=\"javascript:\">第 "+
            "<input type=\"text\" value=\""+pageNo+"\" onkeypress=\"var e=window.event||event||this;var c=e.keyCode||e.which;if(c==13) goPage(this.value,"+pageSize+");\"";
        pageHtml+="onclick=\"this.select();\"/> / "+last+"  页 ， 每页 ";
        pageHtml+="<input type=\"text\" value=\""+pageSize+"\" onkeypress=\"var e=window.event||event||this;var c=e.keyCode||e.which;if(c==13)goPage("+pageNo+",this.value);\" onclick=\"this.select();\"/> 条，";
        pageHtml+="共 " + count + " 条"+(message!=null?message:"")+"</a></li>\n";
        pageHtml+="</ul>";
        pageHtml+="<div style=\"clear:both;\"></div>";
        return pageHtml;
    }
