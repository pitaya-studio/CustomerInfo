	...
	(function($) {
		$.widget("ui.combobox", {
			...
			_create: function() {
				...
				//instance.option( options || {"ulcssid":self.options.target} )._init();
				
				var input = self.input = $('<input/>');
				//$(input).autocomplete('option', 'ulcssid', 2);
				//input.data("autocomplete")._setOption("ulcssid",self.options.target);
					input.attr('id', 'combobox_' + (''+Math.random()).slice(-6))
					.attr('name',self.options.target)
					.css('display', 'inline')
					.attr('size', size)
					...
					.autocomplete({"ulcssid":self.options.target},{
