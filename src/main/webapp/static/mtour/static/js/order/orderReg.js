mtour.directive('orderRegButton', ['urlConfig', 'qcDialog', '$timeout', 'groupAmount', 'qcApi', 'qcMessage', 'calculateTotalFee', '$rootScope', 'commonValue',
                    ...
                    qcDialog.open({
                        ...
                        controller : ['$scope', 'qcApi', function ($scope, qcApi) {
                            ...
                            if (scope.operatorType == 0) {
                                ...
                                $scope.currentOrder.saveModel='new';//为快速生成订单  区分修改
                                $scope.currentOrder.companyRoleCode=$scope.$parent.userInfo.companyRoleCode;
                                $timeout(function () {
                                ...
                                qcApi.post('order/showAirticketOrderDetail', {
                                    ...
                                    operatorType:scope.operatorType
                            ...
                            $scope.commitOrder = function () {
                                ...
                                $scope.currentOrder.fee.arrivedAmount = totalAmount.arrived;
                                $scope.currentOrder.fee.returnAmount = totalAmount.return;
                                if($scope.currentOrder.fee.priceChange.additionalCosts){
                                    if ($scope.currentOrder.fee.priceChange.additionalCosts.length > 0) {
                                        angular.forEach($scope.currentOrder.fee.priceChange.additionalCosts, function (additionalCost) {
                                            if (!additionalCost.amount) {
                                                $scope.currentOrder.fee.priceChange.additionalCosts.remove(additionalCost);
...
mtour.directive('orderRegBaseInfo', ['$rootScope', 'urlConfig', 'commonValue','fixedValue', function ($rootScope, urlConfig, commonValue,fixedValue) {
    ...
    return {
        ...
        link       : function (scope, ele, attrs) {
            ...
            scope.baseInfo.ticketCZ=[];
            scope.baseInfo.ticketXS=[];
            scope.lineTypeList=fixedValue.lineType;
            /*modify for mtour 0580 by wlj at 20170322 -start */
            if(scope.baseInfo.lineType){
                scope.lineTypeList.lineTypeCode=scope.baseInfo.lineType;
            }else{
                scope.lineTypeList.lineTypeCode='1';
                scope.baseInfo.lineType='1';
            }
            /*modify for mtour 0580 by wlj at 20170322 -end */
            function initBaseInfo() {
                ...
                //修改和详情页面   数据来源//ticketName:"769,刘新涛" ticketId:"769"   要换成   manyTicketName  和manyOperatorName
                if ((!scope.baseInfo.ticketCZ||scope.baseInfo.ticketCZ.length==0) && scope.baseInfo.ticketName) {
                    scope.baseInfo.ticketCZ=[];
                    /*if(!scope.baseInfo.tickets){
                     scope.baseInfo.tickets=[];
                     }*/
                    var _name="";
                    var _ticket=scope.baseInfo.ticketName.split(";");
                    for(var i=0;i<_ticket.length;i++){

                        var _ti = {
                            userId  :_ticket[i].slice(0,_ticket[i].indexOf(",")),
                            userName: _ticket[i].slice(_ticket[i].indexOf(",")+1)
                        };
                        _name+=_ticket[i].slice(_ticket[i].indexOf(",")+1)+"、";
                        scope.baseInfo.ticketCZ.push(_ti);
                        /* scope.selectedItem=scope.baseInfo.ticketCZ;
                         scope.baseInfo.tickets.push(_ti);*/
                    }
                    //为详情页面展示做：
                    scope.baseInfo.ticketNameInfo=_name.slice(0,-1);
                }
                if ((!scope.baseInfo.ticketXS||scope.baseInfo.ticketXS.length==0) && scope.baseInfo.operatorName) {
                    scope.baseInfo.ticketXS=[];
                    var _oname="";
                    var _oticket=scope.baseInfo.operatorName.split(";");
                    for(var i=0;i<_oticket.length;i++){

                        var _ti = {
                            userId  :_oticket[i].slice(0,_oticket[i].indexOf(",")),
                            userName: _oticket[i].slice(_oticket[i].indexOf(",")+1)
                        };
                        _oname+=_oticket[i].slice(_oticket[i].indexOf(",")+1)+"、";
                        scope.baseInfo.ticketXS.push(_ti);
                    }
                    //为详情页面展示做：
                    scope.baseInfo.operatorNameInfo=_oname.slice(0,-1);
                }
            ...
            scope.$on('baseInfo.tickets.change', function () {
                scope.baseInfo.ticketCZ=scope.baseInfo.tickets;
                /*scope.baseInfo.ticketId = scope.baseInfo.tickets[scope.baseInfo.tickets.length-1].userId;
                 scope.baseInfo.ticketName = scope.baseInfo.tickets[scope.baseInfo.tickets.length-1].userName;

                 scope.baseInfo.ticketcz = {
                 userId  : scope.baseInfo.ticketId,
                 userName: scope.baseInfo.ticketName
                 };
                 if(!scope.baseInfo.ticketCZ){
                 scope.baseInfo.ticketCZ=[];
                 }
                 scope.baseInfo.ticketCZ.push(scope.baseInfo.ticketcz);*/
            });
            //销售选中的时候的操作
            scope.$on('baseInfo.operators.change', function () {
                scope.baseInfo.ticketXS=scope.baseInfo.operators;
                /* scope.baseInfo.operatorId = scope.baseInfo.operators[scope.baseInfo.operators.length-1].userId;
                 scope.baseInfo.operatorName = scope.baseInfo.operators[scope.baseInfo.operators.length-1].userName;

                 var _oper = {
                 userId  : scope.baseInfo.operatorId,
                 userName: scope.baseInfo.operatorName
                 };
                 if(!scope.baseInfo.ticketXS){
                 scope.baseInfo.ticketXS=[];
                 }
                 scope.baseInfo.ticketXS.push(_oper);*/
            });
...
mtour.directive('orderRegReservation', ['$rootScope','urlConfig', 'fixedValue', 'qcObjectInArray', function ($rootScope,urlConfig, fixedValue, qcObjectInArray) {
    return {
        restrict   : 'A',
        scope      : {
            reservations: '=',
        ...
        controller : ['$scope', function ($scope) {
            this.addContact = function () {
                ...
                //$scope.newOrModity.contacts.push({});
                //$scope.saveModel.contacts.push({});
            };
            this.removeContact = function (contact) {
                $scope.reservations.contacts.remove(contact);
                //$scope.newOrModity.contacts.push({});
                //$scope.saveModel.contacts.remove(contact);
            }
        }],
            ...
            scope.$watch('memo',function(newVal,oldVal){
                if(newVal&&newVal!=oldVal){
                    if(newVal.length>limitation){
                        scope.memo=newVal.slice(0,limitation);
                    }
                }
            })
        }
...
mtour.directive('user', ['urlConfig', 'qcApi', function (urlConfig, qcApi) {
    return {
        ...
        link       : function (scope, ele, attrs, ngModel) {
            ...
            //scope.selectedUser = ngModel.$modelValue;
...
//将多选选中的人展示出来
mtour.directive('userselectedcz', ['urlConfig', function (urlConfig) {
    console.log("nimade  走进函数没");
    return {
        restrict   : 'A',
        require    : '^ngModel',
        scope      : {
        ...
        replace    : true,
        templateUrl: urlConfig.mtourHtmlTemplateUrl + 'order/user-selected-cz.html',
        link       : function (scope, ele, attrs, ngModel) {
            ngModel.$formatters.push(function (modelValue) {
            ...
            scope.remove = function (item) {
                ...
                //取得值，并去掉下面的选中状态即可
                var removeId=item.userId;
                var removeName=item.userName;
