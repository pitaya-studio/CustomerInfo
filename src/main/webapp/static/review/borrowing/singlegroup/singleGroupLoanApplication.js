...
function submitRefund() {
	...
	$.ajax({
		type: "POST",
		url: g_context_url + "/singlegrouporder/lendmoney/LendMoneyApply?singleorderType=" + $("#myOrderType").val(),
		dataType: "text",
		data: $("#f1").serialize(),
		async: false,
...
//				  jBox.tip(msg, "warning");
			if (msg == 'applySuccess') {
				jBox.tip("借款申请成功!", "warning");
				document.location = g_context_url + "/singlegrouporder/lendmoney/borrowAmountList?flowType=19&productType=" + $("#myOrderType").val() + "&orderId=" + orderId;
			} else {
				top.$.jBox.info(msg, '系统提示', {
					submit: function () {
//						  document.location = g_context_url + "/singlegrouporder/lendmoney/borrowAmountList?flowType=19&productType="+$("#myOrderType").val()+"&orderId=" + orderId;
						document.location = g_context_url + "/singlegrouporder/lendmoney/applyBorrowing?orderId=" + orderId + "&productType=" + $("#myOrderType").val();
					}
				});
			}
		}
