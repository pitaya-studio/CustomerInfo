...
$(function () {
	...
	$('select[name="selRebateObj"]').comboboxInquiry();
	
	//renderSelects(selectQuery());
	selectQuery();
	//C475--渠道和返佣对象切换-start

	//开始判断展示渠道还是返佣对象
	if($("span.rebate_channelTh").length>0){
		if($("span.rebate_channelTh").hasClass("unactive")){//显示供应商
			$('.rebate_channel').hide();
			$('.rebate_supply').show();
		}else{
			$('.rebate_channel').show();
	...
	$(document).on('click', '.rebate_channelTh', function () {
		$('.rebate_supplyTh').addClass('unactive');
		$(this).removeClass('unactive');
		$('.rebate_channel').show();
		$('.rebate_supply').hide();
	});
	$(document).on('click', '.rebate_supplyTh', function () {
		$('.rebate_channelTh').addClass('unactive');
		$(this).removeClass('unactive');
		$('.rebate_channel').hide();
		$('.rebate_supply').show();
	});
...
function backReview(rid) {
	...
	$.jBox.confirm("确定要撤销审核吗？", "提示", function (v, h, f) {
		console.log("${ctx}/newRebatesReview/backReview/" + rid);
		if (v == 'ok') {
			$.ajax({
				type : "POST",
				async : false,
				url : $("#ctx").val() + "/newRebatesReview/backReview/" + rid + "?dom=" + Math.random(),
				data : {},
