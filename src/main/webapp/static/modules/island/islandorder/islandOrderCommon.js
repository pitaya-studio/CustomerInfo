...
$(document).ready(function (e) {
    ...
    $("#orderpersonMesdtail").on("click", "span.yd1AddPeople", function () {
        ...
        if (channelType == -1) {
            contactsNum = $('#nonChannelList ul[name="orderpersonMes"]').length;
        } else {
            contactsNum = $('#signChannelList ul[name="orderpersonMes"]').length;
        }
        ...
        //绑定事件
        var $border = $newContacts.find('[name="channelConcat"]');
...
//                                }
        ...
        //重置联系人序号
        if (channelType == -1) {
            $("#nonChannelList ul[name=orderpersonMes]").each(function (index, element) {
                $(element).children("li").eq(0).find("font").text(index + 1);
            });
        }
        else {
            $("#signChannelList ul[name=orderpersonMes]").each(function (index, element) {
                $(element).children("li").eq(0).find("font").text(index + 1);
            });
        }
    });
...
//    });
    ...
    //控票改变
    $("#subControlTicketNum").on("change", ticketAmount);
    //非控票改变
    $("#subUnControlTicketNum").on("change", ticketAmount);
    $(document).click(function (event) {
        ...
        // 删除其他费用
        $(this).parent().parent().remove();
        var islandOrderPriceUuid = $(this).attr("data");
        if(islandOrderPriceUuid && islandOrderPriceUuid != null && islandOrderPriceUuid != ''){
        	delete_orderPrice(islandOrderPriceUuid);
        }
        offersChange();
    });
...
function peopleCountChange() {
    ...
    for (var l = orderInfo.traveller.length; l--;) {
        ...
        if($.trim(count).length == 0 || isNaN(count) == true){
        	count = 0;
        	//parseInt($("#" + traveller.type).find("td:eq(3) input").val(0));
        }
...
//    });
	...
	var usedMap = {};
	$("#passengerInfoTable > tbody > tr").each(function(){
		var spaceLevel = $(this).find("#spaceLevelSelect option:selected").val();	//舱位等级
		var personType = $(this).find("#personTypeSelect option:selected").val();	//游客类型
//		var personType = $(this).find("select[name=personTypeSelect] option:selected").val();
//		var spaceLevel = $(this).find("select[name=spaceLevelSelect] option:selected").val();
	...
	$(".groupPrices_tr").each(function(){
		var travelerType = $(this).find("input[name=islandOrderPrice_travelerType]").val();
		var spaceLevel = $(this).find("input[name=islandOrderPrice_spaceLevel]").val();
		var orderPriceNum = $(this).find("input[name=orderPersonNum]").val();
	...
	if($(".groupPrices_tr").size() != 0) {
		for(var key in usedMap){
		    if(usedMap[key] == 0) {
		    	...
		    	spaceLevelName = $("#add_tours_obj_tr").find("select[name=spaceLevelSelect]").find("option[value="+ spaceLevel +"]").text();
		    	personTypeName = $("#add_tours_obj_tr").find("select[name=personTypeSelect]").find("option[value="+ travelerType +"]").text();
				personTypeNum = 0;
...
//		cache:false,
		...
		success:function(data){
			...
			if(data.agentinfo){
				...
				signChannel.find("input[name=contactsAddress]").val(data.address);
...
//				signChannel.find("input[name=remark]").val(data.agentinfo.remarks);
			...
			$('#signChannelList [name="orderpersonMes"]:not(:first)').remove();
...
function updateIslandOrder(){
	...
	//签约渠道 &非签约渠道 单独存
	if(orderCompany == "-1"){
		$array = $("#nonChannelList"); 
	}else{
		$array = $("#signChannelList");
	}
	
	//联系人列表
	var contactsArr = [];
	$array.children("ul").each(function(index,item){
	...
	//机票扣减张数
	var subControlTicketNum =$("#subControlTicketNum").val();	//控票
	var subUnControlTicketNum =$("#subUnControlTicketNum").val();//非控票
	
	//控房明细
	var controlDetailArr = [];
	$("#controlDetail_table > tbody > tr").each(function(index, tr){
		var number = $(tr).find("input[name='number']").val();
		var islandOrderControlDetailUuid = $(tr).find("#islandOrderControlDetailUuid").val();	//海岛游订单控房明细uuid
		var hotelControlDetailUuid = $(tr).find("#hotelControlDetailUuid").val();	//酒店控房明细uuid
		if(islandOrderControlDetailUuid && islandOrderControlDetailUuid != ""){
			if(number == "" || number == null){
				number = 0;
			}
			controlDetailArr.push({hotelControlDetailUuid:hotelControlDetailUuid,islandOrderControlDetailUuid:islandOrderControlDetailUuid,number:number});
		}else{
			if(number && number != ""){
				controlDetailArr.push({hotelControlDetailUuid:hotelControlDetailUuid,islandOrderControlDetailUuid:islandOrderControlDetailUuid,number:number});
			};
	...
	//订单备注
	var islandOrderRemark = $("#islandOrderRemark").val();
	var islandOrderUuid = $("#islandOrderUuid").val();
	var islandOrderId = $("#islandOrderId").val();
	...
	//提交的数据结构
	saveData={
		islandOrderId:islandOrderId,	//订单id
		islandOrderUuid:islandOrderUuid,	//订单uuid
		...
		subControlTicketNum:subControlTicketNum,
		subUnControlTicketNum:subUnControlTicketNum,
		controlDetailArr:controlDetailArr,	//控房明细
		...
		islandOrderRemark:islandOrderRemark,
	...
	$.ajax({
		type:"POST",
		url: contextPath + "/islandOrder/updateIslandOrder",
		dataType:"json",      
		...
		success:function(data){
			if(data&&data.code == '1'){
				...
				window.location.href = contextPath + "/islandOrder/list/0";
	...
	$.jBox.confirm("确定要删除游客吗？", "提示", function(v, h, f) {
		if (v == 'ok') {
		    ...
		    if(travelerUuid && travelerUuid != ''){
		    	$.ajax({
		    		type:"POST",
		    		url: contextPath + "/islandTraveler/delete",
		    		data:{
		    			uuids:travelerUuid
		    		}, 
		    		success:function(data){
...
//删除订单价格
function delete_orderPrice(islandOrderPriceUuid){
	if(islandOrderPriceUuid && islandOrderPriceUuid != ''){
    	$.ajax({
    		type:"POST",
    		url: contextPath + "/islandOrderPrice/delete",
    		data:{
    			uuids:islandOrderPriceUuid
    		}, 
    		success:function(data){
...
function delete_contacts(orderContactsId){
	if(orderContactsId && orderContactsId != ''){
    	$.ajax({
    		type:"POST",
    		url: contextPath + "/islandOrder/deleteOrderContacts",
    		data:{
