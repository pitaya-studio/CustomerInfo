...
var erecordList = {
		dao : {
			...
			record : function(m,i,createUser,pstatus){
				...
				var namestr = "";
				// (只在确定报价时，出现被选中计调员)
				if( m.estimateStatus==3 || m.estimateStatus==4){
					
					if(m.acceptAoperatorReply!=null){
						namestr+= m.acceptAoperatorReply.userName+"<br/>";
				}
            ...
            html.push('<td width="9%" class="tc">'+$.trekizFormatDate({time:m.admitRequirements.dgroupOutDate,pattern:"yyyy-MM-dd"})+'</td>');//预计出团日期
            html.push('<td width="9%">'+m.admitRequirements.outsideDaySum+'天'+m.admitRequirements.outsideNightSum+'晚</td>');//境外停留天数
            html.push('<td width="8%">'+m.admitRequirements.travelCountry+'</td>');//线路国家
