...
//			}
			...
			html.push('<th width="4%">次数</th>');
			html.push('<th width="9%">询价日期</th>');
		    //html.push('<th width="7%">计调员</th>');
			html.push('<th width="9%">报价日期</th>');
			html.push('<th width="9%">预计出团日期</th>');
			html.push('<th width="9%">境外停留天数</th>');
			html.push('<th width="8%">线路国家</th>');
//				html.push('<th width="10%">团队类型</th>');
			html.push('<th width="10%">地接报价</th>');
			//html.push('<th width="10%">确认外报价</th>');
			//html.push('<th width="10%">确认结算价</th>');
			html.push('<th width="10%">状态</th>');
			html.push('<th width="7%">操作</th></tr></thead></table>');
...
//				}
			if( m.replyDate!=null&& m.replyDate!=""){
				namestr = m.replyDate;
			...
			html.push('<td width="9%" class="tc">' + $.trekizFormatDate({
				time : m.dgroupOutDate,
				pattern : "yyyy-MM-dd"
			}) + '</td>');// 预计出团日期
			html.push('<td width="9%">' + m.outsideDaySum
					+ '天' + m.outsideNightSum + '晚</td>');// 境外停留天数
			if (m.travelCountry.length > 10) {
				html.push('<td width="8%" title="' + m.travelCountry + '">' + m.travelCountry.substring(0, 10) + '……</td>');//线路国家
			} else {
				html.push('<td width="8%">'+m.travelCountry+'</td>');//线路国家
			}
			
//			html.push('<td width="10%">团队类型</td>');
			...
			if(m.replyPrice!=null){
				...
				if(m.priceDetail!=null){
					html.push('<td width="10%" class="tr"><span class="tdred" title='+m.priceDetail+'>已报价</span></td>');
				}else{
			...
			if(pstatus==0){
				html.push('<a target="_blank" href="' + contextPath
						+ '/eprice/manager/project/ePriceInfoReadOnly/' + m.rid + '/1'
						+ '">详情</a><br/>');
			}else{
				if (m.status == 1  ) {
					html.push('<a target="_blank" href="' + contextPath
							...
							+ '/eprice/manager/project/replayaop/' + m.rid
							+ '">报价</a>');
					...
					html.push('<a target="_blank" href="' + contextPath
							+ '/eprice/manager/project/ePriceInfoReadOnly/' + m.rid + '/1'
							+ '">详情</a><br/><a  href="' + contextPath
							+ '/eprice/manager/project/sendProject/' + m.rid
							+ '">发布产品</a><br/>');
				}
			}
	...
	ajax : {
		...
		getProjectList : function(pn, ps) {
			...
			$.ajax({
				type : "POST",
				url : contextPath
						+ "/eprice/manager/ajax/project/plistback?pageSize=" + ps +"&pageNo="
						+ pn,
				data : the_param,
				dataType : "text",
		...
		getEprecordList : function(dom, pid, pn) {
			...
			$.ajax({
				type : "POST",
				url : contextPath + "/eprice/manager/ajax/project/rlist/" + pid
...
function exportExcel(){
	...
	$("#exportExcel").attr("href",contextPath+"/eprice/manager/project/exportEstimateExcel?lsf=listSearchForm");
};
