$(function(){
	...
	//新增单团询价项目第二个表单初始化，主要是事件绑定处理
   	...
   	// 机票计调初始化界面
	addaeprice.service.initTraffic();
	...
	// 再次询价处理
   	var againRecord = $("#againRecord").val();
   	var againCustomerType = $("#againCustomerType").val();
   // 判断是否为再次询价页面
   	if(againRecord==1){
   		// 接待社询价内容
//   		// 团队类型
//   		var travelTeamType = $("#travelTeamType").val();
//   		if(travelTeamType){
//   	   		$("#eprice-travel-require-div-id").find("input[name='travelTeamType'][value='"+travelTeamType+"']").attr("checked","checked");
//   	   		if(travelTeamType==0){
//   	   	   		var travelTeamTitle = $("#travelTeamTitle").val();
//   	   	   		// 如果
//   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='travelTeamType']").find("input[type='radio'][name='"+travelTeamType+"']").attr("checked","checked");
//   	   	   		// 展开十字按钮
//   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='travelTeamType']");
//   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('travelTeamType'));
//   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(travelTeamTitle);
//   	   	   		obj2.find("a.add_seachcheck").hide();
//   	   		}
//   		}
   		// 护照类型
   		var visaJson = $("#visaType").val();
   		if(visaJson!='{}'){
   			var json = $.parseJSON(visaJson);
   			$.each(json,function(i,n){
   				var visaType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='visaType'][value='"+visaType+"']").attr("checked","checked");
   				}
   	   	   		if(visaType==0){
   	   	   	   		var visaTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='visaType']").find("input[type='radio'][name='"+visaType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='visaType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('visaType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(visaTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		}
   		...
   		if(hotelType!='{}'){
   			// 转换字符串为json对象
   			var json = $.parseJSON(hotelType);
   			$.each(json,function(i,n){
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='hotelType'][value='"+n.type+"']").attr("checked","checked");
   				} 
   				if(n.type==0){
   	   	   	   		// 如果
//   					if(n.check=="unchecked"){
//   						alert("关闭");
//   	   	   	   			$("#eprice-travel-require-div-id").find("div[namecode='hotelType']").find("input[type='checkbox'][name='"+n.type+"']").remove("checked");
//   					}
   	   	   	   			// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='hotelType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('hotelType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(n.title);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		...
   		// 酒店位置
   		var hotelPositionJson = $("#hotelPosition").val();
   		if(hotelPositionJson!='{}'){
   			var json = $.parseJSON(hotelPositionJson);
   			$.each(json,function(i,n){
   				var hotelPosition = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='hotelPosition'][value='"+hotelPosition+"']").attr("checked","checked");
   				}
   	   	   		if(hotelPosition==0){
   	   	   	   		var hotelPositionTitle =n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='hotelPosition']").find("input[type='radio'][name='"+hotelPosition+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='hotelPosition']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('hotelPosition',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(hotelPositionTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		...
   		// 房间要求
   		var roomJson = $("#roomType").val();
   		if(roomJson!='{}'){
   			var json =$.parseJSON(roomJson);
   			$.each(json,function(i,n){
   				var roomType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='roomType'][value='"+roomType+"']").attr("checked","checked");
   				}
   	   	   		if(roomType==0){
   	   	   	   		var roomTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='roomType']").find("input[type='radio'][name='"+roomType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='roomType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('roomType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(roomTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		}
   		//用车要求
   		var carJson = $("#carType").val();
   		if(carJson!='{}'){
   			var json = $.parseJSON(carJson);
   			$.each(json,function(i,n){
   				var carType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='carType'][value='"+carType+"']").attr("checked","checked");
   				}
   	   	   		if(carType==0){
   	   	   	   		var carTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='carType']").find("input[type='radio'][name='"+carType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='carType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('carType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(carTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   	   	   		// 大车座位要求
   	   	   		//var sit = $("#carSit").val();
   	   	   		var sit = n.info;
   	   	   		if(sit && sit!='undefined'){
   	   	   			var objSit = $("#eprice-travel-require-div-id").find("input[type='text'][name='carType']");
   	   	   			objSit.val(sit.value);
   	   	   		}
   			});
   		}
   		
   		// 导游要求
   		var guideJson = $("#guideTyp").val();
   		if(guideJson!='{}'){
   			var json =  $.parseJSON(guideJson);
   			$.each(json,function(i,n){
   	   			var guideTyp = n.type;
   	   		if(n.check=="checked"){
   	   			$("#eprice-travel-require-div-id").find("input[name='guideTyp'][value='"+guideTyp+"']").attr("checked","checked");
   	   		}
   	   	   		if(guideTyp==0){
   	   	   	   		var guideTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='guideTyp']").find("input[type='radio'][name='"+guideTyp+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='guideTyp']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('guideTyp',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(guideTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		}
   		// 领队要求
   		var leaderJson = $("#leaderType").val();
   		if(leaderJson!='{}'){
   			var json = $.parseJSON(leaderJson);
   			$.each(json,function(i,n){
   				var leaderType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='leaderType'][value='"+leaderType+"']").attr("checked","checked");
   				}
   	   	   		if(leaderType==0){
   	   	   	   		var leaderTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='leaderType']").find("input[type='radio'][name='"+leaderType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='leaderType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('leaderType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(leaderTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		}
   		// 景点要求
   		var attractionJson = $("#attractionType").val();
   		if(attractionJson!='{}'){
   			var json =  $.parseJSON(attractionJson);
   			$.each(json,function(i,n){
   				var attractionType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='attractionType'][value='"+attractionType+"']").attr("checked","checked");
   				}
   	   	   		if(attractionType==0){
   	   	   	   		var attractionTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='attractionType']").find("input[type='radio'][name='"+attractionType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='attractionType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('attractionType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(attractionTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		...
   		// 公务活动
   		var publicWordJson = $("#publicWordType").val();
   		if(publicWordJson!='{}'){
   			var json = $.parseJSON(publicWordJson);
   			$.each(json,function(i,n){
   				var publicWordType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='publicWordType'][value='"+publicWordType+"']").attr("checked","checked");
   				}
   	   	   		if(publicWordType==0){
   	   	   	   		var publicWordTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='publicWordType']").find("input[type='radio'][name='"+publicWordType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='publicWordType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('publicWordType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(publicWordTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   	   	   		// 公务活动安排城市
   	   	   		var publicWordCity = n.info;
   	   	   		if(publicWordCity && publicWordCity!='undefined'){
   	   	   			var objPublicWordCity = $("#eprice-travel-require-div-id").find("input[type='text'][name='publicWordType']");
   	   	   			objPublicWordCity.val(publicWordCity.value);
   	   	   		}
   			});
   		}
   		// 早餐要求
   		var breakfastJson = $("#breakfastType").val();
   		if(breakfastJson!='{}'){
   			var json = $.parseJSON(breakfastJson);
   			$.each(json,function(i,n){
   				var breakfastType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='breakfastType'][value='"+breakfastType+"']").attr("checked","checked");
   				}
   	   	   		if(breakfastType==0){
   	   	   	   		var breakfastTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='breakfastType']").find("input[type='radio'][name='"+breakfastType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='breakfastType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('breakfastType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(breakfastTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		}
   		// 正餐要求
   		var dinnerJson= $("#dinnerType").val();
   		if(dinnerJson!='{}'){
   			var json = $.parseJSON(dinnerJson);
   			$.each(json,function(i,n){
   				var dinnerType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='dinnerType'][value='"+dinnerType+"']").attr("checked","checked");
   				}
   	   	   		if(dinnerType==0){
   	   	   	   		var dinnerTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='dinnerType']").find("input[type='radio'][name='"+dinnerType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='dinnerType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('dinnerType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(dinnerTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   			});
   		}
   		// 签证要求
   		var visaNeedJson = $("#visaNeedType").val();
   		if(visaNeedJson!='{}'){
   			var json = $.parseJSON(visaNeedJson);
   			$.each(json,function(i,n){
   				var visaNeedType = n.type;
   				if(n.check=="checked"){
   					$("#eprice-travel-require-div-id").find("input[name='visaNeedType'][value='"+visaNeedType+"']").attr("checked","checked");
   				}
   	   	   		if(visaNeedType==0){
   	   	   	   		var visaNeedTitle = n.title;
   	   	   	   		// 如果
   	   	   	   		$("#eprice-travel-require-div-id").find("div[namecode='visaNeedType']").find("input[type='radio'][name='"+visaNeedType+"']").attr("checked","checked");
   	   	   	   		// 展开十字按钮
   	   	   	   		var obj2 = $("#eprice-travel-require-div-id").find("div[namecode='visaNeedType']");
   	   	   	   		obj2.find("a.add_seachcheck").before(addaeprice.service.otherAgain('visaNeedType',n.check));
   	   	   	   		obj2.find("input[type='text'][name='travelRequirements']").val(visaNeedTitle);
   	   	   	   		obj2.find("a.add_seachcheck").hide();
   	   	   		}
   	   	   		// 签证备注
   	   	   		var visaNeedInfo = n.info;
   	   	   		if(visaNeedInfo && visaNeedInfo!='undefined'){
   	   	   			var visaNeed = $("#eprice-travel-require-div-id").find("input[type='text'][name='visaNeedType']");
   	   	   			visaNeed.val(visaNeedInfo.value);
   	   	   		}
   			});
   		}
   		// 添加要求
   		var st = $("#otherRequirementsList").val();
   		if(st!='{}'){
   			var array = st.split(",");
   	   		var num = array.length;
   	   		if(num>0){
   	   			var inquiry2AddIpt = $("#eprice-b-form-id").find("a.inquiry2AddIpt");
   	   			for(var add=0 ; add<num;add++){
   	   				inquiry2AddIpt.before(addaeprice.service.inquiry2AddIpt());
   	   			}
   	   			// 添加值
   	   			for(var i = 0;i<array.length;i++){
	   				$("#eprice-b-form-id").find("input[name='otherRequirement']:eq("+i+")").val(array[i]);
	   			}
   		...
   		// 询价要求
   		var str = $("#epriceRequirementsList").val();
   		if(str){
   			var array = str.split(",");
   	   		var num = array.length;
   	   		if(num>1){
   	   			// 增加询价要求textArea的位置
   	   			var objzxtext = $("#eprice-b-form-id").find("span.inquiry_zxtext");
   	   			// 计算需要新增的询价要求textArea，然后添加。
   	   			for(var addtext = 0;addtext<(num-1);addtext++){
   	   				objzxtext.append(addaeprice.service.inquiryZxtext());
   	   			}
   	   			// 向询价要求textArea中添加值
   	   			for(var i = 0;i<array.length;i++){
   	   				$("#eprice-b-form-id").find("textarea[name='epriceRequirement']:eq("+i+")").text(array[i]);
   	   			}
   	   		}else{
   	   			$("#eprice-b-form-id").find("textarea[name='epriceRequirement']:eq(0)").text(array[0]);
   	   		}
   		}
   		// 护照类型
		var visaJson = $("#visaType").val();
		if(visaJson!='{}'){
			var json = $.parseJSON(visaJson);
			//var json = JSON.parse('{"option_0":{"name":"护照类型","type":"1","title":"因私护照"}}');
			$.each(json,function(i,n){
				var visaType = n.type;
				$("#eprice-travel-require-div-id").find("input[name='visaType'][value='"+visaType+"']").attr("checked","checked");
	   	   		if(visaType==0){
...
var addaeprice = {
		...
		dao : {
			 ...
			 * 用于二次询价
			 */
			otherAgain : function(m,check){
				var ifCheck = 'checked="checked"';
				if(check=="unchecked"){
					ifCheck = "";
				}
				var html = '<span class="seach_check seach_checkadd"><input type="checkbox"'+check+' name="'+m.name+'" value="0">';
					html += '<input type="text" name="travelRequirements"><a class="inquiry_zxdel">删除</a></span>';
					return html;
			}
...
//				$("#eprice-travel-require-div-id :checkbox").click(function(){
//					var $this = $(this);
//					var name = $this.attr("name");
//					var id = $this.attr("id");
//					// 设定单选
...
//				$("#eprice-travel-require-div-id :checkbox").live("click",function(){
//					var $this = $(this);
//					var name = $this.attr("name");
//					var id = $this.attr("id");
//					// 设定单选
...
//				}
//				
//				$("#eprice-c-div-p-id select[name=startTime1]").each(function(){
//						$(this).html(html1);
//					});
//					
//				$("#eprice-c-div-p-id select[name=startTime2]").each(function(){
//						$(this).html(html2);
//					});
...
//					$(nexts).find("option:lt("+i+")").hide();
			...
			other : function(m){
				...
				$($html).find(".inquiry_zxdel").click(function(){
					 ...
					 // 将焦点转到第一个radio
					 //$(this).parent().parent().find("input:radio:eq(0)").prop("checked",true);
				 	$(this).parent().remove();
				});
			...
			require : function(m){
				...
				//$($html).find("input:radio:eq(0)").prop("checked",true);
				
				//让复选框执行单选方式
				/*$($html).find("input:checkbox").live("click",function(){
					if($(this).prop("checked")){
						$(this).parents("p.seach_r").find("input:checkbox:checked").not(this).prop(false);
					}
				});*/
...
//					$("#eprice-travel-require-div-id").append(addaeprice.service.require(json[i]));
//				}
			...
			secondFormSerialize : function(){
				...
				// 地接计调数组
				var aoperatorUserIdList = new Array();
				var nu = 0;
				$("input[name=aoperatorUserId]").each(function(){
					aoperatorUserIdList[nu] = $(this).val();
					nu++;
				});
				param += "&aoperatorUserId="+aoperatorUserIdList;
				
				param += "&dgroupOutDate="+$("#dgroup-out-date-id").val();//预计出团日期
				param += "&outAreaName="+$("#eprice-b-form-id input[name=outAreaName]").val();//出境口岸
				param += "&outAreaId="+$("#eprice-b-form-id select[name=outAreaId]").val();//出境口岸ID
				//线路国家id
				...
				param += "&onceAgain="+$("#id_onceAgain").val();//是否再次询价标记
				...
				//param += "&travelCountry="+$("#eprice-b-form-id select[name=travelCountryId] option[value="+travelCountryId+"]").html();//线路国家name
...
//				param += "&salerTripFileId="+$("#saler-trip-file-id-id").val();//行程文档
				var fileNum = 0;
...
//							var o = {};
//							o.name=m.name;
////							var ss = $("#eprice-travel-require-div-id input[name="+m.code+"]:checked");
////							var ss2 = $("#eprice-travel-require-div-id input[name="+m.code+"]:checked").val();
////							
////							$("#eprice-travel-require-div-id input[name="+m.code+"]:checked").each(function(){
////							});
////							
//							o.type = $("#eprice-travel-require-div-id input[name="+m.code+"]:checked").val();
//							if(o.type && o.type!=0){//非其他选项
//								var op = m.options["option_"+o.type];
//								o.title = encodeURIComponent(op.name);
//								
//								if(op.info){
//									o.info = op.info;
//									var div = $("#eprice-travel-require-div-id div[name=require][namecode="+m.code+"]");
//									o.info.value = encodeURIComponent($(div).find("input[pname="+m.code+"-"+op.value+"]").val());
//								}
//							}else{//其他选项
//								o.title = encodeURIComponent($("#eprice-travel-require-div-id input[name="+m.code+"]").next("input:text").val());
//							}
//							
//							json[m.code]=o;
//						}
//				);
				...
				/*$("#eprice-travel-require-div-id div[name=require]").each(function(){
					json += '"'+$(this).attr()namecode
					$(this).find("input")
				});*/
...
//				}
					...
					"afterInvalid":function(event,data){
						...
						if(-1 == $.inArray(data,Array_default)){
							...
							if(isIncluded){
								...
								$("#countryShow").append('<a>{0}<input type="hidden" name="travelCountryId" value="{1}"/><input type="hidden" name="travelCountry" value="{2}"/></a>'.replace("{0}",data).replace("{1}",selectValue).replace("{2}",data));
							}
						}
					}
				});
			 ...
			 */
			otherAgain : function(m,check){
				var $html = $(addaeprice.dao.otherAgain(m,check));
				$($html).find(".inquiry_zxdel").click(function(){
					 $(this).parent().parent().find('a').show();
					 // 将焦点转到第一个radio
					 $(this).parent().parent().find("input:radio:eq(0)").prop("checked",true);
		...
		ajax : {
			...
			firstSubmit : function(){
				...
				the_param += "&onceAgain="+$("#id_onceAgain").val();//是否再次询价标记
			...
			secondSubmit : function(){
				...
				$.ajax({
					type : "POST",
					url : contextPath+"/eprice/manager/ajax/project/twosave",
					data : the_param,
					dataType : "text",
					success : function(html){
						var json;
			...
			thirdSubmit : function(){
				...
				$.ajax({
					type : "POST",
					url : contextPath+"/eprice/manager/ajax/project/treesave",
					data : the_param,
					dataType : "text",
					success : function(html){
						var json;
