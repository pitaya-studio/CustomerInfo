var rebates = {
	showDetail: function(rebatesId){
		window.open(contextPath + "/airticketRebates/showAirticketRebatesDetail/" + rebatesId);
	},
	...
	ajax:{
		saveRebatesAjax: function(rebatesList, orderId, orderType){
			...
			$.ajax( {
				type : "POST",
				url : contextPath + "/order/comdiscount/saveComdiscount",
				data : {
				...
				success : function(msg) {
					if(msg == "success"){
						jBox.tip("保存成功！");
						window.location.href = contextPath + "/order/comdiscount/showComdiscountList?orderId=" + orderId + "&productType=" + 7;
					}else if(msg==""){
						jBox.tip("不可以重复提交！");
					}else{
						jBox.tip("保存失败！" + msg);
		...
		cancleRebatesAjax:function(rid){
			$.ajax( {
				type : "POST",
				url : contextPath + "/order/comdiscount/cancleRebates",
				data : {
					rid : rid
				},
				success : function(msg) {
...
function totallend(){
	...
	for(var key in currencyCalObj){
		...
		if(currencyCalObj[key].toString()==0){
			...
			leijiArray.push(cObj.id+":"+currencyCalObj[key].toString());
		}
	}
