var rebatesReview = {
	showReviewDetail: function(rebatesId){
		window.open(contextPath + "/order/rebates/review/showRebatesReviewDetail/" + rebatesId);
	},
	showRebatesReview: function(rebatesId,rid,userLevel){
		$.ajax({
			type : "POST",
			async: false, 
			...
			success : function(msg) {
				if(msg == "true"){
					window.open(contextPath + "/order/rebates/review/showRebatesReview/" + rebatesId + "/" + userLevel);
				}else if(msg == "false"){
					jBox.tip("此记录已审核！");
					location.reload();
				}else{
