var rebates = {
	...
	showDetail: function(rebatesId,orderUuid){
		window.open(contextPath + "/order/rebates/showHotelRebatesDetail/" + rebatesId+"/"+orderUuid);
	},
	...
	ajax:{
		saveRebatesAjax: function(rebatesList, orderId, orderType){
			$.ajax( {
				type : "POST",
				url : contextPath + "/order/rebates/saveHotelRebates",
				data : {
				...
				success : function(msg) {
					if(msg == "success"){
						jBox.tip("保存成功");
						window.location.href = contextPath + "/order/rebates/showHotelRebatesList?orderId=" + orderId + "&orderType=" + orderType+"&orderUuid="+$("#orderUuid").val();
					}else{
						jBox.tip("保存失败！" + msg);
					}
				}
		...
		getRebatesStatusAjax:function(orderId, orderType,orderUuid){
			$.ajax( {
				...
				success : function(msg) {
//					if(msg == "true"){
						window.open(contextPath + "/order/rebates/addHotelRebates/" + orderId + "/" + orderType + "/" +orderUuid);
//					}else{
