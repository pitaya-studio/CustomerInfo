var rebates = {
	...
	showDetail: function(rebatesId){
		window.open(contextPath + "/order/rebates/showRebatesDetail/" + rebatesId);
	},
	...
	saveRebates: function(){
		...
		$("tr[group^='travler']").each(function(index, element) {
			...
			if(rebatesDiff != "" && isFloat(rebatesDiff) > 0 && costname == ""){
				...
				rebatesList.push(rebates._getRebates(orderId, travelerId, currencyId, isFloat(rebatesDiff), oldRebates, newRebates, costname, remark, orderType));
			}
		});
		$(".gai-price-ol li").each(function(index, element) {
			...
			if(rebatesDiff != "" && isFloat(rebatesDiff) > 0  && costname == ""){
				...
				rebatesList.push(rebates._getRebates(orderId,"",currencyId,rebatesDiff, "", newRebates,costname,remark,orderType));
			};
		});
	...
	_getRebates:function(orderId,travelerId,currencyId,rebatesDiff,oldRebates,newRebates,costname,remark,orderType){
	...
	ajax:{
		saveRebatesAjax: function(rebatesList, orderId, orderType){
			$.ajax( {
				type : "POST",
				url : contextPath + "/order/rebates/saveRebates",
				data : {
				...
				success : function(msg) {
					if(msg == "success"){
						jBox.tip("保存成功");
						window.location.href = contextPath + "/order/rebates/showRebatesList?orderId=" + orderId + "&orderType=" + orderType;
					}else{
						jBox.tip("保存失败！" + msg);
					}
				}
		...
		getRebatesStatusAjax:function(orderId, orderType){
			$.ajax( {
				type : "POST",
				async: false, 
				url : contextPath + "/visa/order/validRebates",
				data : {
					orderId : orderId,
					orderType : orderType
				},
				success : function(msg) {
					if(msg == "true"){
						window.open(contextPath + "/visa/order/addRebates/" + orderId + "/" + orderType);
					}else{
						if(msg == "false1"){
							jBox.tip("上次返佣申请流程未结束，不能再次申请！");
						}else if(msg == "false2"){
							jBox.tip("签证返佣结算单已锁定，不能申请返佣数据");
						}
					}
				}
			});
