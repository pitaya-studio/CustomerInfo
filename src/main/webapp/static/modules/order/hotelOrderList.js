...
$(function(){
    ...
    $("#contentTable").delegate(".tuanhao","click",function(){
        $(this).addClass("on").siblings().removeClass('on');
        $('.riqi_cen').removeClass('onshow');
        $('.tuanhao_cen').addClass('onshow');
    });
    $("#contentTable").delegate(".riqi","click",function(){
         $(this).addClass("on").siblings().removeClass('on');
         $('.tuanhao_cen').removeClass('onshow');
         $('.riqi_cen').addClass('onshow');
        
    });
    //控房单号和产品名称相互切换
    $("#contentTable").delegate(".kongfang","click",function(){
    	$(this).addClass("on").siblings().removeClass('on');
    	$('.chanpin_cen').removeClass('onshow');
    	$('.kongfang_cen').addClass('onshow');
    });
    $("#contentTable").delegate(".chanpin","click",function(){
    	$(this).addClass("on").siblings().removeClass('on');
    	$('.kongfang_cen').removeClass('onshow');
	...
	if ($("#countryUuid").val() != "") {
		getAjaxSelect("island", $("#countryUuid"));
	}
	if ($("#islandUuid").val() != "") {
		getAjaxSelect("hotel", $("#islandUuid")[0]);
	}
...
function getAjaxSelect(type,obj){
	$.ajax({
		type: "POST",
	   	url: contextPath + "/hotelControl/ajaxCheck",
	   	data: {
				"type" : type,
				"uuid" : $(obj).val()
			  },
	   	...
	   	success: function(data){
	   		...
	   		if(data){
	   			if(type=="hotel"){
		   			...
		   			$("#hotelUuid").val($("#hotelUuidValue").val());
	   			}else if(type=="roomtype"){
	   				$.each(data.roomtype,function(i,n){
	   					var html = "<option value='" + n.uuid + "'>" + n.roomName + "</option>";
	   				...
	   				$("#islandUuid").val($("#islandUuidValue").val());
...
function cancelOrder(orderUuid) {
    ...
    var submit = function(v, h, f){
        ...
        if(f.description.length > 100){
            ...
            $.ajax({
                type: "POST",
                url: contextPath + "/hotelOrder/cancelOrder",
                data: {
                	orderUuid : orderUuid,
...
function invokeOrder(orderUuid) {
	$.ajax({
        type: "POST",
        url: contextPath + "/hotelOrder/invokeOrder?dom=" + Math.random(),
        data: {
        	orderUuid : orderUuid
        },
        success: function(msg) {
...
function changepayVoucher(payUuid, orderUuid){
	window.open (contextPath + "/hotelOrder/modifypayVoucher/" + payUuid + "/" + orderUuid);
}
...
function query(orderStatus) {
	...
	$('#searchForm').attr("action", contextPath + "/hotelOrder/list/" + orderStatus + ".htm");
	$('#searchForm').submit();
}
...
function viewExitGroup(orderUuid) {	
	window.open(contextPath + "/hotelOrder/viewExitGroup?flowType=8&orderUuid=" + orderUuid);
}
/**
 * 改价
 * Date 2015-6-29
 * @param orderUuid 订单UUID
 */
function viewHotelChangePriceList(orderId,orderUuid) {	
	window.open(contextPath + "/island/review/hotelChangePriceList?orderUuid="+orderUuid+"&orderId="+orderId+"&productType=11&flowType=10");
}
/**
 * 团队退款
 * param id   订单唯一标识
...
function viewGroupRefund(id,orderType,orderUuid) {	
	window.open(contextPath + "/orderCommon/manage/viewHotelGroupRefund?productType=11&&flowType=1&orderId="+id+"&orderUuid="+orderUuid);
}
...
function changeGroup(orderId){
	window.open(contextPath + "/orderTransFerGroup/transFerGroupList/"+orderId);
}
/**
 * 申请借款列表
 * param id   订单唯一标识
...
function viewBorrowingList(id,orderType,uuid) {	
	window.open(contextPath + "/island/review/viewHotelBorrowingList?productType=11&flowType=19&orderId="+id+"&orderUuid="+uuid);
}
/**
 * 订单页面查看订单发票信息列表
 * param orderNum   订单号
...
var rebatesInfo = {
	rebatesOrder: function(orderId,orderType,uuid){
		window.open(contextPath + "/order/rebates/showHotelRebatesList?orderId=" + orderId + "&orderType=" + orderType+"&orderUuid="+uuid);
	}
};
...
function doOrderLockStatus(uuid, actionName, tipMsg){
	$.ajax({
		type: "POST",
        url: contextPath + "/hotelOrder/" + actionName,
        data: {
            "uuid" : uuid
        },
        success: function(msg) {
...
function setOrderFiles(orderUuid, fileIds) {
	$.ajax({
        type: "POST",
        url: contextPath + "/hotelOrder/setOrderFiles",
        data: {
        	orderUuid : orderUuid,
        	fileIds : fileIds
        },
...
function downloadFiles(orderUuid) {
	$.ajax({
        type: "POST",
        url: contextPath + "/hotelOrder/getFilesInfo?dom=" + Math.random(),
        data: {
        	orderUuid : orderUuid
        },
        success: function(msg) {
...
function downloadGroupFiles(groupUuid) {
	...
	if(!orderUuids || orderUuids == "") {
		...
		$.ajax({
	        type: "POST",
	        url: contextPath + "/hotelOrder/downloadGroupFiles?dom=" + Math.random(),
	        data: {
