...
function loadAsynchronousData() {
	$.ajax({
		type :"POST",
		url : $("#ctx").val() + "/hotelOrder/loadAsynchronousData",
		cache : false,
		data : {hotelOrderId : $("#hotelOrderId").val()},
			...
			if (changePriceData && changePriceData.length > 0) {
				...
				$(changePriceData).each(function(index, obj) {
					changePriceHtml += '<tr group="travler1">' 
											+ '<td class="tc">' + obj.createDate + '</td>' 
											+ '<td class="tc">' + getData('hotel_traveler', 'id', obj.travelerId, 'name') + '</td>' 
			...
			if (rebatesData && rebatesData.length > 0) {
				...
				$(rebatesData).each(function(index, obj) {
					rebatesHtml += '<tr group="travler1">' 
										...
										+ '<td class="tr">' + getData('hotel_money_amount', 'serialNum', obj.totalMoney, 'amount') + '</td>' 
...
function getData(tableName, queryName, queryValue, getName) {
	...
	$.ajax({
		type :"POST",
		url: $("#ctx").val() + "/hotelOrder/getData?dom=" + Math.random(),
		async: false,
		cache : false,
		data : {
				tableName : tableName,
