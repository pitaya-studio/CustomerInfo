...
$.extend(Datepicker.prototype, {
	...
	***
	...
	_generateHTML: function(inst) {
		...
		var isChickArr = this._get(inst, 'isChickArr');
			...
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						if(isChickArr.length!=0){
							for(var i=0; i<isChickArr.length; i++){
								var disabledate = this._daylightSavingAdjust(isChickArr[i]);
...
$(function(){
	$.fn.datepickerRefactor = function(options,dateStart,dateEnd) { //datepickerRefactor
		...
		$a.live("click",function(){
			...
			$targetVal = $(opts.target).val();
			writeDate($targetVal,dateStart,dateEnd);
		});
		
		function writeDate($targetVal,dateStart,dateEnd){
			...
			$(".grouprow").each(function(){
				...
				var flag = false;
				for(var i=0;i<datesArray.length-1;i++){
					if(datesArray[i]==value){
						flag=true;
						break;
					}
				}
				var rownum=$(this).attr('id').replace('grouprow','');
				if(flag){
					$(this).show();
				}else{
					$(this).hide();
					$(this).nextAll('[class^='+rownum+' ]').hide();
				}
				//alert(value);
			});
		}
