...
// obj需要添加滚动条的对象id w滚动条宽度 className滚动条样式名称
// obj元素 必须指定高度，并设置overflow:hidden;
// 如要兼容IE6 必须给obj元素 指定 overflow:hidden;
function jsScroll(obj, w, className,scrollId)
{
	alert(1);
	...
	if(!className) {
		obj.scrollBar.style.backgroundColor = 'rgb(255,255,255)';
		obj.scrollBarIndex.style.borderRadius='3px';//兼容ie9+
		obj.scrollBarIndex.style.backgroundColor = 'rgb(222, 222, 222)';
		obj.scrollBarIndex.style.zIndex='110';
	}
	if(scrollId){
		obj.scrollBarIndex.id = scrollId;
	}
	...
	//鼠标悬浮事件
	obj.scrollBarIndex.onmouseover = function(evt){
		if(this.scrollDiv.scrollBar.className == '') {
			this.scrollDiv.scrollBarIndex.style.backgroundColor = 'rgb(87,87,87)';
		}
		return false;
	}
	//鼠标移除事件
	obj.scrollBarIndex.onmouseout = function(evt){
		if(this.scrollDiv.scrollBar.className == '') {
			this.scrollDiv.scrollBarIndex.style.backgroundColor = 'rgb(204, 204, 204)';
		}
		return false;
	}
	//拖动滚动条滚动
