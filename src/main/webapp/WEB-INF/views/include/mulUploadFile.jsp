...
var arrIndex = 0;
var clickCount = 0;
//错误提示   Add by songyang 2016年3月14日16:32:37
var uperror = function (file,errorCode,errorMsg){
    switch (errorCode) {  
    case -110:  
...
$(document).ready(function() {
 ...
 //mark fix uploadify chrome error.
	...
	var uploader = "";
	if ('${addFlag}' == 'addFlag') {
		uploader = '${ctx}/MulUploadFile/upload;jsessionid=<%=session.getId()%>?addFlag=addFlag';
	} else {
    ...
    $('#file_upload').uploadify({
        ...
        'onUploadStart' : function (file) {
        	if (file.name.length > 50) {
        		top.$.jBox.tip('文件名称过长', 'error');
        		this.cancelUpload(file.id);
        		$('#' + file.id).remove();
        	}
        },
        'onSelectError': uperror,
...
<%--    $('#file_upload').uploadifySettings('queueData',{'rnd':Math.random(),'innerCode':'00001'});--%>
    ...
    function setUpoaldPath() {
		...
		if(divCon.length > 0) {
		    for(var i = 0; i < dataArr.length; i++) {
		    	...
		    	if(docId && docId != null){
		    	divCon.append("<input type='hidden' name='docID' value='" +docId+ "' />")
    ...
    function submitDoc2Order(){
    	alert(dataArr);
    	return true;
    }
    
    //bug16185 防止用户双击，出现重复提交  modify by yang.wang  2016.10.9
    function successUpload(obj) {
    	if (clickCount == 0) {
    		$(obj).prev().eq(2).click(); 
    		setUpoaldPath();
    		clickCount++;
...
<p class="file_btn">
    ...
    <a onclick="successUpload(this);" href="javascript:void(0)" class="successUpload"></a>
</p>
