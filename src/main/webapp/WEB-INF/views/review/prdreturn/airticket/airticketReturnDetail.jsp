						...
						function() {
							if ($(this).prop("checked")) {
								...
								for ( var i = 0; i < $(".table_borderLeftN")
										...
										if($(".table_borderLeftN").find("input[type='checkbox']")[i].disabled == false){
											$(".table_borderLeftN").find("input[type='checkbox']")[i].checked = true;
										}
								}
							} else {
...
// 			var travelerId = str[i];
				$.ajax({
		            type: "POST",
		            url: "${ctx}/airticketreturnnew/beforeAddReview",
		            data: {
			            orderId : orderid,
			            travelerids : str		            
		            },
		            	...
		            	if(msg.isMutexBoolean == false){
		            		...
		            		if(msg.canCancel == false){
								...
								if(v=='ok'){
										$.ajax({
								            type: "POST",
								            url: "${ctx}/airticketreturnnew/cancelOtherReview",
								            data: {
									            ids : msg.ids,
									            ids2 : msg.ids2		            
								            },
...
<body>
	...
	<form id="airticketReturnDetailForm_Id" method="post"
		action="${ctx}/airticketreturnnew/startAirticketReturn">
					...
					<tr>
						...
						<c:if test="${travelInfo.isAirticketFlag==null || travelInfo.isAirticketFlag=='0'}">
							<td class="table_borderLeftN">
								<input name="activityId" type="checkbox" disabled="disabled"/><br><br>
							</td>
						</c:if>
		...
		<div class="ydBtn ydBtn2">
			<a class="ydbz_s gray" href="${ctx}/airticketreturnnew/airticketReturnList?orderId=${airticketReturnDetailInfoMap.id}&flowType=3&orderType=7">取消</a><a class="ydbz_s"
				onClick="sendData(this);">提交</a>
		</div>
		<!-- href = "sendData(this)" -->
	</form>
</body>
