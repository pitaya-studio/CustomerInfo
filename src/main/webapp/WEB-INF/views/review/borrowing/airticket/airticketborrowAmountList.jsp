	...
	top.$.jBox.confirm('确认要取消流程申请吗？','系统提示',function(v){
		if(v=='ok'){
			$.ajax({                 
				cache: true,                 
				type: "POST",                 
				url:g_context_url+ "/activityOrder/lendmoney/cancelBorrowAmount",                 
				data:{
					id:id,
					status:status
				},                
...
<body>
				<!--右侧内容部分开始-->
                <div class="ydbz_tit orderdetails_titpr">订单 > 机票 > 借款记录
                	<a href="${ctx}/activityOrder/lendmoney/airticketLendMoneyApply?orderId=${orderId}"  class="ydbz_x"   target="_blank">申请借款</a>
                </div>
						...
						<td class="p0">
							<dl class="handle">
								...
								<dd class="">
									<p>
										<span></span> 
										<a href="${ctx}/activityOrder/lendmoney/airticketLendMoneyByReviewId?orderId=${orderId }&reviewId=${plist.reviewId }" target="_blank">借款详情</a>
