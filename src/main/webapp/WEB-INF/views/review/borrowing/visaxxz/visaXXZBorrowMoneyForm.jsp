...
<script type="text/javascript">
	...
	function subForm() {
		/*
		 * 0407forbug_注释还款日期操作_djw
		 */
		//270-djw-对还款日期的校验,规则:还款日期设置为1且还款日期为空时进行
		 var eltRefundDate=$("#refundDate").val();//还款日期的值
		 var refundDateOption='${refunDateSel}';
		//alert(refundDateOption);
		 if((refundDateOption==1) &&(eltRefundDate==''||eltRefundDate==null)){
		...
		if($(".borrowPrice[value!=''][value!='0'][value!='-']").length) {
			$.ajax({
				cache: true,
	            type: "POST",
	            url:g_context_url+"/visa/xxz/borrowmoney/createVisaXXZBorrowMoney",
	            ...
	            success: function(data) {
	            	...
	            	if(data.appResult==1){
	            		top.$.jBox.tip("申请成功！");
	            		window.location.href=g_context_url+"/visa/xxz/borrowmoney/borrowMoneyRecord4XXZactivity?orderId="+data.orderId;
	            	}else{
	            		top.$.jBox.tip("申请失败！");
	            	}
	            }
...
<!-- 					</div> -->
				...
				<div class="allzj">
					<tr>
						...
						<c:if test="${ refunDateSel eq '1' }">
						<span class="xing">*</span>
						</c:if>
						还款日期：
						</td>
						<td>
							<input maxlength="20" id="refundDate" name="refundDate" readonly="readonly"
								value="" class="dateinput valid" onfocus="WdatePicker()" type="text">
						</td>
					</tr>
				</div>
				<!-- 270_添加还款日期_djw -->
