...
<body>
		...
		<!--右侧内容部分开始-->
		<div class="mod_nav">审批 > 签证借款 > 签证借款审批</div>
		...
		<div class="ydbz_tit">
			<span class="fl">签证借款</span><span class="fr wpr20">报批日期：
			<fmt:formatDate value="${revCreateDate }" pattern="yyyy-MM-dd HH:mm:ss"/>
			</span>
      		</div>
		<table id="contentTable" class="activitylist_bodyer_table">
		   ...
		   <tbody>
			  <tr>
				 ...
				 <td class="tr">${revCurrency.currencyMark }<fmt:formatNumber value="${revBorrowAmount }" pattern="###,##0.00"/> </td>
		...
		<div class="allzj tr f18">
                  <div class="all-money">借款总金额：<font class="f14"></font><span class="f20">${revCurrency.currencyMark }</span> 
                  <fmt:formatNumber value="${revBorrowAmount}" pattern="###,##0.00"/>
                  </div>
		</div>
		...
		<form id="searchForm" action="${ctx}/visa/borrowMoney/review/batchReviewVisaBorrowMoneybyBatchNo" method="post">
				<!-- 添加提交请求所需数据 -->
				<!-- 1：审核通过按钮，0：驳回按钮 -->
