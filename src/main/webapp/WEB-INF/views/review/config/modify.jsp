...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("#needNoReview").click(function () {
			if ($(this).attr("checked")=='checked') {
				$(".title input[name='edit-main-flow']").addClass("edit-disabled");
				$(".title input[name='edit-main-flow']").attr("disabled","disabled");
			} else {
				$(".title input[name='edit-main-flow']").removeClass("edit-disabled");
				$(".title input[name='edit-main-flow']").removeAttr("disabled");
			}
		});
	});
	$(document).ready(function(){
                        ...
                        <div class="main-flow-container">
							...
							<div class="special-need">
								<ul  name="costPaymentApproval">
									...
									<li><input id="paymentEqualsCost" name="specialType" type="checkbox" disabled value="3" <c:if test="${(not empty reviewCostPaymentConfiguration) and (reviewCostPaymentConfiguration.isPaymentEqualsCost eq 1)}">checked</c:if>/><label>付款申请金额须等于实际成本金额</label></li>
								</ul>
							</div>
                            ...
                            <div class="title">
                                <span>流程配置</span>
                                <em>
									<input value="显示历史版本" name="display-history-img" class="btn btn-primary "
										   type="button">
									<input value="编辑" name="edit-main-flow" onclick="editModel('${ctx}','${needNoReviewProcessKey}', '${modelId}','${processKey}','${serialNumber}' )" class="btn btn-primary "
                                           type="button">
                                </em>
                            </div>
                            <div class="flow-body-container">
								<c:if test="${!empty firstDeploymentMap}">
                                <div class="block">
                                    <img src="${ctx}/sys/review/configuration/model/diagram/${firstDeploymentMap.modelId}" onerror="javascript:this.src='${ctxStatic}/images/configure-flow-no-pic.jpg';"/>
                                </div>
								<p>最新编辑时间：<fmt:formatDate value="${firstDeploymentMap.deployDate}" pattern="yyyy-MM-dd HH:mm:ss" /></p>
								</c:if>
								<div class="dash-line"></div>
								<div class="history-flow">
									<c:if test="${!empty deploymentMaps}">
									<c:forEach var="deploymentMap" items="${deploymentMaps}">
										<div class="block">
											<img src="${ctx}/sys/review/configuration/process/diagram/${deploymentMap.deployId}" onerror="javascript:this.src='${ctxStatic}/images/configure-flow-no-pic.jpg';"/>
										</div>
										<p>启用时间：<fmt:formatDate value="${deploymentMap.deployDate}" pattern="yyyy-MM-dd HH:mm:ss" /></p>
									</c:forEach>
									</c:if>
								</div>
                            </div>
