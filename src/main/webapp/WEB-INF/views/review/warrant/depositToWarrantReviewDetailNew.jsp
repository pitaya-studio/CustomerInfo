...
<body>
	...
	<div class="ydbz_tit">订单详情</div>
		...
		<table id="contentTable" class="activitylist_bodyer_table">
			...
			<tbody>
				<tr>
					<td>${map.travellerName}</td>
					<td>${map.orderCreatorName}</td>
					<td class="tc">
						<c:if test="${not empty visaOrder.createDate}">
                       		<fmt:formatDate pattern="yyyy-MM-dd" value="${visaOrder.createDate}"/>
                       	</c:if>
					</td>
					<td>${map.priceCurrency}</td>
					<td class="tr">${map.guaranteeAmount}</td>
					<td>
						<c:if test="${not empty visaStatus}">
							<c:choose>
								<c:when test="${visaStatus eq '0'}">未送签</c:when>
								<c:when test="${visaStatus eq '1'}">送签</c:when>
								<c:when test="${visaStatus eq '2'}">约签</c:when>
								<c:when test="${visaStatus eq '3'}">出签</c:when>
								<c:when test="${visaStatus eq '4'}">申请撤签</c:when>
								<c:when test="${visaStatus eq '5'}">撤签成功</c:when>
								<c:when test="${visaStatus eq '6'}">撤签失败</c:when>
								<c:when test="${visaStatus eq '7'}">拒签</c:when>
								<c:when test="${visaStatus eq '8'}">调查</c:when>
								<c:when test="${visaStatus eq '9'}">续补资料</c:when>
							</c:choose>
						</c:if>
					</td>
					<td>
						<c:if test="${not empty map.extend2}">
							<c:choose>
								<c:when test="${map.extend2 eq 1}">担保</c:when>
