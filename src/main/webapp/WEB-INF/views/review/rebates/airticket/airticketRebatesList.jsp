...
<!--右侧内容部分开始-->
  ...
  <c:choose>
  			<c:when test="${fns:getUser().company.uuid eq '049984365af44db592d1cd529f3008c3'}"> 
				...
				<div class="ydbz_tit orderdetails_titpr">宣传费记录<a href="javascript:rebates.addRebates(${orderId},'7');"   class="ydbz_x">申请宣传费</a>
		</c:when>
     		<c:otherwise>
     			<div class="mod_nav">订单 > 机票 > 返佣记录</div>
				<div class="ydbz_tit orderdetails_titpr">返佣记录<a href="javascript:rebates.addRebates(${orderId},'7');"   class="ydbz_x">申请返佣</a>
      		</c:otherwise>
</c:choose>   
	</div>
	   ...
	   <tbody>
	   	<c:forEach items="${rebatesList}" var="rebates">
	   		<tr>
				...
				<td class="tr">
					<c:if test="${empty fns:getPreRebatesAppliedNew(rebates.rid)}">——</c:if>${fns:getPreRebatesAppliedNew(rebates.rid)}
				</td>
				<td class="tr">
