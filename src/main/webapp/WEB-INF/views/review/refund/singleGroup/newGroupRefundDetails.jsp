...
describe:单团订单，散拼订单，游学订单，大客户订单，自由行订单 申请退款页面
...
function refunds(){
	$('.refund-price-btn').click(function() {
		var html = '<li><i><input type="hidden" name="travelerId" value="0"><input type="text" name="refundAmount" class="gai-price-ipt1" flag="istips" /><span class="ipt-tips ipt-tips2">款项</span></i>&nbsp;';
	...
	if(b){
		...
		var orderTotal=$("#orderTotal").val();
	   	selects.each(function(index, element) {
			...
			if(checkinput.length==0){
				...
				if((n !="" && n !=undefined &&n >0) && (ra !="" &&ra !=undefined) ){
					refund.push(n);
					currency_id.push(s);
					remark.push(remarks);
					refundAmount.push(ra);
	   ...
	   if(flag){
		   if (submitNum == 1) {
			   ...
			   $.ajax({
				   type: "POST",
				   url: "${ctx}/singleOrder/refund/checkSingleGroupRefund",
				   data: {
				   ...
				   success: function(msg){
					   if(msg.isMutexBoolean == false){
						   startSingleGroupRefund(objArray, travelerId, refundAmount, currency_id, refund, remark);
					   }else{
						   $.jBox.tip(msg.showMsg, "warning");
						   submitNum--;
					   }
				   }
			   });
 ...
 * @param remark
 */
function startSingleGroupRefund(objArray, travelerId, refundAmount, currency_id, refund, remark) {
	//保存退团信息，发起审批流程
	$.ajax({
		type: "POST",
		url: "${ctx}/singleOrder/refund/startSingleGroupRefund",
		data: {
			refundRecordsStr : JSON.stringify(objArray),
		...
		success: function(result){
			if(result.msg == "success"){
				...
				window.location.href ="${ctx}/singleOrder/refund/newGroupRefundList?productType=${productType}&&orderId=${orderId}&flowType=1";
			}else{
				if (result.reply != "") {
					$.jBox.tip(result.reply, "error");
				} else {
					$.jBox.tip("申请失败", "error");
				}
				submitNum--;
...
<div class="bgMainRight_right">
	<div class="ydbzbox fs">
			...
			<table id="contentTable" class="activitylist_bodyer_table">
			   ...
			   <tr>
					...
					<td width="90%" colspan="5" class="p0" style="border:none">
						...
						<tr>
                        	<td width="22%" class="refundtd"><input type="text" name="refundAmount" maxlength="128" class="inputTxt"/><div class="pr"><i title="添加款项" class="gaijia-add"></i></div>
                        	</td>
			...
			<ol class="gai-price-ol">
               	<li >
					<i>
						...
						<input type="text" name="refundAmount" class="gai-price-ipt1" flag="istips" /><span class="ipt-tips ipt-tips2">款项</span>
					</i>
                	<i>
                		<select style="width:90%;" class="selectrefund" name="currency">
							<c:forEach items="${currencyList}" var="cur" varStatus="b">
		...
		<div class="allzj tr f18">
			<div class="all-money">退款总金额：<span></span></div>
