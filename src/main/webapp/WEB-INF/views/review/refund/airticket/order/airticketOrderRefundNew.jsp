...
<script type="text/javascript"
	src="${ctxStatic}/modules/order/airticket/airticketOrderRefundNew.js"></script>
				...
				<c:when test="${orderDetail.airType == 1 }">
					...
					<c:forEach items="${orderDetail.flightInfoList }" var="flightInfo">
						<div class="title_samil">
							第${flightInfo.orderNumber }段：<c:choose>
 ...
 -->
				<c:if test="${reviewId == null}">
					...
					<c:forEach items="${orderDetail.travelInfoList }" var="v">
						<tr>
							<td width="10%">
							<c:if test="${v.delFlag == 6 }">
							<div class="ycq yj" style="margin-top:1px;">
						 		已改签
							</div>
							</c:if>
							<c:if test="${v.isAirticketFlag == 0 }">
								<div class="ycq yj" style="margin-top:1px;">
							 		已退票
								</div>
							</c:if>
		...
		<div class="ydBtn ydBtn2">
			<a class="ydbz_s gray" id="bt_cancel">取消</a>
			<input type="button"  value="申请退款" id="bt_submit"  class="btn btn-primary">
		</div>
	</div>
	<!--右侧内容部分结束-->
</body>
