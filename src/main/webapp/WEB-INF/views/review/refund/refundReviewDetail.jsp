...
<html>
<head>
<title>退款-<c:if test="${flag == 0}">审批</c:if><c:if test="${flag != 0}">详情</c:if></title>
...
// 	var s = 0;
	...
	//驳回
	function jbox_bohui_refund3(revId,result,denyReason,moneyAmount,orderTypeSub,orderId,currencyId){
		...
		$.jBox(html, { title: "驳回原因",buttons:{"提 交":"1"}, submit:function(v, h, f){
			if (v=="1"){
				...
				$.ajax({
	            	type : "post",
					url : "${ctx}/refundnew/refundReview",
					data:{ 
						"revId":revId,
						"result" : result,
	...
	function review(revId,result,denyReason,moneyAmount,orderTypeSub,orderId,currencyId){
		$("#failBtn").attr('disabled',true);
		$("#succBtn").attr('disabled',true);
...
<body>
	<!--右侧内容部分开始-->
	<div class="mod_nav">审批 > 退款审批 > 退款审批</div>
		...
		<table border="0" width="98%" style="margin-left: 25px">
			<tbody>
				...
				<tr>
					...
					<!-- C460V3 签证订单团号  改为订单所关联的产品团号  -->
					<c:if test="${orderDetail.prdtype ne 6}">
						<td class="mod_details2_d2">${orderDetail.groupno}</td>
					</c:if>
					<c:if test="${orderDetail.prdtype eq 6}">
				...
				<tr>
					...
					<td class="mod_details2_d2">
						<%--签证的订单状态少，而且叫法不一样--%>
						<c:choose>
							<c:when test="${orderDetail.prdtype==6}">
								<c:choose>
									<c:when test="${orderDetail.orderstatus == 1}">未收款</c:when>
									<c:when test="${orderDetail.orderstatus == 5}">已收款</c:when>
									<c:when test="${orderDetail.orderstatus == 3}">预定</c:when>
									<c:when test="${orderDetail.orderstatus == 99}">已取消</c:when>
								</c:choose>
							</c:when>
							<c:otherwise>
			...
			<c:if test="${orderDetail.type == 2 }">
				...
				<table width="90%" border="0">
					<tbody>
						...
						<tr>
							<td class="mod_details2_d1">航空公司：</td>
							<td class="mod_details2_d2">${fns:getAirlineNameByAirlineCode(orderDetail.flightInfoList[1].airlines)}</td>
							<td class="mod_details2_d1">舱位等级：</td>
	...
	<c:if test="${orderDetail.prdtype == 2}">
		...
		<div class="orderdetails2">
			...
			<ul class="ydbz_dj specialPrice">
				<li><span class="ydtips">单价</span>
					<p>成人：${fns:getMoneyAmountBySerialNum(orderDetail.settlementAdultPrice,2)} 元</p>
					<p>儿童：${fns:getMoneyAmountBySerialNum(orderDetail.settlementcChildPrice,2)} 元</p>
					<p>特殊人群：${fns:getMoneyAmountBySerialNum(orderDetail.settlementSpecialPrice,2)} 元</p></li>
				<li><span class="ydtips"> 出行人数</span>
					<p>
	...
	<div class="orderdetails2">
		...
		<ul class="ydbz_info">
			...
			<li class="orderdetails2_text" title="苏州" id="mddtargetAreaNames"><span>目的地：</span>${orderDetail.targetarea}</li>
		</ul>
<!-- 兼容游轮 注释掉了这里 替换为下面的模块
	...
	<c:if test="${reviewdetail.travelerId != 0 }">
		...
		<table id="contentTable" class="activitylist_bodyer_table">
			...
			<tbody>
				<tr>
					<td class="tc" rowspan="1">${reviewdetail.travellerName}</td>
	...
	<c:if test="${flag == 0}">
		<div class="dbaniu">
			<form id="searchForm" action="${ctx}/refundReview/refundReview" method="post">
				...
				<input type = "hidden" id = "orderTypeSub" name="orderTypeSub" value = "${orderDetail.productType}"/>
				...
				<a class="ydbz_s"  id = "failBtn" onclick="jbox_bohui_refund3('${reviewdetail.id}',0,'','${reviewdetail.refundPrice}','${reviewdetail.productType}','${reviewdetail.orderId}','${reviewdetail.currencyId}');">驳回</a>
				<input id = "succBtn" type="button" value="审批通过" onclick="review('${reviewdetail.id}',1,'','${reviewdetail.refundPrice}','${reviewdetail.productType}','${reviewdetail.orderId}','${reviewdetail.currencyId}')" class="btn btn-primary">
			</form>
		</div>
	</c:if>
