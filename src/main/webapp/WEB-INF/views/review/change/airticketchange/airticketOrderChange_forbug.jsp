	...
	<div class="ydbzbox fs">
		<div class="orderdetails">
			<div class="orderdetails_tit">
				<span></span>订单信息 <input id="orderId" type="hidden"
					value="${orderDetailInfoMap.orderId }">
			</div>
					...
					<tbody>
						<tr>
							...
							<td class="mod_details2_d2"><fmt:formatDate
									value="${orderDetailInfoMap.orderCreateDate}"
						...
						<tr>
							...
							<td class="mod_details2_d2">${orderDetailInfoMap.orderGroupCode
								}</td>
							<td class="mod_details2_d1">团队类型：</td>
							<td class="mod_details2_d2"><c:choose>
						...
						<c:if test="${orderDetailInfoMap.type == 2 }">
							<tr>
								...
								<td class="mod_details2_d2">${orderDetailInfoMap.mainOrderId
									}</td>
								<td class="mod_details2_d1">参团团号：</td>
								<td class="mod_details2_d2">${orderDetailInfoMap.activityGroupCode
									}</td>
							</tr>
						</c:if>
					</tbody>
			...
			<div class="mod_information_dzhan" style="overflow:hidden;">
				<div class="mod_information_dzhan_d mod_details2_d">
					<c:choose>
						<c:when
							test="${not empty orderDetailInfoMap.flightInfoList && fn:length(orderDetailInfoMap.flightInfoList) == 2 }">
							<!--往返-->
							...
							<table width="90%" border="0">
								<tbody>
									<tr>
										...
										<td class="mod_details2_d2">${fns:getDictLabel(orderDetailInfoMap.departureCity,"from_area"
											, "")}</td>
										...
										<td class="mod_details2_d2">${orderDetailInfoMap.reservationsNum
											}人</td>
									</tr>
								</tbody>
							</table>
								...
								<table width="90%" border="0">
									<tbody>
										<tr>
											...
											<td class="mod_details2_d2">${flightInfo.startAirportName
												}</td>
										...
										<tr>
											...
											<td class="mod_details2_d2"><fmt:formatDate
													value="${flightInfo.startTime }" pattern="yyyy-MM-dd HH:mm" /></td>
											<td class="mod_details2_d1">到达时刻：</td>
											<td class="mod_details2_d2" colspan="3"><fmt:formatDate
													value="${flightInfo.arrivalTime }"
													pattern="yyyy-MM-dd HH:mm" /></td>
										</tr>
										<tr>
											<td class="mod_details2_d1">航空公司：</td>
											<td class="mod_details2_d2">${fns:getDictLabel(flightInfo.airlines,"traffic_name"
												, "")}</td>
											<td class="mod_details2_d1">舱位等级：</td>
											<td class="mod_details2_d2">${fns:getDictLabel(flightInfo.spaceGrade,"spaceGrade_Type"
												, "无")}</td>
											<td class="mod_details2_d1">舱位：</td>
											<td class="mod_details2_d2">${fns:getDictLabel(flightInfo.airspace,"airspace_Type"
												, "无")}</td>
										</tr>
									</tbody>
								</table>
						...
						<c:when
							test="${not empty orderDetailInfoMap.flightInfoList && fn:length(orderDetailInfoMap.flightInfoList) == 1 }">
							<!--单程-->
								...
								<table width="90%" border="0">
									<tbody>
										...
										<tr>
											<td class="mod_details2_d1">航空公司：</td>
											<td class="mod_details2_d2">
												${fns:getAirlineNameByAirlineCode(flightInfo.airlines)}</td>
											<td class="mod_details2_d1">舱位等级：</td>
											<td class="mod_details2_d2">${fns:getDictLabel(flightInfo.spaceGrade,"spaceGrade_Type"
												, "无")}</td>
											<td class="mod_details2_d1">舱位：</td>
						...
						<c:when
							test="${not empty orderDetailInfoMap.flightInfoList && fn:length(orderDetailInfoMap.flightInfoList) >= 3 }">
							<!--多段-->
					...
					<ul class="ydbz_dj specialPrice">
						<li style="display: none;"><input type="text"
							class="required" value="${orderDetailInfoMap.personNum}"
							onafterpaste="this.value=this.value.replace(/\D/g,'')"
							onkeyup="this.value=this.value.replace(/\D/g,'')"
							id="orderPersonelNum"></li>
						<li><span class="ydtips">单价</span> <input id="aPrice"
							type="hidden" value="${orderDetailInfoMap.adultPrice }">
							<input id="cPrice" type="hidden"
							value="${orderDetailInfoMap.childPrice }"> <input
							id="sPrice" type="hidden"
							value="${orderDetailInfoMap.specialPrice }">
							<p>
								成人：<font color="#FF0000">${orderDetailInfoMap.adultPrice
									}</font>
							</p>
							<p>
								儿童：<font color="#FF0000">${orderDetailInfoMap.childPrice
									}</font>
							</p>
							<p>
								特殊人群：<font color="#FF0000">${orderDetailInfoMap.specialPrice
									}</font>
							</p></li>
						<li><span class="ydtips"> 出行人数</span>
							<p>
								成人：<span>${orderDetailInfoMap.adultNum }</span> 人
							</p>
							<p>
								儿童：<span>${orderDetailInfoMap.childNum }</span> 人
							</p>
							<p>
								特殊人群：<span>${orderDetailInfoMap.specialNum }</span> 人
							</p></li>
						<li class="ydbz_single"><span class="">税费：</span>${orderDetailInfoMap.taxamt
							}人</li>
					</ul>
				</div>
			</div>
			...
			<div flag="messageDiv" class="ydbz2_lxr">
				<form class="contactTable">
					<p>
						<label style="vertical-align:top;width:300px;text-align:center;">航班备注：<c:if
								test="${ not empty orderDetailInfoMap.remark }">${orderDetailInfoMap.remark }</c:if>无
						</label>
					</p>
				</form>
			</div>
			...
			<table class="activitylist_bodyer_table">
				<thead>
					<tr>
						<th width="5%" class="table_borderLeftN">全选 <input
							id="selectall" type="checkbox" name="selectall"></th>
						<th width="15%">姓名</th>
						<th width="15%">是否联运</th>
						<th width="15%">联运类型</th>
						<th width="15%">联运价格</th>
				...
				<tbody>
					<c:forEach items="${orderDetailInfoMap.travelerStrategy}"
						var="travelerStrategy">
						<tr>
							<td class="table_borderLeftN"><input type="checkbox"
								id="traId${travelerStrategy.id}" class="introselect"
								name="${travelerStrategy.name}" value="${travelerStrategy.id}"></td>
							<td>${travelerStrategy.name}</td>
							<td><c:if test="${travelerStrategy.intermodalId eq null}">
						 否
						 </c:if> <c:if test="${travelerStrategy.intermodalId ne null}">
						 是
						 </c:if></td>
							<td><c:if test="${travelerStrategy.intermodalId eq null}">
						无联运
						 </c:if> <c:if test="${travelerStrategy.type==1}">
						 全国
						 </c:if> <c:if test="${travelerStrategy.type==2}">
						 分区
						 </c:if></td>
			...
			<div class="changeGroup">
				改签机票产品编号：<input type="text" id="product_code" /> <input
					type="button" value="查找" onclick="query(this)"
		...
		<input type='hidden' id="travelerIds" /> <input type='hidden'
			id="newProId" /> <input type='hidden' id="freePosition"
			name="freePosition" />
		<div class="ydBtn ydBtn2">
			<a class="ydbz_s" onClick="closeCurWindow();">关闭</a><a class="ydbz_s"
				id="submit">申请改签</a>
		</div>
	</div>
	<!--右侧内容部分结束-->
