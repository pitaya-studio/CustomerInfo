...
<!-- 								<td class="mod_details2_d2">${orderDetailInfoMap.activityGroupCode }</td> -->
			...
			<div class="orderdetails4">
				...
				<c:if test=" ${orderDetailInfoMap.nagentName ne ''}">
					<p class="ydbz_qdmc">非签约渠道名称： ${orderDetailInfoMap.nagentName }</p>
				</c:if>
				<%-- <ul class="ydbz_qd">
					<li>渠道联系人：${orderDetailInfoMap.agentContact }</li>
					<li>渠道联系人电话：<span class="fArial">${orderDetailInfoMap.agentTel }</span>
					</li>
					<li>固定电话：<span class="fArial">${orderDetailInfoMap.agentTel }</span>
					</li>
				</ul> --%>
				<ul class="ydbz_qd" >
					<c:forEach items="${orderDetailInfoMap.contactsLists }" var = "listc">
					   <li><label>渠道联系人：</label>${listc.agentContact }</li>
	                   <li class="ydbz_qd_lilong"><label>渠道联系人电话：</label>${listc.agentTel }<div class="zksx boxCloseOnAdd" onclick="boxCloseOnAdd(this,'','2')">展开全部</div></li>
	                   <li flag="messageDiv" style="display:none" class="ydbz_qd_close">
	                       <ul>
		                        <li><label>渠道地址：</label>${listc.agentAddress }</li>
		                        <li><label>传真：</label>${listc.agentFax }</li>
		                        <li><label>QQ：</label>${listc.agentQQ }</li>
		                        <li><label>Email：</label>${listc.agentEmail }</li>
		                        <li><label>渠道邮编：</label>100000</li>
		                        <li><label>其他：</label>${listc.agentRemarks }</li>
	                       </ul>
                   	   </li>
					</c:forEach>
			...
			<c:if test="${orderDetailInfoMap.type == 1 }">
				...
				<c:forEach items="${orderDetailInfoMap.orderPayInfoList }" var="payInfo">
					<p class="orderdetails6">
						<span style="width: 16.6%">付款方式：${payInfo.payTypeName }</span> 
						<span style="width: 16.6%">支付款类型：${fns:getDictLabel(payInfo.payPriceType,"payprice_Type" , "")}</span> 
						<span style="width: 16.6%">付款金额：${payInfo.payPrice }</span> 
						<span style="width: 16.6%">付款时间： <fmt:formatDate value="${payInfo.createDate }" pattern="yyyy-MM-dd HH:mm:ss"/></span> 
						<span style="width: 16.6%">付款凭证：
	    ...
	    <%@ include file="/WEB-INF/views/review/borrowing/common/newairticketOrderReviewInfo.jsp"%>
	</div>
	<!--右侧内容部分结束-->
