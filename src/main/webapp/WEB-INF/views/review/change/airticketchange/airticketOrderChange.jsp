...
// 				document.location = "${ctx}/airticketChange/airChange/airticketApprovalHistoryList?orderId=" + $("#orderId").val();
			}
			$.ajax({    
 				type: "POST",                 
				url:"${ctx}/airticketChange/airChange/areaGaiQian",                 
				data:{travelerIds:ids,newProId:$("#newProId").val(),orderId:$("#orderId").val()},                
				dataType:"json",           
				success: function(data) {
				    if(data.canCancel == true && data.isMutexBoolean == true){
// 				    	$.jBox.tip(data.message, "warning");
				    	top.$.jBox.confirm(data.message,'系统提示',function(v){
								if(v=='ok'){
									$.ajax({
							            type: "POST",
							            url:  "${ctx}/airticketChange/airChange/cancelOtherReview",
							            data: {
							            	ids:data.ids,
							            	oids:data.oids
							            },
				    ...
				    }else if(data.canCancel == false && data.isMutexBoolean == true){
				    	$.jBox.tip(data.message, "warning");
				    }else{
...
// 					async: false,//配置同步请求
// 	 				type: "POST",                 
// 					url:"${ctx}/airticketChange/airChange/airticketChangeCheck",                 
// 					data:{travelerIds:ids,newProId:$("#newProId").val(),orderId:$("#orderId").val()},                
// 					dataType:"json",           
// 					success: function(data) {
// 						if(data.result=='0'){
// 							submitAirticketChange();
// 						}else{
// 							alert(data.message);
// 							$.jBox.tip(data.message, "warning");
// 							document.location = "${ctx}/airticketChange/airChange/airticketApprovalHistoryList?orderId=" + $("#orderId").val();
...
// 			});
		...
		//提交流程
		function submitAirticketChange(){
			$.ajax({    
 				type: "POST",                 
				url:"${ctx}/airticketChange/airChange/areaGaiQian",                 
				data:{travelerIds:ids,newProId:$("#newProId").val(),orderId:$("#orderId").val()},                
...
<!-- 								<td class="mod_details2_d2">${orderDetailInfoMap.activityGroupCode }</td> -->
           ...
           <input type='hidden' id="freePosition" name="freePosition"/>
               ...
               <a class="ydbz_s" onClick="closeCurWindow();">关闭</a>
               <input type="button"   value="申请改签" id="submit" class="btn btn-primary">
            </div>
	</div>
	<!--右侧内容部分结束-->
