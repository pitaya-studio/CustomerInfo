...
<html>
<head>
<title>改价-<c:if test="${flag == 0}">审批</c:if><c:if test="${flag != 0}">详情</c:if></title>
<!-- 页面左边和上边的装饰 -->
...
<script type="text/javascript">
	...
	//驳回
	function jbox_bohui_refund3(revid,orderid,producttype,travelerid,changedprice,currencyid){
		...
		$.jBox(html, { title: "驳回原因",buttons:{"提 交":"1"}, submit:function(v, h, f){
			if (v=="1"){
				$.ajax({
					type : "post",
					url : "${ctx}/changepricenew/changepricenewreview",
					data : {
						"revId" : revid,
						"orderId" : orderid,
						"orderTypeSub" : producttype,
						"travelerId" : travelerid,
						"moneyAmount" : changedprice,
					...
					success : function(data){
						if(data.flag == 'success'){
							...
							$.jBox.tip(data.msg);
							$("#revSucBtn").attr('disabled',false);
							$("#revFailBtn").attr('disabled',false);
						}
					}
				})
			} else {
	...
	function review(revid,orderid,producttype,travelerid,changedprice,currencyid){
		<c:forEach items="${changePrice.moneyList}" var="money" varStatus="cc">
			currencyid += ${money.key} + ",";
			changedprice += ${money.initValue} + ",";
		</c:forEach>
		$("#revSucBtn").attr('disabled',true);
		$("#revFailBtn").attr('disabled',true);
	...
	$(function() {
		...
		<%--if(${productType} != 6 && ${productType} != 7) {--%>
			(function countToatl(){
				var returnObj={};
				$("tbody").find("tr:last").each(function(){
					var innerHtml1='';
					var oldPrice=$(this).find("td").eq("3");
					...
					$(this).find("td").eq("6").empty().html(innerHtml1);
					$("#sumMoney").html(innerHtml1);
//				$("#travlelercha").html(innerHtml1);
				})
			})();
//		}
...
<body>
	<!--右侧内容部分开始-->
	<div class="mod_nav">审批 > 改价审批 > 改价审批审批</div>
		...
		<div class="orderdetails2">
			...
			<c:if test="${orderDetail.type == 1 }">
				...
				<c:forEach items="${orderDetail.flightInfoList }" var="flightInfo">
					<div class="ydbz_tit ydbz_tit_child">第${flightInfo.number}段：
					<c:choose>
			...
			<ul class="ydbz_dj specialPrice">
				...
				<li><span class="ydtips">单价</span>
					<p>
						成人：<font color="#FF0000">${fns:getCurrencyInfo(orderDetail.currid, 0, 'mark')}${orderDetail.settlementAdultPrice}</font>
					</p>
					<p>
						儿童：<font color="#FF0000">${fns:getCurrencyInfo(orderDetail.currid, 0, 'mark')}${orderDetail.settlementcChildPrice}</font>
					</p>
					<p>
						特殊人群：<font color="#FF0000">${fns:getCurrencyInfo(orderDetail.currid, 0, 'mark')}${orderDetail.settlementSpecialPrice}</font>
					</p>
				</li>
				...
				<li class="ydbz_single"><span class="">税费：</span>${fns:getCurrencyInfo(orderDetail.currid, 0, 'mark')}${orderDetail.taxamt}/人
				</li>
			</ul>
		</div>
	</c:if>
	...
	<c:if test="${orderDetail.prdtype == 2}">
		...
		<div class="orderdetails2">
			<p class="ydbz_mc">${orderDetail.prdname}</p>
			<ul class="ydbz_info">
				<li title="${orderDetail.groupno}"><span>团号：</span>${orderDetail.groupno}</li>
				<li><span>出团日期：</span>${orderDetail.opendate}</li>
				<li title="苏州,南京" id="mddtargetAreaNames"><span>目的地：</span>${orderDetail.targetarea}</li>
...
<!-- 兼容游轮 注释掉了这里 替换为下面的模块
	...
	<table id="contentTable" class="activitylist_bodyer_table">
		...
		<tbody>
			<tr>
				<td >${reviewdetail.travellerName}</td>
				<%--<td>${reviewdetail.currencyname}</td>--%>
				<td>
					<c:choose>
						<c:when test="${not empty changePrice.moneyList}">
							<c:forEach items="${changePrice.moneyList}" var="money" varStatus="cc">
								<c:choose>
									<c:when test="${!cc.last}">
										<span>${fns:getCurrencyInfo(money.key,0,'name')}</span>、
									</c:when>
									<c:otherwise>
										<span>${fns:getCurrencyInfo(money.key,0,'name')}</span>
									</c:otherwise>
						...
						<c:otherwise>
							${reviewdetail.currencyname}
						</c:otherwise>
					</c:choose>
				</td>
				<td class="tr">${reviewdetail.originalTravelerMoney}</td>
				<td class="tr">
					<c:choose>
						<c:when test="${productType ne 6 and productType ne 7}">
							<%--<c:forEach items="${changePrice.originalMoneyList}" var="originalMoney" varStatus="c">--%>
								...
								<%--</c:choose>--%>
							<%--</c:forEach>--%>
							<c:forEach items="${fns:getMoneyBeforeCP(changePrice.travellerId,orderDetail.orderid,reviewdetail.createDate)}" var="money" varStatus="count">
								<c:choose>
						...
						<c:otherwise>
                        <span>${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}</span> <span><fmt:formatNumber  type="currency" pattern="#,##0.00" value="${reviewdetail.curtotalmoney}"/></span>
						</c:otherwise>
					</c:choose>
				<%--${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${reviewdetail.curtotalmoney}"/>--%>
				...
				<td class="tr">
					<c:choose>
						...
						<c:otherwise>
                            <span>${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}</span> <span><fmt:formatNumber type="currency" pattern="#,##0.00" value="${reviewdetail.changedprice}" /></span>
						</c:otherwise>
					</c:choose>
				</td>
				<td>${reviewdetail.remark}</td>
				<td class="tr" id="sumMoney">
				</td>
				</td>
			</tr>
		</tbody>
...
<%--<!-- 			<font class="gray14" flag="bz">美元</font> -->--%>
			<%--<span class="f20 tdred" id="travlelercha">--%>
				<%--<c:choose>--%>
					<%--<c:when test="${not empty changePrice.moneyList}">--%>
						<%--<c:forEach items="${changePrice.moneyList}" var="money" varStatus="cc">--%>
							<%--<c:choose>--%>
					...
					<%--</c:when>--%>
					<%--<c:otherwise>--%>
						<%--${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber type="currency" pattern="#,##0.00" value="${reviewdetail.changedtotalmoney}" />--%>
					<%--</c:otherwise>--%>
				<%--</c:choose>--%>
			<%--&lt;%&ndash;${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${reviewdetail.changedprice}"/>&ndash;%&gt;--%>
			<%--</span>--%>
			<%--</span>--%>
		<%--</div>--%>
	<%--</div>--%>
	...
	<table class="activitylist_bodyer_table modifyPrice-table">
		...
		<tbody>
			<tr>
				...
				<td>${reviewdetail.currencyname}</td>
				<td class="tr">${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}${reviewdetail.oldtotalmoney}</td>
				<td class="tr">${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}${reviewdetail.curtotalmoney}</td>
				<td class="tr">${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${reviewdetail.changedprice}"/></td>
				<td>${reviewdetail.remark}</td>
				<td class="tr">${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}${reviewdetail.changedtotalmoney}</td>
			</tr>
		</tbody>
	</table>
	</c:if>
	...
	<%--<div class="allzj tr f18">--%>
		<%--改前金额：<span class="f20">--%>
		<%--&lt;%&ndash;${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${reviewdetail.curtotalmoney}"/>&ndash;%&gt;--%>
		<%--<c:choose>--%>
			<%--<c:when test="${not empty changePrice.moneyList}">--%>
				<%--<c:forEach items="${changePrice.originalMoneyList}" var="originalMoney" varStatus="c">--%>
					<%--<c:choose>--%>
		...
		<%--<div class="all-money">--%>
			<%--改后总额：<span class="f20" id="sumMoney">--%>
			<%--&lt;%&ndash;${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber type="currency" pattern="#0.00" value="${reviewdetail.changedtotalmoney}" />&ndash;%&gt;--%>
			<%--&lt;%&ndash;${fns:getCurrencyInfo(reviewdetail.currencyid, 0, 'mark')}<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${reviewdetail.changedprice}"/>&ndash;%&gt;--%>
		<%--</span>--%>
		<%--</div>--%>
	<%--</div>--%>
	<c:if test="${flag == 0}">
		<div class="dbaniu">
		<!-- 添加提交请求所需数据 -->
		<form id="searchForm" action="${ctx}/changePrice/changePriceReview" method="post">
...
<!-- 				<input type = "hidden" id = "nowlevel" name="nowlevel" value = "${nowlevel}"/> -->
				...
				<input type = "hidden" id = "orderTypeSub" name="orderTypeSub" value = "${productType}"/>
				<input type = "hidden" id = "travelerId" name="travelerId" value = "${reviewdetail.travellerId}"/>
			...
			<a class="ydbz_s gray" href = "${ctx}/changepricenew/changpricereviewnewlist">返回</a> 
			<a class="ydbz_s" id="revFailBtn" onclick="jbox_bohui_refund3('${reviewdetail.id}','${orderDetail.orderid}','${productType}','${reviewdetail.travellerId}','${reviewdetail.changedprice}','${reviewdetail.currencyid}');">驳回</a> 
			<input type="button" id="revSucBtn" value="审批通过" onclick="review('${reviewdetail.id}','${orderDetail.orderid}','${productType}','${reviewdetail.travellerId}','${reviewdetail.changedprice}','${reviewdetail.currencyid}')" class="btn btn-primary">
		</form>
		</div>
	</c:if>
