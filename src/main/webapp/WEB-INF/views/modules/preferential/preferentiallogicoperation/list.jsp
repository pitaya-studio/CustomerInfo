...
$(function(){
	...
	if("${preferentialLogicOperation.runClass}"||"${preferentialLogicOperation.runMethod}"||"${preferentialLogicOperation.uuid}"){
		$(".zksx").click();
	}
});
...
function show(uuid){
	window.open("${ctx}/preferentialLogicOperation/show/" + uuid );
}
function edit(uuid){
	window.open( "${ctx}/preferentialLogicOperation/edit/" + uuid );
}
function del(uuid){
	var ids = [];
	ids.push(uuid);
...
function v_deleteItems(ids){
	...
	$.jBox.confirm("确定要删除数据吗？", "系统提示", function(v, h, f) {
		if (v == 'ok') {
			...
			$.post( "${ctx}/preferentialLogicOperation/delete", {"uuids":ids.join(",")}, 
				function(data){
					if(data.result=="1"){
...
function page(n,s){
	$("#pageNo").val(n);
	$("#pageSize").val(s);
	$("#searchForm").attr("action"," ${ctx}/preferentialLogicOperation/list");
	$("#searchForm").submit();
}
</script>	
</head>
...
<!--右侧内容部分开始-->
   ...
   <form id="searchForm" action=" ${ctx}/preferentialLogicOperation/list" method="post">
        ...
        <div class="activitylist_bodyer_right_team_co">
           ...
           <div class="activitylist_bodyer_right_team_co2 pr wpr20">
               <label>名称：</label><input style="width:260px" class="txtPro inputTxt inquiry_left_text" id="name" name="name" value="${preferentialLogicOperation.name}" type="text" flag="istips" />
                        <span class="ipt-tips">仅支持逻辑规则名称的搜索</span>
                    </div>
					...
					<div class="ydxbd">
						<div class="activitylist_bodyer_right_team_co1">
							...
							<input value="${preferentialLogicOperation.uuid}" id="uuid"
								name="uuid" />
						</div>
						<div class="activitylist_bodyer_right_team_co1">
							<div class="activitylist_team_co3_text">执行类：</div>
							<input value="${preferentialLogicOperation.runClass}"
								id="runClass" name="runClass" />
						</div>
						<div class="activitylist_bodyer_right_team_co1">
							<div class="activitylist_team_co3_text">执行方法：</div>
							<input value="${preferentialLogicOperation.runMethod}"
								id="runMethod" name="runMethod" />
						</div>
	...
	<a class="btn btn-primary" href=" ${ctx}/preferentialLogicOperation/form" target="_blank">添加</a>
</div>
		            ...
		            <c:forEach var="entry" items="${page.list}" varStatus="v">
			          <tr>
			            ...
			            <td align="center"><input name="ids" type="checkbox" value="${entry.uuid}" /></td>
							<td>${entry.uuid}</td>
							<td>${entry.name}</td>
							<td>${entry.runClass}</td>
							<td>${entry.runMethod}</td>
			            <td class="p0">
	                    	<dl class="handle">
