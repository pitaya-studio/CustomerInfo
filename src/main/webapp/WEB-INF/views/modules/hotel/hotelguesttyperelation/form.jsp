			...
			$("#inputForm").validate({
				rules:{
					label:{
						...
						remote: {
							type: "POST",
							url: "${ctx}/hotelGuestTypeRelation/check?type="+$('#type').val()+"&uuid="+$('#uuid').val()
								}
						},
					sort:{
						required:true,
				...
				submitHandler: function(form){
					var url = "";
					if($("#uuid").val()=='') {
						url="${ctx}/hotelGuestTypeRelation/save";
					} else {
						url="${ctx}/hotelGuestTypeRelation/update";
					}
...
<body>
	...
	<form:form method="post" modelAttribute="hotelGuestTypeRelationInput" action="" class="form-horizontal" id="inputForm" novalidate="">
		...
		<div class="maintain_add">
			...
			<p>
				<label><em class="xing"></em>酒店住客类型uuid：</label>
				<form:input path="hotelGuestTypeUuid" htmlEscape="false" maxlength="11" class="required" />
			</p>
			<p>
				<label><em class="xing"></em>住客类型名称：</label>
				<form:input path="hotelGuestTypeName" htmlEscape="false" maxlength="11" class="required" />
			</p>
			<p>
			...
			<p>
				<label><em class="xing"></em>酒店房型uuid：</label>
				<form:input path="hotelRoomUuid" htmlEscape="false" maxlength="11" class="required" />
			</p>
			<p>
			...
			<p>
				...
				<input readonly onclick="WdatePicker()" value="<fmt:formatDate value="${hotelGuestTypeRelation.createDate}" pattern="yyyy-MM-dd" />" name="createDate" class="inputTxt dateinput" id="createDate" />
			</p>
			<p>
			...
			<p>
				...
				<input readonly onclick="WdatePicker()" value="<fmt:formatDate value="${hotelGuestTypeRelation.updateDate}" pattern="yyyy-MM-dd" />" name="updateDate" class="inputTxt dateinput" id="updateDate" />
			</p>
			<p>
