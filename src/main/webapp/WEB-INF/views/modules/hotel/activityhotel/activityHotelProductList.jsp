...
// 								top.$.jBox.tip(data.message, 'warning');
	...
	function showSearchPanel() {
		...
		var status = "${activityHotelQuery.status }";
		if (isNotEmpty(activityName) || isNotEmpty(hotel)
				...
				|| isNotEmpty(hotelstar) || isNotEmpty(activitySerNum) || isNotEmpty(status)) {
			$('.zksx').click();
		}
	}
</script>
...
//        	   							roomNameNight+=""+"*"+n.night+"晚</t>";
        ...
        function checkall(obj){
			...
			if($("#"+obj).attr("checked")){
				...
				$("#children"+parent).removeAttr("checked");
				$("input[name='children"+parent+"']").each(function(){
...
<body>
	...
	<div class="mod_nav">产品 &gt; 酒店产品 &gt; 酒店产品列表</div>
	<div class="activitylist_bodyer_right_team_co_bgcolor">
		<form:form method="post"
			action="${ctx}/activityHotel/activityHotelList?showType=2"
			...
			<input  id="activityUuid"  type="hidden" />
			<div class="activitylist_bodyer_right_team_co">
				...
				<!-- @author chao.zhang 2016/1/7 根据E419V2要求将其隐藏 -->
			...
			<div class="filter_sort">
				...
				<c:choose>
					<c:when test="${orderBy=='' || orderBy==null}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort "></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
							class="i_sort"></i>
						</a>
						<a onclick="cantuansortby('ahgl.price',this)">同行价格<i
							class="i_sort"></i>
						</a>
					</c:when>
					<c:when test="${orderBy=='ah.createDate DESC'}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort  i_sort_down"></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
							class="i_sort"></i>
						</a>
					...
					<c:when test="${orderBy=='ah.createDate ASC'}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort  i_sort_up"></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
							class="i_sort"></i>
						</a>
					...
					<c:when test="${orderBy=='ah.updateDate DESC'}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort"></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
					...
					<c:when test="${orderBy=='ah.updateDate ASC'}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort"></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
					...
					<c:when test="${orderBy=='ahgl.price DESC'}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort"></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
					...
					<c:when test="${orderBy=='ahgl.price ASC'}">
						<a onclick="cantuansortby('ah.createDate',this)">创建时间<i
							class="i_sort"></i>
						</a>
						<a onclick="cantuansortby('ah.updateDate',this)">更新时间<i
		...
		<table class="activitylist_bodyer_table" id="contentTable">
			<thead>
					<tr>
						<th width="5%">序号</th>
						<!-- 根据要求隐藏控房单号 @author 张潮 2016/1/7 -->
						<!-- <th width="10%" style="display:none;">控房单号</th> -->
						<th width="20%">产品名称</th>
						<th width="10%">国家</th>
						<th width="15%">岛屿名称</th>
						<th width="15%">酒店&amp;星级</th>
						<th width="10%" class="tr" >同行价格/人</th>
						<th width="5%">操作</th>
					</tr>
			</thead>
			<tbody>
				...
				<c:forEach items="${page.list }" var="record" varStatus="s">
					<tr id="${record.uuid}---------" data-value="${record.uuid}">
					...
					<%-- 	<td class="tc" style="display:none;">${record.activitySerNum}</td> --%>
						...
						<td class="tc"><trekiz:autoId2Name4Table
								...
								srcColumnName="name_cn" value="${record.country}" />
						</td>
						<td class="tc" data-value="${record.island_uuid}" data-country="${record.country }"><trekiz:autoId2Name4Table tableName="island"
								sourceColumnName="uuid" srcColumnName="island_name"
								value="${record.island_uuid}" />
						</td>
						<td class="tc" data-value="${record.hotel_uuid}"><trekiz:autoId2Name4Table tableName="hotel"
						...
						<td class="tr" data-value="${record.lowPriceList[0].currencyId}"><c:forEach items="${record.lowPriceList}"
								var="variable">
								<p data-value="${variable.currencyId}">
									<input id="tcurrencyId" type="hidden" value="${variable.currencyId}"/>
								...
								<dd>
									<p>
										<span></span> <a onclick="expand('#child${record.uuid}',this)"
											...
											href="${ctx}/activityHotel/editActivityHotel/${record.uuid}"
											target="_blank">修改</a> <a href="javascript:void(0)"
											onclick="updateGroupStatusByActivityUuid('要上架该产品吗？', '${record.uuid}', 1,this)">上架</a><a
											href="javascript:void(0)"
											onclick="updateGroupStatusByActivityUuid('要下架该产品吗？', '${record.uuid}', 2,this)">下架</a><a
											href="javascript:void(0)"
											onclick="updateGroupStatusByActivityUuid('要恢复该产品吗？', '${record.uuid}', 3,this)">恢复</a>
										<a href="javascript:void(0)"
											onclick="updateGroupStatusByActivityUuid('要删除该产品吗？', '${record.uuid}', 4,this)">删除</a>
										<a
											href="${ctx}/activityHotel/showActivityHotelDetail/${record.uuid}?type=product"
											target="_blank">详情</a> <a class="addLink"
											href="javascript:void(0)">新增团期</a>
									</p>
								</dd>
							</dl></td>
					</tr>
					...
					<tr id="child${record.uuid}" style="display:none" >
						<td colspan="8" class="product_twolist_table_bgtd">
							<table style="width:100%" class="hotel_table_openbg subTable product_twolist_table2" id="productTable">
								<thead>
									<tr>
										<th class="tc" width="3%">
										...
										<th class="tc" width="10%"><span class="tr">余位/预报名</span>
										</th>
								...
								<tbody>
									<c:forEach items="${groupList[s.index]}" var="entry" varStatus="t">
										...
										<tr id="${entry.uuid }" data-tag="${entry.uuid }" style="display: table-row;">
											...
											<input type="checkbox"  onclick="" name="children${record.uuid}" value="${entry.uuid}" />${t.index+1}</td>
											...
											<td rowspan="${entry.baseMealNum }" class="tc"><c:forEach
													...
													<p>
														<span><%-- <trekiz:defineDict name="traveler_type"
																type="traveler_type" defaultValue="${price.type }"
																readonly="true" /> --%>
																<trekiz:autoId2Name4Table tableName="traveler_type"
										sourceColumnName="uuid" srcColumnName="name"
										value="${price.type}" /> : <trekiz:autoId2Name4Table
																tableName="currency" sourceColumnName="currency_id"
																srcColumnName="currency_mark"
																value="${price.currencyId }" />
																<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${price.price }" /> </span>
													</p>
												</c:forEach>
											</td>
											<td rowspan="${entry.baseMealNum }" class="tc">
											<span style="color: red;">${entry.remNumber}</span>/<span>${entry.orderNum}</span>
											</td>
											<td rowspan="${entry.baseMealNum }" class="tr"><trekiz:autoId2Name4Table
													tableName="currency" sourceColumnName="currency_id"
													srcColumnName="currency_mark" value="${entry.currency_id}" />
												<span>${entry.singlePrice}/间</span>
											</td>
                    ...
                    <td style="display:none;">
                        <p class="upMealType">
                            <span>
                                ...
                                <select name="peoplePrice" class="w50_30 mr3 currency">
...
<!--                     	<input  name="detailNum" value=""/> -->
 ...
 <script type="text/javascript">
        $(document).ready(function () {
        	 ...
        	 $("#contentTable").on('click', 'a.addLink', function () {
        		   // 复制并新增
        		 var id = $("#contentTable tbody tr").has(this).attr("data-value");
                 var islandway=$("#contentTable > tbody > tr").has(this).find("td:eq(4)").attr("data-value");
                 var hotel=$("#contentTable > tbody > tr").has(this).find("td:eq(5)").attr("data-value");
                 var countryVal=$("#contentTable > tbody > tr").has(this).find("td:eq(4)").attr("data-country");
                 var currId = $("#contentTable > tbody > tr").has(this).find("td:eq(6)").attr("data-value");
                 $("#activityUuid").val(id);
                 // 新增团期
                 var $table = $("#contentTable tr").has(this).next().find("table.subTable");
                 var options = {
                	 id:id,
                     isAdd: true,
                     table: $table,
                 ...
                 //赋值初始化币种
                 $("[name=peoplePrice]").val($("#tcurrencyId").val());

             }).on('click', 'a.copyLink', function () {
                // 复制并新增
                var id=$("#contentTable .subTable tr").has(this).attr("id");
                var islandway=$("#contentTable > tbody > tr").has(this).prev().find("td:eq(4)").attr("data-value");
                var hotel=$("#contentTable > tbody > tr").has(this).prev().find("td:eq(5)").attr("data-value");
                var countryVal=$("#contentTable > tbody > tr").has(this).find("td:eq(4)").attr("data-country");
              	$.ajax({
            		type:"post",
