	...
	<div class="produceDiv">
		<form:form method="post" modelAttribute="hotel" action="${ctx}/hotel/save" class="form-search" id="inputForm" novalidate="">
			...
			<div class="seach25">
                ...
                <form:select path="position" class="domesticOverseas" onchange="domesticOverseas(this);">
                   ...
                   <c:forEach items="${positionMap}" var="item">
                   	<option value="${item.key}" <c:if test="${item.key==hotel.position}">selected="selected"</c:if>>${item.value}</option>
                   </c:forEach>
                </form:select>
            </div>
			...
			<div class="seach25">
				...
				<trekiz:defineDict name="type" input="select" type="hotel_type" defaultValue="${hotel.type}" onchange="addHotelType(this)" element="<option value=''>+添加</option>" />
			</div>
                        ...
                        <span class="sysselect_s">
                            <form:select path="country">
                            	<c:forEach items="${countrys}" var="country">
                            		<c:choose>
			                        	<c:when test="${country.uuid==hotel.country}">
	                        ...
	                        <form:select path="province" onclick="getGeoSelectState('',$('#country'),'province')">
	                        	<c:forEach items="${provinces}" var="province">
                            		<c:choose>
			                        	<c:when test="${province.uuid==hotel.province}">
	                        ...
	                        <form:select path="city" onclick="getGeoSelectState('',$('#province'),'city')">
	                        	<c:forEach items="${citys}" var="city">
                            		<c:choose>
			                        	<c:when test="${city.uuid==hotel.province}">
	                        ...
	                        <form:select path="district"  onclick="getGeoSelectState('',$('#city'),'district')">
	                        	<c:forEach items="${districts}" var="district">
                            		<c:choose>
			                        	<c:when test="${district.uuid==hotel.province}">
                    ...
                    <p class="overseas">
                        ...
                        <span class="sysselect_s">
                        	...
                        	<select id="overseas_city" name="overseas_city" onclick="getGeoSelectState('',$('#province'),'city')">
                        		<c:forEach items="${citys}" var="city">
                            		<c:choose>
			                        	<c:when test="${city.uuid==hotel.city}">
                            ...
                            <input type='text' name="shortAddress" id="shortAddress" value="${hotel.shortAddress}"/>
                        </span>
                    </p>
