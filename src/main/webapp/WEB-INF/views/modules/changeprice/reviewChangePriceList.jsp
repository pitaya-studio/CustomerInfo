...
<body>
	<page:applyDecorator name="changeprice_review_head">
   			...
   			<div class="message_box_li">
	   			<c:if test="${conditionsMap.userJobId == userJob.id}">
	   				<div class="message_box_li_a curret ydbz_x">
						<a href="${ctx}/changePrice/changePriceReviewList?headPrd=${conditionsMap.headPrd}&userJobId=${userJob.id}"><span><font style="color:white">${fns:abbrs(userJob.deptName,userJob.jobName,40)}</font></span>
				...
				<c:if test="${conditionsMap.userJobId != userJob.id}">
					<div class="message_box_li_a ydbz_x gray">
						<a style="none" href="${ctx}/changePrice/changePriceReviewList?headPrd=${conditionsMap.headPrd}&userJobId=${userJob.id}"><span><font style="color:white">${fns:abbrs(userJob.deptName,userJob.jobName,40)}</font></span>
						<c:if test ="${userJob.count gt 0}">
							<div class="message_box_li_em" >${userJob.count}</div>
						</c:if>
						<c:if test="${fns:getStringLength(userJob.deptName,userJob.jobName) gt 37}">
...
<!--    		<c:forEach items="${userJobs}" var="userJob"> -->
<!--    			<c:if test="${conditionsMap.userJobId == userJob.id}"> -->
<!-- 				<a class="ydbz_x" href="${ctx}/changePrice/changePriceReviewList?userJobId=${userJob.id}">${userJob.deptName}_${userJob.jobName}</a> -->
<!-- 			</c:if>  -->
<!-- 			<c:if test="${conditionsMap.userJobId != userJob.id}"> -->
<!-- 				<a class="ydbz_x gray" href="${ctx}/changePrice/changePriceReviewList?userJobId=${userJob.id}">${userJob.deptName}_${userJob.jobName}</a> -->
<!-- 			</c:if>  -->
<!-- 		</c:forEach> -->
<!--     </div> -->
	<form id="searchForm" action="${ctx}/changePrice/changePriceReviewList?headPrd=${conditionsMap.headPrd}"
		method="post">
                             ...
                             <dl class="handle">
                                 ...
                                 <p>
                                     ...
                                     <c:if test="${changePriceReviewInfo.revLevel == changePriceReviewInfo.curlevel && changePriceReviewInfo.revstatus == '1'}">
										<a href="${ctx}/changePrice/changePriceReviewDetail?orderid=${changePriceReviewInfo.orderid}&revid=${changePriceReviewInfo.revid}&prdType=${changePriceReviewInfo.prdtype}&flag=0&nowlevel=${changePriceReviewInfo.curlevel}">审批</a><br />
									</c:if>
									<c:if test="${changePriceReviewInfo.prdtype == 7 || changePriceReviewInfo.prdtype == 6}">
									...
									<a href="${ctx}/changePrice/changePriceReviewDetail?orderid=${changePriceReviewInfo.orderid}&revid=${changePriceReviewInfo.revid}&prdType=${changePriceReviewInfo.prdtype}&flag=1&nowlevel=${changePriceReviewInfo.curlevel}">查看</a><br />
