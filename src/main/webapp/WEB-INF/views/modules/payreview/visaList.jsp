...
<body>
			...
			<div class="message_box_li">
				<c:choose>
					<c:when test="${userJobId==role.id}">
						<a href="${ctx}/pay/review/visaList/${reviewLevel}?userJobId=${role.id}">
	...
	<div class="activitylist_bodyer_right_team_co_bgcolor">
		<form:form id="searchForm" modelAttribute="VisaProducts" action="${ctx}/pay/review/visaList/${reviewLevel}" method="post">
			...
			<div class="activitylist_bodyer_right_team_co">
				<div class="activitylist_bodyer_right_team_co2">
					<%--<label>签证国家：</label>--%>
					<!-- 添加签证国家/地区查询条件 -->
					<input class="txtPro inputTxt searchInput" id="sysCountryName" name="sysCountryName" value="${sysCountryName}" type="text" placeholder="请输入签证国家" />
				</div>
				<a class="zksx">筛选</a>
				...
				<div class="ydxbd">
					...
					<div class="activitylist_bodyer_right_team_co1" >
						<label class="activitylist_team_co4_text" >审核发起人：</label>
		...
		<table id="contentTable" class="table activitylist_bodyer_table mainTable">
			...
			<c:forEach items="${page.list}" var="visaProduct" varStatus="s">
				...
				<c:if test="${not empty review }"> 
					...
					<td class="tc"><fmt:formatDate pattern="yyyy-MM-dd" value="${visaProduct.createDate}"/></td>
					...
					<td id="result${visaProduct.id}">
						${fns:getNextPayReview(visaProduct.id)}
					</td>
					<td class="tc" id="change${visaProduct.id}">
						<c:if test="${visaProduct.payNowLevel == reviewLevel && visaProduct.payReview==1 }">
							<a href="${ctx}/pay/review/visa/${visaProduct.visaId}/${reviewLevel}/${reviewCompanyId}" target="_blank">审核</a>
						</c:if> 
					</td>
					<td class="p0">
						<a href="${ctx}/pay/review/visaRead/${visaProduct.visaId}/${reviewLevel}" target="_blank">查看</a>&nbsp;
						...
						<c:if test="${visaProduct.payUpdateBy eq fns:getUser().id and visaProduct.payNowLevel ne 1 and visaProduct.payNowLevel-1 ne reviewCompany.topLevel and visaProduct.payReview ne 0 }">
							<a href="javascript:void(0)" onClick="revokePayAudit('${visaProduct.id}','${visaProduct.payNowLevel}','6')">撤销</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
