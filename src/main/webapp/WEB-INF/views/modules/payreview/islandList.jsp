...
<head>
   ...
   var _$review = $("#review").val();   
     ...
     $( ".spinner" ).spinner({
			...
			//展开筛选按钮--UG_V2替换成统一方法 by tlw 20170314
    		launch();
...
function expand(child,obj,srcActivityId){
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/pay/review/island/${reviewLevel}");
			$("#searchForm").submit();
	    }
            ...
            <div class="message_box_li">
              <c:choose>
                <c:when test="${userJobId==role.id}">
                      <a  href="${ctx}/pay/review/island/${reviewLevel}?userJobId=${role.id}">
...
<!--   <div class="activitylist_bodyer_right_team_co_bgcolor">-->
	 <form:form id="searchForm" modelAttribute="activityIslandGroup" action="${ctx}/pay/review/island/${reviewLevel}" method="post" >
     ...
     <div class="activitylist_bodyer_right_team_co">    	 
		...
		<div class="ydxbd">
            ...
            <div class="activitylist_bodyer_right_team_co1">
				...
				<input id="groupOpenDate" class="inputTxt dateinput" name="groupOpenDate" value='<fmt:formatDate value="${activityIslandGroup.groupOpenDate }" pattern="yyyy-MM-dd"/>' onfocus="WdatePicker({onpicking:function(dp){var vvv=dp.cal.getNewDateStr();if($dp.$('groupCloseDate').value==''){$dp.$('groupCloseDate').value=vvv;}}})" readonly/>
				<span> 至</span>
				<input id="groupEndDate" class="inputTxt dateinput" name="groupEndDate" value='<fmt:formatDate value="${activityIslandGroup.groupEndDate }" pattern="yyyy-MM-dd"/>' onClick="WdatePicker()" readonly/>
        	</div>
            <div class="activitylist_bodyer_right_team_co1">
            ...
            <div class="activitylist_bodyer_right_team_co4">
                ...
                <input type="text" class="inputTxt" placeholder="输入金额" name="startCurrency" id="startCurrency" value="${param.startCurrency}"/><span>~</span><input type="text" class="inputTxt" placeholder="输入金额"  name="endCurrency" id="endCurrency" value="${param.endCurrency}"/>
            </div>
	     </div>
      </div>
	</form:form>
	...
	<table id="contentTable" class="table activitylist_bodyer_table mainTable" >
		...
		<tbody>
          ...
          <td class="tc">${activity.groupCode}</td>
              ...
              <a target="_blank" href="${ctx}/pay/review/islandDetail/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}/${reviewCompanyId}?from=operatorPre">审核</a>               
         </c:if>   
              </td>           
              <td class="p00">            
          <a target="_blank" href="${ctx}/pay/review/islandRead/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}?from=operatorPre">查看</a>&nbsp;
