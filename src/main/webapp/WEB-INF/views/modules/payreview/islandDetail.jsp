...
<head>
    ...
    */
    ...
    var getCost = function(classType, $List){
        ...
        $.ajax({
            type: "GET",
            url: "${ctx}/pay/manager/getCosts/" + classType + "/" + groupId,
            dataType:"json",
            async:false,
            cache:false,
            data:{
    ...
    var save = function(){
        ...
        $.ajax({
            type: "POST",
            url: "${ctx}/pay/manager/flow/start/save/" + groupId,
            data : {types : "operator,operatorBudget"},
            async:false,
            success: function(e){
                if(e == "true"){
                    location.href = '${ctx}/pay/manager/list/operatorPre';
                }else{
                    top.$.jBox.tip('请求失败。','error');
                    return false;
                }
    ...
    var submit = function(){
        ...
        $.ajax({
            type: "POST",
            url: "${ctx}/pay/manager/flow/start/commit/" + groupId,
            data : {types : "operator,operatorBudget"},
            async:false,
            success: function(e){
                if(e == "true"){
     ...
     var submit = function (v, h, f) {            
            ...
            if(remarkLength > 200){
                top.$.jBox.tip('项目备注不能大于200字','error' ,{ focusId: 'comment' }); return false;
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/pay/review/denyactivitycost",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
                        ...
                        success: function(array){
                            window.location.href = "${ctx}/pay/review/list/${typeId}/${reviewLevel}";
                           // window.location.reload();                          
                        },
                        error : function(e){
...
function passActivityCost(dom,id,review){ 
  $.ajax({
                     type: "POST",                      
                     url: "${ctx}/pay/review/passactivitycost",                      
                     cache:false,
                     dataType:"json",
                     async:false,
...
function tosales()
 ...
 function saveCheckBox(id){   
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/pay/review/saveCostIslandList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
