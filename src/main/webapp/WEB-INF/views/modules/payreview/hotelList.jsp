...
<head>
   ...
   var _$review = $("#review").val();   
     ...
     $( ".spinner" ).spinner({
			...
			//展开筛选按钮
        //UG_V2 调用common.js里的展开/收起筛选方法 by tlw 20170314
//    		$('.zksx').click(function(){
...
function expand(child,obj,srcActivityId){
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/pay/review/hotel/${reviewLevel}");
			$("#searchForm").submit();
	    }
            ...
            <div class="message_box_li">
              <c:choose>
                <c:when test="${userJobId==role.id}">
                      <a  href="${ctx}/pay/review/hotel/${reviewLevel}?userJobId=${role.id}">
...
<!--   <div class="activitylist_bodyer_right_team_co_bgcolor">-->
	 <form:form id="searchForm" modelAttribute="activityHotelGroup" action="${ctx}/pay/review/hotel/${reviewLevel}" method="post" >
    ...
    <input id="userJobId" name="userJobId"  type="hidden"  value="${userJobId}"> 
     <div class="activitylist_bodyer_right_team_co">    	 
    	 <div class="activitylist_bodyer_right_team_co2 pr">
         	<%--<label>团号：</label>--%>
             <input class="txtPro inputTxt searchInput" id="wholeSalerKey" placeholder="请输入团号"name="groupCode" value="${groupCode }"/>
		...
		<div class="ydxbd">
            ...
            <div class="activitylist_bodyer_right_team_co1">
      	    ...
      	    <div class="activitylist_bodyer_right_team_co1">
				...
				<input id="groupOpenDate" class="inputTxt dateinput" name="groupOpenDate" value='<fmt:formatDate value="${activityHotelGroup.groupOpenDate }" pattern="yyyy-MM-dd"/>' onfocus="WdatePicker({onpicking:function(dp){var vvv=dp.cal.getNewDateStr();if($dp.$('groupCloseDate').value==''){$dp.$('groupCloseDate').value=vvv;}}})" readonly/>
				<span> 至</span>
				<input id="groupEndDate" class="inputTxt dateinput" name="groupEndDate" value='<fmt:formatDate value="${activityHotelGroup.groupEndDate }" pattern="yyyy-MM-dd"/>' onClick="WdatePicker()" readonly/>
        	</div>
         	<div class="activitylist_bodyer_right_team_co1">
           		 <label class="activitylist_team_co3_text" >发起人：</label>
            ...
            <div class="activitylist_bodyer_right_team_co4">
                ...
                <input type="text" class="inputTxt" placeholder="输入金额" name="startCurrency" id="startCurrency" value="${param.startCurrency}"/>
                <span>~</span>
                <input type="text" class="inputTxt" placeholder="输入金额"  name="endCurrency" id="endCurrency" value="${param.endCurrency}"/>
            </div>
        ...
        <div class="activitylist_paixu">
          <div class="activitylist_paixu_left">
            ...
            <li class="activitylist_paixu_left_biankuang liupdateDate"><a onClick="sortby('updateDate',this)">更新时间</a></li>
             <li class="activitylist_paixu_left_biankuang ligroupOpenDate"><a onClick="sortby('groupOpenDate',this)">入住日期</a></li>
            </ul>
      </div>
		...
		<tbody>
          ...
          <td class="tc">${activity.groupCode}</td>
                  <td class="tc">${activity.activityName}</td>             
                    ...
                    <td class="tc">${fns:getHotelRoom(activity.activityUuid)}</td>
					<td>${activity.remNumber}/${activity.totalRoom}</td>
                    <td><fmt:formatDate pattern="yyyy-MM-dd" value="${activity.updateDate}"/></td>  
                    <td>${activity.createByName}</td>                   
                    <td id="result${activity.id}">
                   ${fns:getNextPayHotelReview(activity.id)} 
              </td>			 
              <td id="change${activity.id}">
                 <c:if test="${ activity.payNowLevel == reviewLevel && activity.payReview==1}"> 
              <a target="_blank" href="${ctx}/pay/review/hotelDetail/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}/${reviewCompanyId}?from=operatorPre">审核</a>               
         </c:if>   
              </td>           
              <td class="p00">            
          <a target="_blank" href="${ctx}/pay/review/hotelRead/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}?from=operatorPre">查看</a>&nbsp;
         <a href="${ctx }/cost/manager/forcastList/${activity.activityUuid}/${typeId}" target="_blank">预报单</a>&nbsp;
     <a href="${ctx }/cost/manager/settleList/${activity.activityUuid}/${typeId}" target="_blank">结算单</a>&nbsp;
