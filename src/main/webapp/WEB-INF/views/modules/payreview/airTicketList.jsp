...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/pay/review/airTicketList/${reviewLevel}");
			$("#searchForm").submit();
		}
...
<body>
		...
		<c:forEach items="${userJobs}" var="role">
			...
			<c:choose>
				<c:when test="${userJobId==role.id}">
					<a href="${ctx}/pay/review/airTicketList/${reviewLevel}?userJobId=${role.id}">
	...
	<div class="activitylist_bodyer_right_team_co_bgcolor inventory_management">
		<form:form id="searchForm" modelAttribute="activityAirTicket" action="${ctx}/pay/review/airTicketList/${reviewLevel}" method="post">
			...
			<div class="activitylist_bodyer_right_team_co">
				<div class="activitylist_bodyer_right_team_co2">
					<%--<label>出发城市：</label>--%>
					<input name="departureCity" type="text" class="inputTxt" value="${departureCity}" placeholder="出发城市">
						—
				<%--</div>--%>
				<%--<div class="activitylist_bodyer_right_team_co2 pr wpr20">--%>
					<%--<label>到达城市：</label>--%>
					<input value="${arrivedCity}" name="arrivedCity" class="inputTxt" placeholder="到达城市">
				</div>
				...
				<div class="ydxbd">
					...
					<div class="activitylist_bodyer_right_team_co1">
						...
						<input readonly="" onfocus="WdatePicker({onpicking:function(dp){var vvv=dp.cal.getNewDateStr();if($dp.$('groupCloseDate').value==''){$dp.$('groupCloseDate').value=vvv;}}})" value="${startingDate}" name="groupOpenDate" class="inputTxt dateinput" id="groupOpenDate">
						<span> 至 </span>
						<input readonly="" onclick="WdatePicker()" value="${returnDate}" name="groupCloseDate" class="inputTxt dateinput" id="groupCloseDate">
					</div>
					<div class="activitylist_bodyer_right_team_co1" >
						<label class="activitylist_team_co3_text" >审核发起人：</label>
		...
		<table id="contentTable" class="table activitylist_bodyer_table mainTable">
			...
			<c:forEach items="${page.list}" var="airticket" varStatus="s">
				...
				<tr id="review${airticket.id}_${review}">
					...
					<td>${airticket.payReserveNumber}</td>
					...
					<c:if test="${airticket.payNowLevel == reviewLevel && airticket.payReview==1}">
						<a href="${ctx}/pay/review/airTicketPreRecord/${airticket.airId}/${reviewLevel}/${reviewCompanyId}" target="_blank">审核</a>
					</c:if>
					</td>
					<td>
						<a href="${ctx}/pay/review/airTicketRead/${airticket.airId}/${reviewLevel}" target="_blank">查看</a>
						<br><a href="${ctx}/cost/manager/forcastList/${airticket.airId}/7" target="_blank">预报单</a>
						<br><a href="${ctx}/cost/manager/settleList/${airticket.airId}/7" target="_blank">结算单</a><br>  
						...
						<a href="javascript:void(0)" onClick="revokePayAudit('${airticket.id}','${airticket.payNowLevel}','${airticket.product_type_id }')">撤销</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
			...
			<div class="pagination">
				<dl>
					...
					<dd>
						...
						<input type="button" value="批量审批" class="btn" onclick="jbox__shoukuanqueren_chexiao_fab();">
					</dd>
				</dl>
			</div>
		</c:if>
