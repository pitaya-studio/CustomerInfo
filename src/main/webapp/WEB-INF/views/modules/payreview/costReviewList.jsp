...
<body> 
			...
			<div class="message_box_li">
				<c:choose>
					<c:when test="${userJobId==role.id}">
						<a href="${ctx}/pay/review/list/${typeId}/${reviewLevel}?userJobId=${role.id}">
	...
	<div class="activitylist_bodyer_right_team_co_bgcolor inventory_management">
		<form:form id="searchForm" modelAttribute="travelActivity" action="${ctx}/pay/review/list/${typeId}/${reviewLevel}" method="post">
			...
			<div class="activitylist_bodyer_right_team_co">
				...
				<div class="ydxbd">
					...
					<div class="activitylist_bodyer_right_team_co1">
						<label>出团日期：</label>
						<input id="groupOpenDate" class="inputTxt dateinput" name="groupOpenDate" value='<fmt:formatDate value="${travelActivity.groupOpenDate }" pattern="yyyy-MM-dd"/>' onfocus="WdatePicker()" readonly/>
			...
			<c:forEach items="${page.list}" var="activity" varStatus="s">
				...
				<c:if test="${not empty review }">
					<tr id="review${activity.id}_${review}">
						...
						<td>
							<div id="area${s.count}">
								<c:forEach items="${fns:findTargetAreaById(activity.srcActivityId)}" var="targetArea" varStatus="t">
									<c:if test="${t.count<3}">${targetArea.name},</c:if>
									<c:if test="${t.count eq 3}">${targetArea.name}&nbsp;</c:if>
									<c:if test="${t.count eq 4}">.......</c:if>
								</c:forEach> 
							</div>
						</td>
						...
						<td id="result${activity.id}">${fns:getNextPayReview(activity.id)}</td>
						<td class="tc" id="change${activity.id}">
							<c:if test="${activity.payNowLevel == reviewLevel && activity.payReview==1}"> 
								<a target="_blank" href="${ctx}/pay/review/start/${activity.productId}/${activity.agpId}/${reviewLevel}/${reviewCompanyId}?from=operatorPre">审核</a>
							</c:if>
						</td>
						<td class="p00">
							<a target="_blank" href="${ctx}/pay/review/read/${activity.productId}/${activity.agpId}/${reviewLevel}?from=operatorPre">查看</a>
						...
						<c:if test="${activity.payUpdateBy eq fns:getUser().id and activity.payNowLevel ne 1 and activity.payNowLevel-1 ne reviewCompany.topLevel and activity.payReview ne 0 }">
							<a href="javascript:void(0)" onClick="revokePayAudit('${activity.id}','${activity.payNowLevel}','${activity.activityKind }')">撤销</a>
						</c:if>
						</td>
					</tr>
				</c:forEach>
		...
		<c:if test="${empty review}">
			<div class="pagination">
				<dl>
					...
					<dd>
						<input class="btn ydbz_x" type="button" onclick="jbox__shoukuanqueren_chexiao_fab();" value="批量开票">
					</dd>
				</dl>
			</div>
		</c:if>
