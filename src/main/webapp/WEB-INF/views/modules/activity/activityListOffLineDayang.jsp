...
<%--					</td>--%>
					...
					<c:if test="${activityKind eq '10'}">
						<td id="suggestadultprice${activity.id }" class="tr"><c:if test="${activity.suggestAdultPrice==0}">价格待定</c:if>
						<c:if test="${activity.suggestAdultPrice>0}">${fns:getCurrencyInfo(activity.currencyType,1,'mark')}<span class="tdblue fbold">
						<fmt:formatNumber type="currency" pattern="#,##00.00" value="${activity.suggestAdultPrice}" /></span>起</c:if></td>
					</c:if>
					<td class="p0">
                    	<dl class="handle">
