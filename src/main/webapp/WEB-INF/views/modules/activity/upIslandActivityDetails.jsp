...
<script type="text/javascript" src="${ctxStatic}/js/upIslandPrices.js"></script>
...
<!--右侧内容部分开始-->
                ...
                <!--币种模板结束-->
                <div class="mod_nav">订单 > <c:set var="productType" value="${productTypeSecond}"></c:set><c:if test="${productType==1}">单团</c:if><c:if test="${productType==2}">散拼</c:if><c:if test="${productType==3}">游学</c:if><c:if test="${productType==4}">大客户</c:if><c:if test="${productType==5}">自由行</c:if><c:if test="${productType==10}">游轮</c:if><c:if test="${productType==12}">海岛游</c:if> > 改价记录 > 改价申请</div>
                <!-- add by jyang -->
