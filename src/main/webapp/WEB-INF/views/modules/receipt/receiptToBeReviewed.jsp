...
function expand(child, obj, srcActivityId, payMode_deposit, payMode_advance, payMode_full, payMode_op, 
	payMode_cw, payMode_data, payMode_guarantee, payMode_express, showType) {
    if ($(child).is(":hidden")) { //展开订单列表
        if (payMode_deposit != "") {
            ...
            $.ajax({
                type: "post",
                url: "${ctx}/receipt/limit/supplyviewdetailopen/" + payMode_deposit + "?1=1&currencyId=" + currencyId 
                	+ "&orderMoneyBegin=" + orderMoneyBegin + "&orderMoneyEnd=" + orderMoneyEnd + "&operator=" + operator,
                success: function(msg) {
                    if (msg.length > 0) {
                        ...
                        for (var i = 0; i < msg.length; i++) {
                            ...
                            + msg[i][14] + "</td><td class='tr'>" + msg[i][16] + "</td><td class='tr'>" + msg[i][17] 
                            + "</td><td class='tr'>" + msg[i][18] + "</td><td class='tr'>" + msg[i][11] + "</td></tr>";
                        }
...
//明细页
function viewdetail(uuid, verifyStatus, orderType) {
    verifyStatus = '-2';
    window.open("${ctx}/receipt/limit/supplyviewrecorddetail/" + uuid + "/" + verifyStatus + "/" + orderType);
}
//审核页
function makinvoice(uuid, verifyStatus, orderType) {
    window.location.href = "${ctx}/receipt/limit/supplyreviewrecorddetail/" + uuid + "/" + verifyStatus + "/" + orderType;
...
<body>
     <page:applyDecorator name="receipt_head">
           <page:param name="current">review</page:param>
     </page:applyDecorator>
      <!--右侧内容部分开始-->
        <form method="post" id="searchForm" action="${ctx}/receipt/limit/supplyreceiptlist/0">
          <div class="activitylist_bodyer_right_team_co">
            <div class="activitylist_bodyer_right_team_co2 pr">
                <%--bug 17457 添加class inputTxt by tlw at 20170307--%>
              <input id="orderNum" name="orderNum" class="searchInput inputTxt inputTxtlong" value="${orderNum }" placeholder="请输入订单号"/>
             </div>
            ...
            <div style="display: none;" class="ydxbd">
                <span></span>
				<input type = "hidden" name = "selectVerifyStatus" value = "0"><!-- 写死 0标示待审核 -->
                ...
                <div class="activitylist_bodyer_right_team_co1">
	                <div class="activitylist_team_co3_text">开收据状态：</div>
                    <div class="selectStyle">
                        <select id="createStatus" name="createStatus">
                            <option value="" <c:if test="${empty createStatus}">selected</c:if>>全部</option>
                            <option value="0" <c:if test="${createStatus eq '0'}">selected</c:if>>待开</option>
                            <option value="1" <c:if test="${createStatus eq '1'}">selected</c:if>>已开</option>
                            <option value="2" <c:if test="${createStatus eq '2'}">selected</c:if>>已还</option>
                        </select>
                    </div>
				</div>
                <div class="activitylist_bodyer_right_team_co1">
        ...
        <table class="activitylist_bodyer_table mainTable" id="contentTable">
          ...
          <c:forEach items="${page.list}" var="receiptInfo" varStatus="s">
            <tr>
              ...
              <td class="tc"><c:if test="${receiptInfo['createStatus'] eq '1' || receiptInfo['createStatus'] eq '2'}">
              ...
              <td><a href="javascript:void(0)" onClick="makinvoice('${receiptInfo['uuid']}','0','${receiptInfo['orderType']}')">审核</a>
              <a href="javascript:void(0)" onClick="viewdetail('${receiptInfo['uuid']}','0','${receiptInfo['orderType']}')">明细</a> <br />
