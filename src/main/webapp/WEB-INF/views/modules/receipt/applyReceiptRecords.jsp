...
//撤销开收据状态 改为未开
function revokeToUninvioce(uuid, createStatus){
	var submit = function (v, h, f) {
	    if (v == 'ok') {
	    	$.jBox.tip("正在撤销数据...", 'loading');
	    	$.ajax({ 
		          type:"POST",
		          url:"${ctx}/receipt/limit/revokeToUninvioce/" + uuid+"/" + createStatus,
		          dataType:"json",
		          data:{},
...
//还收据
function returnReceipt(uuid){
	var submit = function (v, h, f) {
	    if (v == 'ok') {
	    	$.jBox.tip("还数据中...", 'loading');
	    	$.ajax({ 
		          type:"POST",
		          url:"${ctx}/receipt/limit/returnReceipt/" + uuid,
		          dataType:"json",
		          success:function(data){
		        	if('success' == data.success){
		        		$.jBox.tip('还收据成功', 'success');
		        		$("#searchForm").submit();
		        	}else{
		        		$.jBox.tip(data.fail, 'error');
		        	}
		          }
		      });
	    }
...
<body>
 	<page:applyDecorator name="receipt_head">
		<page:param name="current">a_rece_c</page:param>
	</page:applyDecorator>
      <!--右侧内容部分开始-->
        <form method="post" id="searchForm" action="${ctx}/receipt/limit/supplyreceiptlist/2">
          	<div class="activitylist_bodyer_right_team_co">
            	<div class="activitylist_bodyer_right_team_co2 pr">
             		<input id="groupCode" name="groupCode" class="inputTxt inputTxtlong" value="${groupCode }"placeholder="请输入团号"/>
             	</div>
            	...
            	<div style="display: none;" class="ydxbd">
					<span></span>
             		...
             		<div class="activitylist_bodyer_right_team_co1">
                		<label class="activitylist_team_co3_text">开收据金额：</label>
        ...
        <table class="activitylist_bodyer_table mainTable" id="contentTable">
          ...
          <c:forEach items="${page.list}" var="receiptInfo" varStatus="s">
            <tr>
              ...
              <td class="tc tda">
	              <c:if test="${receiptInfo['createStatus'] eq  '1'}">
 	              	<a href="javascript:void(0)" onClick="revokeToUninvioce('${receiptInfo['uuid']}', 0)">撤销</a> 
	              </c:if> 
	              <c:if test="${receiptInfo['createStatus'] eq  '2'}"> 
	              	<a href="javascript:void(0)" onClick="revokeToUninvioce('${receiptInfo['uuid']}', 1)">撤销</a> 
	              </c:if> 
	              <c:if test="${receiptInfo['createStatus'] eq  '0' || receiptInfo['createStatus'] eq  ''}">
		              <a href="javascript:void(0)" onClick="makinvoice('${receiptInfo['uuid']}','-1','${receiptInfo['orderType']}')">开收据</a>
	              </c:if>
	              <c:if test="${receiptInfo['createStatus'] eq  '1'}">
	              		<a href="javascript:void(0)" onClick="returnReceipt('${receiptInfo['uuid']}')">还收据</a>
	              </c:if>
	              <a href="javascript:void(0)" onClick="viewdetail('${receiptInfo['uuid']}','${param.verifyStatus}','${receiptInfo['orderType']}')">明细</a> <br />
				  <a class="team_a_click" href="javascript:void(0)" onclick="expand('#child${s.count}',this,'${s.count}','${receiptInfo['uuid']}')">展开订单列表</a>
			</td>
            </tr>
