		...
		<table id="treeTable" class="table table-striped table-bordered">
			...
			<c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne 1?menu.parent.id:'0'}">
					<td width="20%"><i class="icon-${not empty menu.icon?menu.icon:' hide'}"></i><a href="${ctx}/sys/menu/form?id=${menu.id}">${menu.name}</a></td>
					<td width="37%">${menu.href}</td>
					...
					<td width="3%">${menu.isShow eq 1?'显示':'隐藏'}</td>
					<td width="22%">${menu.permission}</td>
					<shiro:hasPermission name="sys:menu:edit"><td width="12%">
