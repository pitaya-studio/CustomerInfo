...
function show(uuid){
	window.open("${ctx}/countryLine/show/" + uuid );
}
function edit(uuid){
	window.open( "${ctx}/countryLine/edit/" + uuid );
}
function del(uuid){
	var ids = [];
	ids.push(uuid);
...
function v_deleteItems(ids){
	...
	$.jBox.confirm("确定要删除数据吗？", "系统提示", function(v, h, f) {
		if (v == 'ok') {
			...
			$.post( "${ctx}/countryLine/delete", {"uuids":ids.join(",")}, 
				function(data){
					if(data.result=="1"){
...
function page(n,s){
	$("#pageNo").val(n);
	$("#pageSize").val(s);
	$("#searchForm").attr("action"," ${ctx}/countryLine/list");
	$("#searchForm").submit();
}
</script>	
</head>
...
<!--右侧内容部分开始-->
   ...
   <form id="searchForm" modelAttribute="countryLineQuery" action=" ${ctx}/countryLine/list" method="post">
...
<input value="${countryLineQuery.uuid}" id="uuid" name="uuid"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
线路国家uuid：
</div>
<input value="${countryLineQuery.countryUuid}" id="countryUuid" name="countryUuid"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
线路国家名称：
</div>
<input value="${countryLineQuery.countryName}" id="countryName" name="countryName"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
国家简称：
</div>
<input value="${countryLineQuery.countryShort}" id="countryShort" name="countryShort"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
备注：
</div>
<input value="${countryLineQuery.remark}" id="remark" name="remark"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
创建人：
</div>
<input value="${countryLineQuery.createBy}" id="createBy" name="createBy"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
创建时间：
</div>
<input onclick="WdatePicker()" readonly value="<fmt:formatDate pattern="yyyy-MM-dd" value="${countryLine.createDate}"/>" id="createDate" name="createDate"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
修改人：
</div>
<input value="${countryLineQuery.updateBy}" id="updateBy" name="updateBy"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
修改时间：
</div>
<input onclick="WdatePicker()" readonly value="<fmt:formatDate pattern="yyyy-MM-dd" value="${countryLine.updateDate}"/>" id="updateDate" name="updateDate"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
删除状态：
</div>
<input value="${countryLineQuery.delFlag}" id="delFlag" name="delFlag"/>								</div>
	...
	<a class="btn btn-primary" href=" ${ctx}/countryLine/form" target="_blank">添加</a>
</div>
		            ...
		            <c:forEach var="entry" items="${page.list}" varStatus="v">
			          <tr>
			            ...
			            <td align="center"><input name="ids" type="checkbox" value="${entry.uuid}" /></td>
							<td>${entry.uuid}</td>
							<td>${entry.countryUuid}</td>
							<td>${entry.countryName}</td>
							<td>${entry.countryShort}</td>
							<td>${entry.remark}</td>
