			...
			$("#inputForm").validate({
				rules:{
					label:{
						...
						remote: {
							type: "POST",
							url: "${ctx}/countryLine/check?type="+$('#type').val()+"&uuid="+$('#uuid').val()
								}
						},
					sort:{
						required:true,
				...
				submitHandler: function(form){
					var url = "";
					if($("#uuid").val()=='') {
						url="${ctx}/countryLine/save";
					} else {
						url="${ctx}/countryLine/update";
					}
...
<body>
	...
	<form:form method="post" modelAttribute="countryLineInput" action="" class="form-horizontal" id="inputForm" novalidate="">
		...
		<div class="maintain_add">
			...
			<p>
				<label><em class="xing"></em>线路国家uuid：</label>
				<form:input path="countryUuid" htmlEscape="false" maxlength="11" class="required" />
			</p>
			<p>
				<label><em class="xing"></em>线路国家名称：</label>
				<form:input path="countryName" htmlEscape="false" maxlength="11" class="required" />
			</p>
			<p>
				<label><em class="xing"></em>国家简称：</label>
				<form:input path="countryShort" htmlEscape="false" maxlength="11" class="required" />
			</p>
			<p>
			...
			<p>
				...
				<input readonly onclick="WdatePicker()" value="<fmt:formatDate value="${countryLine.createDate}" pattern="yyyy-MM-dd" />" name="createDate" class="inputTxt dateinput" id="createDate" />
			</p>
			<p>
			...
			<p>
				...
				<input readonly onclick="WdatePicker()" value="<fmt:formatDate value="${countryLine.updateDate}" pattern="yyyy-MM-dd" />" name="updateDate" class="inputTxt dateinput" id="updateDate" />
			</p>
			<p>
