...
<title>
	<c:if test="${newORold==0 }">公司信息-新增第2步-网站信息</c:if>
	<c:if test="${newORold==1 }">公司信息-修改第2步-网站信息</c:if>
</title>
...
<script type="text/javascript">
	$(function(){
		...
		$("#last").click(function(){
			// 跳转到上一步页面
			location.href=contextPath + "/company/saler/backWholeOfficeOne/${office.id}/${newORold}";
		});
		// 下一步
		$("#next").click(function(){
	...
	function subForm(){
		...
		$.ajax({
			type:"POST",
			url:contextPath+"/company/saler/addWholeOfficeTwo",
			data:the_param,
			dataType:"text",
			success:function(html){
				...
				if(json.res=="success"){
					...
					if(saveOrnext==1){
						...
						// 跳转到下一步页面
						location.href=contextPath + "/company/saler/gotoAddWholeOfficeThree/${office.id}/${newORold}";
					}
