...
<title>
	<c:if test="${newORold==0 }">公司信息-新增第3步-银行账户信息</c:if>
	<c:if test="${newORold==1 }">公司信息-修改第3步-银行账户信息</c:if>
</title>
...
<script type="text/javascript">
	$(function(){
		...
		$("#last").click(function(){
			// 跳转到上一步页面
			location.href=contextPath + "/company/saler/backWholeOfficeTwo/${office.id}/${newORold}";
		});
		// 下一步
		$("#next").click(function(){
			// 跳转到下一步页面
			location.href=contextPath + "/company/saler/gotoAddWholeOfficeFour/${office.id}/${newORold}";
		});
		// 提交
		$("#save").click(function(){
			// 跳回批发商列表
	...
	function ajaxForm(the_param){
		$.ajax({
			type : "POST",
			url : contextPath+"/company/saler/addNewBlankCode",
			data:the_param,
			dataType:"text",
	...
	function delBank(){
		...
		if(id){ // 判断，如果id 存在，则从数据库删除该记录，否则只删除页面div
			...
			$.ajax({
				type : "POST",
				url:contextPath+"/company/saler/delNewBlankCode",
				data:the_param,
				dataType:"text",
				success:function(html){
					var json;
