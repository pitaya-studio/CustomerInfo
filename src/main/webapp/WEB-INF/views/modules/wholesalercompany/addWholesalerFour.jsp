...
<title>
	<c:if test="${newORold==0 }">公司信息-新增第4步-资质</c:if>
	<c:if test="${newORold==1 }">公司信息-修改第4步-资质</c:if>
</title>
...
<script type="text/javascript">
	$(function(){
		...
		$("#last").click(function(){
			// 跳转到上一步页面
			location.href=contextPath + "/company/saler/backWholeOfficeThree/${office.id}/${newORold}";
		});
		// 提交
		$("#save").click(function(){
   ...
   // 	$("#upfileShow").append("<p class='seach_r'><span  class='seach_checkbox'  id='"+obj.name+"'></span></p>");
     ...
     */
     ...
     function ajaxSub(){
    	 ...
    	 $.ajax({
    		 type:"POST",
    		 url : contextPath+"/company/saler/addWholeOfficeFour",
    		 data : param,
 			dataType : "text",
	                      ...
	                      <input type="button" name="acctype1"  class="btn btn-primary" value="上传文件"  onclick="uploadFiles(' ${ctx}','passportfile',this,'true');"/>
	           			   <span class="thecheckbox">
	           			   		<c:if test="${not empty qual1 }">
		           			   		<a>${qual1.docInfoName }
		           			   			<input type="hidden"  name="salerTripFileId"  value="${qual1docInfoId }"/>
