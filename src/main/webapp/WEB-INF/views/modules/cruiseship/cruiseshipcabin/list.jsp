...
function show(uuid){
	window.open("${ctx}/cruiseshipCabin/show/" + uuid );
}
function edit(uuid){
	window.open( "${ctx}/cruiseshipCabin/edit/" + uuid );
}
function del(uuid){
	var ids = [];
	ids.push(uuid);
...
function v_deleteItems(ids){
	...
	$.jBox.confirm("确定要删除数据吗？", "系统提示", function(v, h, f) {
		if (v == 'ok') {
			...
			$.post( "${ctx}/cruiseshipCabin/delete", {"uuids":ids.join(",")}, 
				function(data){
					if(data.result=="1"){
						top.$.jBox.tip('删除成功!');
						$("#searchForm").submit();
...
function page(n,s){
	$("#pageNo").val(n);
	$("#pageSize").val(s);
	$("#searchForm").attr("action"," ${ctx}/cruiseshipCabin/list");
	$("#searchForm").submit();
}
</script>	
</head>
...
<!--右侧内容部分开始-->
   ...
   <form id="searchForm" modelAttribute="cruiseshipCabinQuery" action=" ${ctx}/cruiseshipCabin/list" method="post">
...
<input value="${cruiseshipCabinQuery.uuid}" id="uuid" name="uuid"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
游轮UUID：
</div>
<input value="${cruiseshipCabinQuery.cruiseshipInfoUuid}" id="cruiseshipInfoUuid" name="cruiseshipInfoUuid"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
批发商id：
</div>
<input value="${cruiseshipCabinQuery.wholesalerId}" id="wholesalerId" name="wholesalerId"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
舱型名称：
</div>
<input value="${cruiseshipCabinQuery.name}" id="name" name="name"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
创建人：
</div>
<input value="${cruiseshipCabinQuery.createBy}" id="createBy" name="createBy"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
创建时间：
</div>
<input onclick="WdatePicker()" readonly value="<fmt:formatDate pattern="yyyy-MM-dd" value="${cruiseshipCabin.createDate}"/>" id="createDate" name="createDate"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
修改人：
</div>
<input value="${cruiseshipCabinQuery.updateBy}" id="updateBy" name="updateBy"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
修改时间：
</div>
<input onclick="WdatePicker()" readonly value="<fmt:formatDate pattern="yyyy-MM-dd" value="${cruiseshipCabin.updateDate}"/>" id="updateDate" name="updateDate"/>								</div>
								<div class="activitylist_bodyer_right_team_co1">
<div class="activitylist_team_co3_text">
删除标识：
</div>
<input value="${cruiseshipCabinQuery.delFlag}" id="delFlag" name="delFlag"/>								</div>
	...
	<a class="btn btn-primary" href=" ${ctx}/cruiseshipCabin/form" target="_blank">添加</a>
</div>
