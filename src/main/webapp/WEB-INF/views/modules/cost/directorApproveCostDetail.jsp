...
/*         eventHandler('operatorBudget', $('#contentTable'));
        ...
        getCost('finance', $('#financeSpecific'), false);
        calAllPrice();
        var ids = "";
        var names = "";
        ...
        $.ajax({
            ...
            success: function(array){
                ...
                for(var i = 0; i < array.length; i++){
                    htmldoc += '<tr id=' + classType + "_" + array[i].id + '> <td>' + (i + 1) + '</td><td>'+array[i].name+'</td><td price-sum-bind="' + classType + '" class="tr">'+array[i].price+'</td><td>'+array[i].comment+'</td>';
                    if(editable){
                        if(array[i].directorRefused == 1){
                            htmldoc += '<td id="refuseLabel_' + array[i].id + '"><span style="color:#eb0205;" >拒绝</span></td></tr>';
                        }else{
                            htmldoc += '<td id="refuseLabel_' + array[i].id + '"><span>通过</span></td></tr>';
                        }
                    }
                }
                $List.append(htmldoc);
...
<%--                    <c:choose>
            ...
            <span>全部订单：<i id="order-count">${ordersize}</i>&nbsp;单</span>
