    ...
    var deleteCost = function(id, classType){
        $.jBox.confirm("确定要删除成本吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/island/deleteHotel",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
      ...
      var cancelCost = function(id, classType){
        $.jBox.confirm("确定要撤回成本审核吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/island/cancelHotel",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
        ...
        $.jBox.confirm("确定要撤回付款审核吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/island/cancelPayHotel",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
...
function addCurrency(activityId,groupId,typeId) {      
  ...
  function addCostHQX(budgetType) {
      var groupId = '${activityGroup.id}';
           var iframe = "iframe:${ctx}/cost/island/addHotelCost/"+ '${activityHotelUuid}' +"/"+ '${activityHotelGroupUuid}' +"/"+ budgetType  +"/"+ '${typeId}' +"/"+ '${deptId}'; 
      $.jBox(iframe, {
            title: "成本录入",
            width: 400,
            height: 660,
  ...
  function addOtherCost(budgetType) {
    ...
    function updateCostHQX(costid) {
      var groupId = '${activityGroup.id}';
      var iframe = "iframe:${ctx}/cost/island/updateHotelCost/"+ '${activityHotelUuid}' +"/"+ '${activityHotelGroupUuid}' +"/"+ costid  +"/"+ '${typeId}' +"/"+ '${deptId}';         
  ...
  function updateOtherCost(costid) {
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/island/saveCostHotelList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
  ...
  function payCheckBox(id,budgetType){   
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/island/savePayHotelList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
...
<body>
    ...
    <div class="mod_nav">运控 > 产品成本录入 > 酒店成本录入详情页</div>
            ...
            <div class="mod_information_dzhan_d mod_details2_d">
                ...
                <table border="0" width="90%">
                    <tr>
                        ...
                        <td class="mod_details2_d1">入住日期：</td>
                        <td class="mod_details2_d2">
						<fmt:formatDate pattern="yyyy-MM-dd" value="${activityGroup.groupOpenDate }"/></td>
                        <td  width="25%">&nbsp;</td>
                    ...
                    <tr>
                        ...
                        <td class="mod_details2_d2" > ${hotelName}</td>
                        <td class="mod_details2_d1">上岛方式：</td>
                        <td class="mod_details2_d2">
                        	<c:forEach items="${fn:split(activityGroup.islandWay,';')}" var="var">
...
<!--               <td class="tc"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${groupOrder.applyTime}"/></td> -->
              ...
              <td class="tc"><a href="${ctx}/hotelOrder/hotelOrderDetail/${groupOrder.orderUuid}" target="_blank">详情</a>
			  </td>
            </tr>
            </c:forEach>
          </tbody>
                ...
                <table class="table table-striped table-bordered table-condensed table-mod2-group table-toggel mod_information_dzhan">
                     ...
                     <td  class="tc">
                       <c:forEach items="${curlist}" var="currency">
                        ...
                        <c:if test="${budgetIn.reviewType==0}">
                         ${fns:getNextCostHotelReview(budgetIn.id)} 
                        </c:if>
                         <c:if test="${budgetIn.reviewType!=0}">
                    ...
                    <tr>
                      ...
                      <td  class="tc">                     
                       ...
                       ${fns:getNextCostHotelReview(budgetOut.id)} 
                        </c:if>
                         <c:if test="${budgetOut.reviewType!=0}">
                    ...
                    <thead>
                     ...
                     <tbody>
                      <c:forEach items="${actualInList}" var="actualIn" varStatus="status">
                       ...
                       ${fns:getNextCostHotelReview(actualIn.id)} 
                        </c:if>
                         <c:if test="${actualIn.reviewType!=0}">
                    ...
                    <tr>
                       ...
                       <c:forEach items="${curlist}" var="currency">
                        ...
                        <td  name="tdComment">${actualOut.comment}</td>
                         ...
                         ${fns:getNextCostHotelReview(actualOut.id)} 
                        </c:if>
                         <c:if test="${actualOut.reviewType!=0}">
