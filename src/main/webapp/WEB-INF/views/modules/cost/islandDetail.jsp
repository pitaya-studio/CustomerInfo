    ...
    var deleteCost = function(id, classType){
        $.jBox.confirm("确定要删除成本吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/island/deleteIsland",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
      ...
      var cancelCost = function(id, classType){
        $.jBox.confirm("确定要撤回成本审核吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/island/cancelIsland",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
          ...
          var cancelPayCost = function(id, classType){
        $.jBox.confirm("确定要撤回付款审核吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/island/cancelPayIsland",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
...
function addCurrency(activityId,groupId,typeId) {      
  ...
  function addCostHQX(budgetType) {
      var groupId = '${activityGroup.id}';
        var iframe = "iframe:${ctx}/cost/island/addIslandCost/"+ '${activityIslandUuid}' +"/"+ '${activityIslandGroupUuid}' +"/"+ budgetType  +"/"+ '${typeId}' +"/"+ '${deptId}';       
  ...
  function addOtherCost(budgetType) {
      var groupId = '${activityGroup.id}';
      var iframe = "iframe:${ctx}/cost/island/addIslandCost/"+ '${activityIslandUuid}' +"/"+ '${activityIslandGroupUuid}' +"/"+ budgetType  +"/"+ '${typeId}' +"/"+ '${deptId}';     
  ...
  function updateCost(costid) {
      var groupId = '${activityGroup.id}';
      var iframe = "iframe:${ctx}/cost/island/updateIslandCost/"+ '${activityIslandUuid}' +"/"+ '${activityIslandGroupUuid}' +"/"+ costid  +"/"+ '${typeId}' +"/"+ '${deptId}';     
  ...
  function updateOtherCost(costid) {
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/island/saveCostIslandList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
  ...
  function payCheckBox(id,budgetType){   
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/island/savePayIslandList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
...
function expand(child,obj,id) {
        $.ajax({
           url:"${ctx}/cost/island/payedRecord/",
           type:"POST",
           data:{id:id,flag:"island"},
           success:function(data){
             var htmlstr=""
             var num = data.length;
             if(num>0){
          ...
          htmlstr+="</td><td class='tc'>"+str1+"</td>"+"</tr>";
             	...
             	htmlstr+="<tr><td colspan='6' class='tc'>暂无收款记录</td></tr>";
             }
             $("#rpi").html(htmlstr);
           }
        });
...
<body>
    ...
    <div class="mod_nav">运控 > 产品成本录入 > 海岛游成本录入详情页</div>
                ...
                <table border="0" width="90%">
                    <tr>
                        ...
                        <td class="mod_details2_d2">
                            <c:choose> 
                                 <c:when test="${fn:length(activityIsland.activitySerNum) > 20}"> 
                                    <a style="text-decoration: none; color:inherit; cursor:default;" title="${activityIsland.activitySerNum}"><c:out value="${fn:substring(activityIsland.activitySerNum, 0, 20)}..." /></a> 
                                 </c:when> 
                                 <c:otherwise> 
                                    <c:out value="${activityIsland.activitySerNum}" /> 
                                 </c:otherwise>
                            </c:choose>
                        ...
                        <td class="mod_details2_d1">截团日期：</td>
                        <td class="mod_details2_d2">  <fmt:formatDate value="${activityGroup.groupEndDate}" pattern="yyyy-MM-dd"/></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="mod_details2_d1">航班信息：</td>
                        <td class="mod_details2_d2" >${activityIslandGroupAirline.flightNumber } </td>
                        <td class="mod_details2_d1">上岛方式：</td>
                        <td class="mod_details2_d1"><trekiz:autoId2Name4Table tableName="sysdefinedict" sourceColumnName="uuid" srcColumnName="label" value="${activityGroup.islandWay}"/>  </td>
                        ...
                        <td class="mod_details2_d2">${activityGroup.advNumber }</td> 
                    </tr>                  
                </table>
                <div class="kong"></div>
            </div>
...
<!--               <th width="10%">转报名时间</th> -->
          ...
          <c:forEach items="${orderList}" var="groupOrder">
            <tr >
              ...
              <td class="tc"><div class="groupCode">${groupOrder.groupCode}</div>  <div  style="display:none;" class="activityName">${groupOrder.activityName}</div></td>
              <td class="tc">              
...
<!--               <td  class="tc"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${groupOrder.applyTime}"/></td> -->
                      ...
                      <div class="ydbz_tit">
                            ...
                            <div class="button_addcb wpr20 xtjxm">
                              <c:if test="${activityGroup.lockStatus==0}">                             
                                  <c:if test="${companyId!=71 || companyId==71 && isOperator==1}"><a class="button_addcb_a" onclick="addOtherCost('2')">添加项目</a></c:if>                                 
                              </c:if>  
                          </div>
                      </div>
                  </div>
                    ...
                    <thead>
                       ...
                       <c:forEach items="${curlist}" var="currency">
                        ...
                        <td class="p0 tr">
                          <div class="yfje_dd"> 
                        <span class="fbold">
                          <c:if test="${otherCost.payedMoney != '0' }">
                    <c:forEach items="${curlist}" var="currency">
                                  <c:if test="${currency.id==otherCost.currencyId}">
                                     ${currency.currencyMark}
                                  </c:if>
                                </c:forEach>${otherCost.payedMoney }
                              </c:if>
                          </span>
                      </div>
                      <div class="dzje_dd">
                        <span class="fbold">
                          <c:if test="${otherCost.confirmMoney != '0' }">
                    <c:forEach items="${curlist}" var="currency">
                                  <c:if test="${currency.id==otherCost.currencyId}">
                                     ${currency.currencyMark}
                                  </c:if>
                                </c:forEach>${otherCost.confirmMoney }
                            </c:if>
                        </span>
                      </div>
            </td>   
                        <td name="tdComment"><xmp>${otherCost.comment}</xmp></td>                        
                        <td class="tc">                       
                        ...
                        <c:if test="${companyId!=71 || companyId==71 && isOpt!=1 }">                         
                          <a href="javascript:void(0)" onclick="updateOtherCost('${otherCost.id}')">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)"  onclick="return deleteCost('${otherCost.id}','operator')">删除</a>                         
                         </c:if>                        
                         </c:if>
                          <c:if test="${activityGroup.lockStatus==1}">
                            已经锁定
                          </c:if> 
                          <br><a href="${ctx}/cost/island/paymentConfirm/${otherCost.id}?payType=3&groupId=${activityGroup.id }&groupUuid=${otherCost.activityUuid }&orderType=${otherCost.orderType}&supplyName=${otherCost.supplyName}&totalCurrencyFlag=true" target="_blank">收款</a>&nbsp;&nbsp;
                ...
                <table class="table table-striped table-bordered table-condensed table-mod2-group table-toggel mod_information_dzhan">
                     ...
                     <td  class="tc">
                       <c:forEach items="${curlist}" var="currency">
                        ...
                        <c:if test="${budgetIn.reviewType==0}">
                         ${fns:getNextCostIslandReview(budgetIn.id)} 
                        </c:if>
                         <c:if test="${budgetIn.reviewType!=0}">
                        ...
                        <c:if test="${companyId!=71 || companyId==71 && isOperator==1 }">
                          ...
                          <a href="javascript:void(0)" onclick="updateCost('${budgetIn.id}')">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)"  onclick="return deleteCost('${budgetIn.id}','operator')">删除</a>
                         </c:if>
                    ...
                    <tr>
                       ...
                       ${budgetOut.supplyName}
                        ...
                        <td class="tc"  name="tdPrice"> <c:forEach items="${curlist}" var="currency">
                          <c:if test="${currency.id==budgetOut.currencyId}">
                             ${currency.currencyMark}
                          </c:if>
                      ...
                      <td  class="tc">                     
                       ...
                       ${fns:getNextCostIslandReview(budgetOut.id)} 
                        </c:if>
                         <c:if test="${budgetOut.reviewType!=0}">
                       ...
                       <td class="tc">                       
                        ...
                        <td class="tc">
                          ...
                          <a href="javascript:void(0)" onclick="updateCost('${budgetOut.id}')">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)"  onclick="return deleteCost('${budgetOut.id}','operator')">删除</a>
                         </c:if>
...
<c:if test="${companyId!=71 || companyId==71 && isOpt==1}"> 
                    ...
                    <thead>
                     ...
                     <tbody>
                      <c:forEach items="${actualInList}" var="actualIn" varStatus="status">
                       ...
                       ${fns:getNextCostIslandReview(actualIn.id)} 
                        </c:if>
                         <c:if test="${actualIn.reviewType!=0}">
                        ...
                        <td class="tc">
                          ...
                          <c:if test="${ actualIn.review==4  || actualIn.review==0 || actualIn.review==5}">
                           <a href="javascript:void(0)" onclick="updateCost('${actualIn.id}')">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)"  onclick="return deleteCost('${actualIn.id}','operator')">删除</a>
                         </c:if>
                          ...
                          <c:if test="${actualIn.review==2 && actualIn.payReview==1 }">
                           <a href="javascript:void(0)"  onclick="return cancelPayCost('${actualIn.id}','operator')">撤回付款申请</a>
                         </c:if> 
                         </c:if>
                          <c:if test="${activityGroup.lockStatus==1}">
                          已经锁定
                    ...
                    <tr>
                       ...
                       <c:forEach items="${curlist}" var="currency">
                        ...
                        <td  name="tdComment">${actualOut.comment}</td>
                         ...
                         ${fns:getNextCostIslandReview(actualOut.id)} 
                        </c:if>
                         <c:if test="${actualOut.reviewType!=0}">
                      ...
                      <td class="tc">                       
                        ...
                        <td class="tc"><c:if test="${activityGroup.lockStatus==0 && actualOut.reviewType==0}">
                          ...
                          <a href="javascript:void(0)" onclick="updateCost('${actualOut.id}')">修改</a>&nbsp;&nbsp;<a href="javascript:void(0)"  onclick="return deleteCost('${actualOut.id}','operator')">删除</a>
                         </c:if>
                          ...
                          <c:if test="${actualOut.review==2 && actualOut.payReview==1 }">
                           <a href="javascript:void(0)"  onclick="return cancelPayCost('${actualOut.id}','operator')">撤回付款申请</a>
                         </c:if> 
                         </c:if>
                          <c:if test="${activityGroup.lockStatus==1}">
                          已经锁定
          ...
          <ul class="cost-ul" data-total="income">
            ...
            <li>退款合计：￥<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${actualrefund }"/>
                 ...
                 <ul style="margin: 0px;float: right;">
                <c:if test="${costAutoPass==0 && activityGroup.lockStatus==0 }"><input  class="btn-primary" type="button" value="成本审核"  onclick="saveCheckBox('actualId','1')" /></c:if>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;<c:if test="${activityGroup.lockStatus==0}"><input  class="btn-primary" type="button" value="付款申请"  onclick="payCheckBox('payId','1')" /></c:if>                       
                    </ul> 
       </div>
        </c:if> 
...
function costSums(obj,objshow,ordertype){    
  ...
  if(ordertype==2 || (ordertype==0 && trimStr(thisReview) != '已取消')|| (ordertype==1 && trimStr(thisReview) != '已取消' && trimStr(thisReview) != '已驳回' )){       
