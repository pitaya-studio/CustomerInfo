      ...
      var cancelCost = function(id, classType){
        ...
        $.jBox.confirm("确定要撤回付款审核吗？", "提示", function(v, h, f) {
            if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/manager/cancelPay",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
...
function addCurrency(activityId,groupId,typeId) {      
  ...
  function addCostHQX(budgetType) {
      var groupId = '${activityGroup.id}';
      var iframe = "iframe:${ctx}/cost/island/addIslandCost/"+ '${activityId}' +"/"+ '${groupId}' +"/"+ budgetType  +"/"+ '${typeId}' +"/"+ '${deptId}';     
  ...
  function addOtherCostHQX(budgetType) {
    ...
    function updateCostHQX(costid) {
      var groupId = '${activityGroup.id}';
      var iframe = "iframe:${ctx}/cost/island/updateIslandCost/"+ '${activityId}' +"/"+ '${groupId}' +"/"+ costid  +"/"+ '${typeId}' +"/"+ '${deptId}';     
  ...
  function payCheckBox(id,budgetType){   
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/manager/savePayList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
...
<body>
      ...
      <page:param name="current"><c:choose><c:when test="${typeId==1}">single</c:when><c:when test="${typeId==2}">loose</c:when><c:when test="${typeId==3}">study</c:when><c:when test="${typeId==5}">free</c:when><c:when test="${typeId==4}">bigCustomer</c:when></c:choose></page:param>   
      </page:applyDecorator>    
    ...
    <div class="mod_information">
        ...
        <table class="activitylist_bodyer_table mod_information_dzhan" id="contentTable">
          ...
          <c:forEach items="${orderList}" var="groupOrder">
            <tr >
              <td>${groupOrder.orderCompanyName}</td>
                    ...
                    <thead>
                       ...
                       <c:forEach items="${curlist}" var="currency">
                        ...
                        <c:if test="${companyId!=71 || companyId==71 && isOpt!=1 }">                         
                          ...
                          <br><a href="${ctx}/cost/manager/paymentConfirm/${otherCost.id}?payType=3&groupId=${otherCost.activityId }&orderType=${otherCost.orderType}&supplyName=${otherCost.supplyName}" target="_blank">收款</a>&nbsp;&nbsp;
...
<c:if test="${companyId!=71 || companyId==71 && isOpt==1}"> 
                ...
                <table class="table table-striped table-bordered table-condensed table-mod2-group table-toggel mod_information_dzhan">
                    <thead>
                        <tr>
                            ...
                            <th width="6%">录入人</th>
                            <th width="8%">操作</th>
                            <c:if test="${companyId != 68 }"><th width="5%">成本<br>审核申请</th></c:if>
                    ...
                    <tr>
                        <td>境内小计</td>
                        <td colspan=<c:if test="${companyId != 68 }">"15"</c:if><c:if test="${companyId == 68 }">"14"</c:if>>&nbsp;<span id="actualInShow" name="actualInShow"></td>
                    </tr></c:if>
                    ...
                    <tr>
                        <td>境外小计</td>
                        <td colspan=<c:if test="${companyId != 68 }">"15"</c:if><c:if test="${companyId == 68 }">"14"</c:if>>&nbsp;<span id="actualOutShow" name="actualOutShow"></td>
                    </tr></c:if>
                   <!-- <tr>
                        <td>提交审核</td>
          ...
          <ul class="cost-ul" data-total="income">
                 ...
                 <ul style="margin: 0px;float: right;">
                <c:if test="${companyId != 68 &&  activityGroup.lockStatus==0 }"><input  class="btn-primary" type="button" value="成本审核"  onclick="saveCheckBox('actualId','1')" /></c:if>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;<c:if test="${activityGroup.lockStatus==0}"><input  class="btn-primary" type="button" value="付款申请"  onclick="payCheckBox('payId','1')" /></c:if>                       
                    </ul> 
       </div>
        </c:if> 
