...
<body>
			...
			<div class="danwkk">
				...
				<table width="670" border="0" cellspacing="0" cellpadding="0" bgcolor="#cccccc" style="float:left;">
					<tr class="text_normal ie7shiying"  style="border-top:none;">
						<td width="60" height="10" align="center" class="text_normal bgcolorccc">销售</td>
						...
						<td width="100" align="center" class="text_normal bgcolorccc">实际应收</td>
			...
			<div class="danwkk">
				...
				<table width="670" border="0" cellspacing="0" cellpadding="0" bgcolor="#cccccc" style="float:left;">
					...
					<%--<c:forEach items="${vo.actualInGrouped}" var="entry">
						...
						<tr>
							<td height="10" colspan="4" align="right" bgcolor="#F5F5F5" class="text_little_title bgcolorccc" style="border-bottom:none;">小计</td>
				...
				<tr height="10" bgcolor="#FFFFFF">
					<td width="685" height="65">
						<div id="remarkTextDiv" style="width:100%;float:left">
							<textarea id="remark" disabled="disabled" style="width:98%; height:40px" class="noborder print_expand"  onkeyup="javascript:setRemarkCount(this)">${vo.remark}</textarea>
						</div>
	...
	<div class="dbaniu">
		<shiro:hasPermission name="settle:lock && settle:unlock">
			<c:if test="${lockStatus == 0 }">
					<input type="button" class="ydbz_s" value="锁定" onclick="javascript:locker('${activityIdOrUUID}',${orderType },'${lockStatus }',this);">
			</c:if>
			<c:if test="${lockStatus == 1 }">
					<input type="button" class="ydbz_s" value="解锁" onclick="javascript:locker('${activityIdOrUUID}',${orderType },'${lockStatus }',this);">
			</c:if>
		</shiro:hasPermission>
		<shiro:hasPermission name="settle:lock && not settle:unlock">
			...
			<c:if test="${lockStatus == 1 }">
					<input type="button" disabled="disabled" class="ydbz_s gray" value="解锁">
			</c:if>
		</shiro:hasPermission>
		<shiro:hasPermission name="settle:unlock && not settle:lock">
			<c:if test="${lockStatus == 0 }">
					<input type="button" class="ydbz_s gray" disabled="disabled" value="锁定">
			</c:if>
			<c:if test="${lockStatus == 1 }">
					<input type="button" class="ydbz_s" value="解锁" onclick="javascript:locker('${activityIdOrUUID}',${orderType },'${lockStatus }',this);">
			</c:if>
	...
	<script type="text/javascript">
		...
		function download(){
		 ...
		 //限制textarea字数，超过100字即截取
	    $("#remark").keyup(function () {
	        $(this).val($(this).val().substr(0, 100));
	        setRemarkCount($(this)[0]);
	    });
