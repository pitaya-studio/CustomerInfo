...
<div class="activitylist_bodyer_right_team_co">
     	 ...
     	 <div class="activitylist_bodyer_right_team_co2 pr wpr20">
         	<label>产品名称：</label><input class="txtPro inputTxt" id="wholeSalerKey" name="wholeSalerKey" value="${travelActivity.acitivityName }"/>
         	<span class="ipt-tips" style="display: block;">仅支持产品名称的搜索</span>
         </div>
                                ...
                                <tbody>
                                    <c:forEach items="${activity.activityGroups}" var="group" varStatus="s2">
                                        <tr>
                                            ...
                                            <td width="8%" class="tr"><span>${group.freePosition }</span>
                                            <td width="8%">
                                              <shiro:hasPermission name="cost:approve:view">
                                                  <c:choose>
                                                     <c:when test="${group.costStatus eq fnc:getState(\"directorAccepted\")}">
                                                         <a target="_blank" href="${ctx}/cost/manager/flow/directorAccepted/${activity.id}/${group.id}?from=${type}">成本审核</a>
                                                     </c:when>
                                                     <c:otherwise>
                                                             <a target="_blank" href="${ctx}/cost/manager/flow/directorApproveCostDetail/${activity.id}/${group.id}?from=${type}">查看</a>&nbsp;&nbsp;（${fnc:getStateDescByNum(group.costStatus)}）
                                                     </c:otherwise>
                                                  </c:choose>
                                              </shiro:hasPermission>
                                            </td>
