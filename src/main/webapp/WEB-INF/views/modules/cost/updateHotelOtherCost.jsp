...
<body>
	<form:form name="frm" id="costForm" modelAttribute="sysdefinedict"
		action="${ctx}/cost/island/updateHotel" target="_parent" method="post">
				...
				<table border="0" width="96%">
					...
					<tr style="height:35px;">
						...
						<td><input type="radio" name="supplyType" id="supplyType"
							value="0" <c:if test="${costRecordHotel.supplyType == 0 }">checked</c:if> onClick="tosupply()" />地接社 &nbsp;&nbsp; <input
							type="radio" name="supplyType" id="supplyType" value="1"
							<c:if test="${costRecordHotel.supplyType == 1 }">checked</c:if> onClick="tosales()" />渠道商</td>
					</tr>
					<tr id="sales" <c:if test="${costRecordHotel.supplyType == 0 }">style="height:45px;display:none"</c:if>>
						<td align="right">渠道商选择：</td>
						<td><span id="secondagent"> <select name="agentId"
								id="agentId" onchange="selectbank(this.value,1)"
								style="width:180px">
									<option value="">===请选择===</option>
									<option value="-1" <c:if test="${costRecordHotel.supplyId == -1 }">selected="selected"</c:if>>非签约渠道
									<c:forEach var="agentinfo" items="${agentinfo }">
										<option value="${agentinfo.id }"
											<c:if test="${costRecordHotel.supplyId==agentinfo.id}">selected="selected"</c:if>>${agentinfo.agentName}
										</option>
									</c:forEach>
							</select> </span></td>
					</tr>

					<tr id="supplyid" <c:if test="${costRecordHotel.supplyType == 1 }">style="display:none"</c:if>>
						...
						<td><select style="width:180px"
							onchange="selectchange(this.value)" id="first" name="first">
								<c:forEach items="${supplytypelist}" var="dict">
									<option value="${dict.value}" <c:if test="${costRecordHotel.supplierType == dict.value }">selected="selected"</c:if> >${dict.label}</option>
								</c:forEach>
						</select></td>
					</tr>

					<tr id="supply" <c:if test="${costRecordHotel.supplyType == 1 }">style="display:none"</c:if>>
						...
						<td><span id="second"><select style="width:180px"
							onchange="selectchange(this.value)" id="supplier" name="supplier">
								<c:forEach items="${supplierTypeList}" var="supplierType">
									<option value="${supplierType.supplierId}" <c:if test="${costRecordHotel.supplyId == dict.supplierId }">selected="selected"</c:if> >${supplierType.supplierName}</option>
								</c:forEach>
						</select>
						</span></td>
					</tr>
...
<!-- 					
					...
					<tr id="mybankname" <c:if test="${bankList == null }">style="display:none"</c:if>>
						...
						<td><input name="bankname" id="bankname" type="text"
							value="${costRecordHotel.bankName}" style="width:180px" /></td>
					</tr>
					...
					<tr id="myaccount" <c:if test="${bankList == null }">style="display:none"</c:if>>
						...
						<td><input name="account" id="account" type="text"
							value="${costRecordHotel.bankAccount}" style="width:180px" /></td>
					</tr> 

					<tr>
						<td align="right">款项名称：</td>
						<td><input type="text" name="itemname" id="itemname"
							maxlength="50" value="${costRecordHotel.name}" style="width:180px" />
						</td>
					</tr>
							...
							id="currencyAfter" name="currencyAfter" onchange="changeprice()">
								<c:forEach items="${curlist}" var="currency">
									<option value="${currency.id}"
										<c:if test="${costRecordHotel.currencyAfter==currency.id}">selected</c:if>>${currency.currencyName}</option>
								</c:forEach>
						</select>&nbsp; 汇率：<input type="text" name="rate" id="rate"
							value="${costRecordHotel.rate}" onkeyup="changerate()"
							style="width:55px" /></td>
					</tr>
					...
					<tr>
						...
						<td><span id="costfinal"><input type="text"
								...
								value="${costRecordHotel.priceAfter}" readonly style="width:180px" />
						</span></td>
					</tr>
					...
					<tr>
						...
						<td><textarea rows="3" cols="20" id="comment" name="comment"
								maxlength="1000" style="width:180px">${costRecordHotel.comment}</textarea>
						</td>
					</tr>
				</table>
			</div>
			<div class="release_next_add">
				...
				<c:if
					test="${ budgetType==2 && ( costRecordHotel.review==4  ||  costRecordHotel.review==0||  costRecordHotel.review==5) }">
				...
				<c:if
					test="${ budgetType==1 &&  ( costRecordHotel.payReview==0 || costRecordHotel.review==4  ||  costRecord.review==0 ||  costRecordHotel.review==5  ||  costRecordHotel.review==5 || ( costRecordHotel.review==2 &&  (costRecordHotel.payReview==4 ||costRecordHotel.payReview==5) )) }">

					<input type="button" class="btn btn-primary"
						onClick="submitForms( ${costRecordHotel.review}, ${costRecordHotel.payReview},${companyId})"
