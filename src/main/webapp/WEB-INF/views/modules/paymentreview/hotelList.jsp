...
function expand(child,obj,srcActivityId){
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/payment/review/hotel/${reviewLevel}");
			$("#searchForm").submit();
	    }
            ...
            <%--<div class="message_box_li">--%>
              <%--<c:choose>--%>
                <%--<c:when test="${userJobId==role.id}">--%>
                      <%--<a  href="${ctx}/payment/review/hotel/${reviewLevel}?userJobId=${role.id}">--%>
...
<!--   <div class="activitylist_bodyer_right_team_co_bgcolor">-->
	 <form:form id="searchForm" modelAttribute="activityHotelGroup" action="${ctx}/payment/review/hotel/${reviewLevel}" method="post" >
		...
		<div class="ydxbd">     	
         ...
         <select name="supplierId">
      	...
      	<div class="activitylist_bodyer_right_team_co2">
				<label>出团日期：</label>
				<input id="groupOpenDate" class="inputTxt dateinput" name="groupOpenDate" value='<fmt:formatDate value="${activityHotelGroup.groupOpenDate }" pattern="yyyy-MM-dd"/>' style="margin-left: -3px; width: 122px;" onfocus="WdatePicker({onpicking:function(dp){var vvv=dp.cal.getNewDateStr();if($dp.$('groupCloseDate').value==''){$dp.$('groupCloseDate').value=vvv;}}})" readonly/> 
				<span style="font-size:12px; font-family:'宋体';"> 至</span>  
				<input id="groupEndDate" class="inputTxt dateinput" name="groupEndDate" value='<fmt:formatDate value="${activityHotelGroup.groupEndDate }" pattern="yyyy-MM-dd"/>' style="width: 122px;" onClick="WdatePicker()" readonly/>
        	</div>
	...
	<table id="contentTable" class="table activitylist_bodyer_table" >
		...
		<tbody>
          ...
          <td class="tc">${activity.groupCode}</td>
              ...
              <a target="_blank" href="${ctx}/payment/review/hotelDetail/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}/${reviewCompanyId}?from=operatorPre">审核</a>               
         </c:if>   
              </td>           
              <td class="p00">            
          <a target="_blank" href="${ctx}/payment/review/hotelRead/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}?from=operatorPre">查看</a>&nbsp;
