...
function denyCost(dom,id,reviewLevel){
	var items_val = id +"," + reviewLevel + "," + ${typeId};
	$.ajax({
		type: "POST",
...
function passCost(dom,id,reviewLevel){
	var items_val = id +"," + reviewLevel + "," + ${reviewCompanyId} + "," + ${typeId};
	...
	$.ajax({
		...
		dataType:"json",
		async:false,
		data:{items:items_val,comment:""},//单个付款审核备注为空
		success:function (data){
			if(data.flag){
...
function saveCheckBox(id){
	...
	$("input[name='"+id+"']").each(function(){ 
		if ($(this).attr("checked") != null && $(this).attr("checked")=="checked"){
			var val = $(this).attr('value') + ',' + ${reviewCompanyId} + ',' + ${typeId}
			if(tmp == ''){
				tmp = val;
			}else{
				tmp += "&&" + val;
	...
	$.ajax({
		type: "POST",
		url: "${ctx}/payment/review/batchPass",//saveCostList
		cache:false,
		dataType:"json",
		async:false,
