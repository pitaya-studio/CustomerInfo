     ...
     var submit = function (v, h, f) {            
            ...
            if(remarkLength > 200){
                top.$.jBox.tip('项目备注不能大于200字','error' ,{ focusId: 'comment' }); return false;
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/payment/review/denyvisacost",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
                            remark : remark,id: id},
                        success: function(array){
                            window.location.href = "${ctx}/payment/review/visaList/${reviewLevel}";
                            //window.location.reload();                          
                        },
                        error : function(e){
                            top.$.jBox.tip('请求失败。'+e,'error' ,{ focusId: 'price' });
...
function passVisaCost(dom,id,review){
  ...
  $.ajax({
                     type: "POST",                      
                     url: "${ctx}/payment/review/passvisacost",                      
                     cache:false,
                     dataType:"json",
                     async:false,
