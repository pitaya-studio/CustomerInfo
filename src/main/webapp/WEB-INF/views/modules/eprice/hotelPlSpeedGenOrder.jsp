...
$(function(){
	...
	$("#orderCompany").comboboxInquiry().on("comboboxinquiryselect",function(){
...
$(function(){
	$("#baseInfo").validate({
		...
		submitHandler: function(form){
		    ...
		    if(vselno=='1'){
			    ...
			    if(typeForm=='1'){
				    ...
				    $(".groupPrices_tr:visible").each(function(){
					 ...
					 if(!groupPriceVali) {
					 	top.$.jBox.tip("最少输入一个游客,费用为0时,不允许报名!");
					 	//将文本框还原
						$(".groupPrices_tr").each(function(){
			...
			var url = "${ctx}/hotelPlSpeedGenOrder/save";
			/**
					...
					if (v == 'ok') {
						$.post(url,$("#baseInfo").serialize(),function(data){
							if(data.message=="1"){
								$.jBox.tip("保存成功!");
								if(typeFromTravel=='1'){
									window.location = "${ctx}/hotelQuote/hotelQuoteList";
								}else{
									window.location = "${ctx}/autoQuotedPrice/quotedPriceForm";
								}
...
<div class="ydbzbox fs">
			...
			<div class="mod_information_dzhan_d error_add1" id="oneStepContent"
				...
				<div class="mod_information_d2" style="min-width:334px;">
					<label> <span class="xing">*</span>团号选择：
						...
						<%-- <option value="2" <c:if test="${selNo=='2'}">selected="selected"</c:if> >已有团号1</option> --%>
					</select>
				...
				<span class="group_orders_number">
					<div class="mod_information_d2 ">
						...
						<span class=" tdred" style="display:none;">此团号已存在！</span>
					</div>
					<div class="mod_information_d2 ">
						<label>下单人：</label>  
						<form:select path="orderMan" id="groupSelect" cssStyle="width:150px;">
					...
					<div class="mod_information_d2 ">
						<label><span class="xing">*</span>团号：</label>
							<%-- <form:select path="activityHotelGroup.groupCode" onchange="getActivityAndGroupInfo(this)">
					...
					<div class="mod_information_d2 ">
						<label>下单人：</label>
						<%-- <form:select path="orderMan" id="groupSelect">
								<option value="0">请选择</option>
								<c:forEach items="${userList }" var="entity">
									<option value="${entity.id }" <c:if test="${entity.id == user.id }">selected="selected"</c:if>>${entity.name }</option>
								</c:forEach>
			...
			<div class="mod_information_dzhan_d error_add1" id="oneStepContent"
				...
				<div class="mod_information_d2" style="min-width:334px;">
					<label> <span class="xing">*</span>国家： </label>
						...
						<c:if test="${typeFrom==2 }">
						<trekiz:autoId2Name4Table tableName="sys_geography" sourceColumnName="uuid" srcColumnName="name_cn" value="${speedOrderInput.country}" />
						<trekiz:suggest name="countrynouse" style="display:none;" defaultValue="${speedOrderInput.country}" displayValue="${countryName}" ajaxUrl="${ctx}/geography/getAllConListAjax" />
						<input type="hidden" name="country" value="${speedOrderInput.country}" />
						</c:if>
				</div>
				<div class="kongx"></div>
				<div class="mod_information_d2 " style="min-width:334px;">
					...
					<c:if test="${typeFrom==2 }">
						<trekiz:autoId2Name4Table tableName="island" sourceColumnName="uuid" srcColumnName="island_name" value="${speedOrderInput.island}" />
						<input type="hidden" name="island" value="${speedOrderInput.island}" />
					</c:if>
					
					</div>
				<div class="mod_information_d2 ellipsis">
					...
					<c:if test="${typeFrom==2 }">
						<trekiz:autoId2Name4Table tableName="hotel" sourceColumnName="uuid" srcColumnName="name_cn" value="${speedOrderInput.hotel}" />
						<input type="hidden" name="hotel" value="${speedOrderInput.hotel}" />
					</c:if>
				</div>
				<div class="mod_information_d2 ">
					<c:if test="${typeFrom==1 }">
					<label class="title-md tr">酒店集团：</label><span id="hotel_hotelGroup"></span>
					</c:if>
					<c:if test="${typeFrom==2 }">
						<label class="title-md tr">酒店集团：</label>${hotelGroup }
					</c:if>
				</div>
				<div class="kongx"></div>
				<div class="mod_information_d2 " style="width:100%">
					...
					<c:if test="${typeFrom==2 }">
						<label> 交通： </label>
						<c:forEach items="${islandWayList}" var="var">
								<trekiz:defineDict name="island_way" type="islands_way" defaultValue="${var.islandWay}" readonly="true" style="width:20px!important; margin:0px !important;"/>
								<input type="hidden" name="islandWay" value="${var.islandWay}" />
						</c:forEach>
					</c:if>
				</div>
				<div class="kongx"></div>
				<!-- <div class="mod_information_d2 ">
					<label> 预报名间数： </label> <input class="valid" type="text" data-type="number" id="preOrderNum_yes" name="forecaseReportNum"/>
				</div> -->
				<div class="mod_information_d2 " style="min-width:334px;">
				...
				<div class="house_house_sel">
							...
							<c:when test="${fn:length(groupRoomList)>0 }">
								<c:forEach items="${groupRoomList }" var="room">
									<p class="margin_padding_none">
										...
										<input type="hidden" name="roomtype" value="${room.hotelRoomUuid}" />
										<label class="price_sale_house_label">*</label>${room.nights==null?1:room.nights }晚
										<input type="hidden" name="night" value="${room.nights==null?1:room.nights}" />
									</p>
								</c:forEach>
							</c:when>
							<c:otherwise>
					...
					<div class="mod_information_d2 ">
						<c:choose>
							<c:when test="${fn:length(groupRoomList)>0 }">
								<c:forEach items="${groupMealList }" var="meal">
									<p class="margin_padding_none">
								    <label> 餐型： </label>
								    <c:choose>
								    	<c:when test="${meal.hotelMealRiseUuid!=null && meal.hotelMealRiseUuid!='' }">
								    		<trekiz:autoId2Name4Class classzName="HotelMeal" sourceProName="uuid" srcProName="mealName" value="${meal.hotelMealRiseUuid }" />
											<input type="hidden" name="foodtype" value="${meal.hotelMealRiseUuid}" />
								    	</c:when>
								    	<c:otherwise>
								    		<trekiz:autoId2Name4Class classzName="HotelMeal" sourceProName="uuid" srcProName="mealName" value="${meal.hotelMealUuid }" />
								    		<input type="hidden" name="foodtype" value="${meal.hotelMealUuid}" />
								    	</c:otherwise>
								    </c:choose>
								    </p>
								</c:forEach>
		...
		<div class="mod_information_dzhan no_edit_msg" style="display:none;">
			<div class="mod_information_dzhan_d error_add1" id="oneStepContent"
				...
				<div class="payment_information">
					<div class="mod_information_d2 ">
						<label> 余位： </label>
						<input id="remNumber_no" value="${activityHotelGroup.remNumber }"/>
					</div>
					<div class="mod_information_d2 ">
						<label> 预报名间数： </label>
						<input id="preOrderNum_no" data-type="number"  name="forecaseReportNum"/>
					</div>
			...
			<div class="mod_information_dzhan" id="secondStepDiv">
				<div class="mod_information_dzhan_d error_add1" id="oneStepContent" style=" overflow:hidden;">
					...
					<div class="mod_information_d2" id="signChannel">
						...
						<select name="orderCompany" id="orderCompany" onchange="loadAgentInfo();">
	...
	<div id="costTable">
		<table class="table activitylist_bodyer_table_new contentTable_preventive">
			...
			<tbody id="no_edit_tbody"></tbody>
            <tbody id="yes_edit_tbody">
            		<c:if test="${typeFromTravel==1}">
            		<c:forEach items="${travellerList }" var="groupPrice">
            			<c:if test="${groupPrice.priceType==1 }">
						<tr id="${groupPrice.typeUuid}" class="groupPrices_tr">
						<td class="tc">
						    <span name="OrderTravelerTypeName">
						    <trekiz:autoId2Name4Table tableName='traveler_type' sourceColumnName='uuid' srcColumnName='name' value='${groupPrice.typeUuid}'/>
						    </span>
							<!-- 费用调整 -->
							<input type="hidden" name="hotelOrderPrice_activityHotelGroupPriceUuid" value="${groupPrice.typeUuid}" />
							<input type="hidden" name="hotelOrderPrice_remark" value="_"/>
							<input type="hidden" name="hotelOrderPrice_priceType" value="1" />
							<input type="hidden" name="hotelOrderPrice_priceName" value="_" />
						    <input type="hidden" name="hotelOrderPrice_travelerType" value="${groupPrice.typeUuid}" />
						</td>
						<td class="tc">
							<p>
								<trekiz:autoId2Name4Table tableName="currency" sourceColumnName="currency_id" srcColumnName="currency_mark" value="${currencyRMB }"/>
								<input data-currency="<trekiz:autoId2Name4Table tableName='currency' sourceColumnName='currency_id' srcColumnName='currency_mark' value='${currencyRMB }'/>"  
								       style="ime-mode: disabled;" data-type="float" class="price_sale_house_w93" type="text" name="hotelOrderPrice_price" value="${groupPrice.price}" readonly="true"/>
								<input type="hidden" name="hotelOrderPrice_currencyId" value="${currencyRMB }" />
							</p>
						</td>
            		...
            		<c:if test="${typeFrom==2}">
            		<c:forEach items="${guestPricelist }" var="groupPrice">
            		    <c:if test="${groupPrice.isThirdPerson==0 }">
            			<tr id="${groupPrice.travelerType}" class="groupPrices_tr">
						<td class="tc">
						    <span name="OrderTravelerTypeName">${groupPrice.travelerTypeText}</span>
							<!-- 费用调整 -->
							<input type="hidden" name="hotelOrderPrice_activityHotelGroupPriceUuid" value="${groupPrice.travelerType}" />
							<input type="hidden" name="hotelOrderPrice_remark" value="_"/>
							<input type="hidden" name="hotelOrderPrice_priceType" value="1" />
							<input type="hidden" name="hotelOrderPrice_priceName" value="_" />
						    <input type="hidden" name="hotelOrderPrice_travelerType" value="${groupPrice.travelerType}" />
						</td>
						<td class="tc">
							<p>
								<trekiz:autoId2Name4Table tableName="currency" sourceColumnName="currency_id" srcColumnName="currency_mark" value="${groupPrice.currencyId }"/>
								<input data-currency="<trekiz:autoId2Name4Table tableName='currency' sourceColumnName='currency_id' srcColumnName='currency_mark' value='${groupPrice.currencyId }'/>"  
								       style="ime-mode: disabled;" data-type="float" class="price_sale_house_w93" type="text" name="hotelOrderPrice_price" value="${groupPrice.amount}" readonly="true"/>
								<input type="hidden" name="hotelOrderPrice_currencyId" value="${groupPrice.currencyId }" />
							</p>
						</td>
						<td class="tc"><input type="text" data-type="number" class="price_sale_house_w100" name="hotelOrderPrice_num"/></td>
				...
				<table class="contentTable_preventive table_padings" id="add_other_charges_table">
					<tr>
						...
						<td width="280" class="tl">
							...
							<input type="text" class="price_sale_house_w93" data-type="float"  name="hotelOrderPrice_price" id="input5" />
						</td>
					...
					<tr class="calc_amount_tr">
						...
						<td class="tl">
							...
							<input type="text" class="price_sale_house_w93 price" data-type="float" name="hotelOrderPrice_price" id="input6" maxlength="" />
						</td>
						<td class="tr">备注：</td>
						<td colspan="3">
							<span class="tl"> 
					...
					<tr>
						...
						<td class="tl">
							...
							<input type="text" name="hotelOrderPrice_price"  data-type="float" class="price_sale_house_w93" id="input6" />
						</td>
						<td class="tr">备注：</td>
						<td colspan="3">
							<span class="tl">
				...
				<td class="tc">
					<select name="hotelTraveler_personType" class="w80 display_inline" onchange="setTitle(this);">
					    <c:if test="${typeFrom=='2'}">
							<c:forEach items="${guestPricelist }" var="groupPrice">
							    <c:if test="${groupPrice.isThirdPerson=='0' }">
								<option value="${groupPrice.travelerType}">${groupPrice.travelerTypeText}</option>
								</c:if>
							</c:forEach>
					    </c:if>
					    <c:if test="${typeFromTravel=='1'}">
							<c:forEach items="${travellerList }" var="groupPrice">
							    <c:if test="${groupPrice.priceType==1 }">
								<option value="${groupPrice.typeUuid}"><trekiz:autoId2Name4Table tableName='traveler_type' sourceColumnName='uuid' srcColumnName='name' value='${groupPrice.typeUuid}'/></option>
								</c:if>
							</c:forEach>
					    </c:if>
					</select>
...
<script type="text/javascript">
var showType = '${typeFrom}';
var typeFromTravel = '${typeFromTravel}';
var orderInfo;
if(typeFromTravel=='1'){
	orderInfo = {
		    /* traveller: [
	    	...
	    	traveller: [
	            	<c:forEach items="${travellerList }" var="groupPrice" varStatus="status">
	            		<c:if test="${groupPrice.priceType==1 }">
	                	{ type: "${groupPrice.typeUuid}", 
	                	  name: "<trekiz:autoId2Name4Table tableName='traveler_type' sourceColumnName='uuid' srcColumnName='name' value='${groupPrice.typeUuid}'/>",
	                	  cost: "${groupPrice.price}"},
	                	</c:if>
	            	</c:forEach>
	            ],
		    // 订单总额
	...
	orderInfo = {
    	...
    	traveller: [
                	<c:forEach items="${guestPricelist }" var="groupPrice" varStatus="status">
                		<c:if test="${groupPrice.isThirdPerson==0  }">
                    	{ type: "${groupPrice.travelerType}", name: "${groupPrice.travelerTypeText}",
                    		cost: {"${groupPrice.currencyText}" : "${groupPrice.amount}"}},
                    	</c:if>	
                	</c:forEach>
            ],
	    // 订单总额
...
function getTradePrice() {
    ...
    if ($("#costTable tbody:first").is(":visible")) {
        ...
        for (var l = orderInfo.traveller.length; l--;) {
            ...
            if (cost) {
                //var currency = $p.find("select option:selected").attr("data-currency");
                var currency = $p.find("input").attr("data-currency");
                traveller.cost[currency] = (traveller.cost[currency] || 0) + cost;
            }
        }
    }
