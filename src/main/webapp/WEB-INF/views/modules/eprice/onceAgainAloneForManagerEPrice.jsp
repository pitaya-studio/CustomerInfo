...
<!-- 20150902 新增 再次询价时，由计调主管选择计调 -->
	...
	<script src="${ctxStatic}/modules/eprice/onceAgainAloneForManagerEPrice.js" type="text/javascript"></script>
	              ...
	              <p class="seach_r">
	                <span class="xing">*</span>接待社计调主管：
	              	<span class="seach_check">
		               <select name="formanager" style="width:160px">
		                	<option>请搜索计调主管</option>
							<c:forEach items="${managerList }" var="ml">
								<option value="${ml.id }" <c:if test="${aopManager.opManagerId eq ml.id }">selected</c:if>>${ml.name}</option>
							</c:forEach>
						</select>
	                </span>
	              </p>
			              ...
			              <p class="seach_r">
			              	 ...
			              	 <span class="seach_check">
				                <select name="formanager" style="width:160px">
		                			<option>请搜索计调主管</option>
									<c:forEach items="${managerList }" var="ml">
										<option value="${ml.id }" <c:if test="${topManager.opManagerId eq ml.id }">selected</c:if>>${ml.name}</option>
									</c:forEach>
								</select>			
			                </span>
			              </p>
