...
function loadCurrency(){
   $.ajax({
		  type: "GET",
		  url: "${ctx}/orderCommon/manage/currencyJsonIsland",
		  dataType: "json",
		  async: false,
	...
	var checkFlag = false;
	   ...
	   if(flag){
		   if (submitNum == 1) {
			   ...
			   $.ajax({
	                type: "POST",
	                url: "${ctx}/orderCommon/manage/saveIslandGroupRefundInfo",
	                data: {
	                    ...
	                    refund :refund.join(','),
	                    remark :remark.join(','),
	                    productType:12,flowType:1,orderId:"${orderId}",orderUuid:"${orderUuid}"
	                    
	                },
	                success: function(msg){
		                if(msg.sign == 1){
				            	window.location.href ="${ctx}/orderCommon/manage/viewIslandGroupRefund?productType=${productType}&orderId=${orderId}&orderUuid=${orderUuid}&flowType=1";
				            }else{
				            	submitNum--;
         ...
         <div class="ydbz_tit pl20"><span class="fl">退款</span></div>
                    <div class="jbox_batch_change_price fl" style="width:100%; height:30px; line-height:30px;margin:20px 0px 5px 0px; padding-top:5px; background-color:#FEF9E6;">
                        <ul class="ydbz_info">
                            ...
                            <li style="width:190px;"><span>退款金额：</span><input id="batchMoney" class="w80" type="text" data-type="float" maxlength="9"/></li>
                    ...
                    <table id="contentTable" class="table activitylist_bodyer_table activitylist_bodyer_table_new">
                        ...
                        <c:forEach items="${travelerList}" var="travelerRebates" varStatus="s">
		  					...
		  					<td class="table_borderLeftN"><input type="checkbox" data-group="refund" name="checkSign" /></td>
								...
								<td class="tc">
									${fns:getDictLabel(travelerRebates.spaceLevel,"spaceGrade_Type" , "无")}
								</td>
								...
								<td class="tr">${fns:getRefundPayMoneyTravelByOrderType(orderUuid,travelerRebates.uuid,12)}</td>
		...
		<tbody>
			...
			<c:forEach items="${bAList}" var="rebates">
				<tr>
					...
					<td class="tc">
						${fns:getDictLabel(rebates.spaceLevel,"spaceGrade_Type" , "无")}
					</td>
