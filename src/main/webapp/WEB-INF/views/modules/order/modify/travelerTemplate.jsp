  ...
  <form name="travelerForm" class="form-search">
    ...
    <div class="tourist">
        <div class="tourist-t">
            ...
            <div class="tourist-t-off" <c:if test="${priceType eq 2}">style="display: none;"</c:if>>
               <span class="fr">
                	<b style="font-size:18px"><c:if test="${priceType eq 2}">成人价</c:if><c:if test="${priceType eq 1}">直客价</c:if><c:if test="${priceType ne 1 and priceType ne 2 }">同行价</c:if>：</b><span name="jsPrice" class="ydFont2"></span> 
            ...
            <div class="tourist-t-on">
                <c:choose>
					<c:when test="${productorder.orderStatus == '10'}">
						<label><input type="radio" class="traveler" name="personType" value="1" />1/2<c:if test="${priceType eq 1}">直客</c:if><c:if test="${priceType ne 1 }">同行</c:if></label>
                		<label><input type="radio" class="traveler" name="personType" value="2" />3/4<c:if test="${priceType eq 1}">直客</c:if><c:if test="${priceType ne 1 }">同行</c:if></label>
                	</c:when>
					<c:otherwise>
                        ...
                        <span>
                           <select onchange="setIntermodal($(this))" name="intermodalStrategy">
                                <c:forEach items="${intermodalList}" var="intermodal">
                                   <option value="${intermodal.price}" intermodalId="${intermodal.id}" priceCurrencyId="${intermodal.priceCurrency.id}" priceCurrency="${intermodal.priceCurrency.currencyName}">${intermodal.groupPart}</option>
                                </c:forEach>
                           </select>
          ...
          <div class="tourist-con">
                                ...
                                <ul flag="messageDiv" class="zbqinfo">
                                    ...
                                    <div class="ydbz_scleft">
                                        <table class="table-visa">
                                            ...
                                            <c:forEach items="${fns:obtainTravelerAllVisaList(travelerone.traveler.orderId,travelerone.traveler.id)}" var="tvisa">
                                            <tbody id="qztemplate">
                                                <tr class="${tvisa.countryName}" name="visainfo">
                                                    ...
                                                    <td>
                                                        <input type="text" name="contractDate" value="<fmt:formatDate value="${tvisa.contractDate}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'dd/MM/yyyy'})" class="inputTxt dateinput">
                                                    </td>
            ...
            <div class="tourist-left">
                ...
                <ul class="tourist-info1 clearfix" flag="messageDiv">
                    <li>
                        <label class="ydLable"><span class="xing">*</span>姓名：</label>
                        <input type="text" maxlength="25" name="travelerName" class="traveler required" onkeyup="this.value=this.value.replaceSpecialChars()"  onafterpaste="this.value=this.value.replaceSpecialChars()">
                    </li>
                    <li>
                        <label class="ydLable">英文／拼音：</label>
                    ...
                    <li>
                        ...
                        <select name="nationality" class="selCountry">
                            ...
                            <c:if test="${country.id==461}">selected="selected"</c:if>>${country.countryName_cn}</option>
                            </c:forEach>
                        </select>
                    </li>
                     <li>
                    ...
                    <li>
                     ...
                     <li>
                        <label class="ydLable">护照号：</label>
                        <input type="text" name="passportCode" class="traveler" maxlength="30" onkeyup="this.value=this.value.replace(/[^\d\+\-a-zA-Z]/g,'')"  onafterpaste="this.value=this.value.replace(/[^\d\+\-a-zA-Z]/g,'')" >
                    ...
                    <li>
                        ...
                        <input type="text" name="idCard" style="width:128px;padding-left:1px;padding-right:1px;" class="traveler" maxlength="30" onkeyup="this.value=this.value.replace(/[^\d\+\-a-zA-Z]/g,'')"  onafterpaste="this.value=this.value.replace(/[^\d\+\-a-zA-Z]/g,'')" onblur="getBirthday(this);">
                    </li>
                    <li>
                ...
                <ul flag="messageDiv" class="zbqinfo">
                    <div class="ydbz_tit ydbz_tit_child">自备签 
                        <p class="tourist-ckb">
                            ...
                            <c:forEach items="${targetForeignCountry}" var="tfc" varStatus="s">
                                <input type="checkbox" class="traveler" value="${tfc.applyCountryName}" name="zibeiqian"><label class="ckb-txt">自备${tfc.applyCountryName}签证</label>
                            </c:forEach>
                        </p>
                    </div>
	                ...
	                <ul class="ydbz_2uploadfile ydbz_scleft certificate">
	                    <li class="seach25 seach33"><p>护照首页：</p><input name="passportfile" type="text" readonly="readonly"><input type="button" name="passport" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','passportfile',this,'true');"/><span class="fileLogo"></span></li>
	                    <li class="seach25 seach33"><p>身份证正面：</p><input name="idcardfrontfile" type="text"  readonly="readonly"><input type="button" name="idcardfront" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','idcardfrontfile',this,'true');"><span class="fileLogo"></span></li>
	                    <li class="seach25 seach33"><p>申请表格：</p><input name="entryformfile" type="text"  readonly="readonly"><input type="button" name="entryform" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','entryformfile',this,'true');"><span class="fileLogo"></span></li>
						<p class="kong"></p>
	                    <li class="seach25 seach33"><p>房产证：</p><input name="housefile" type="text"  readonly="readonly"><input type="button" name="house" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','housefile',this,'true');"><span class="fileLogo"></span></li>
	                    <li class="seach25 seach33"><p>电子照片：</p><input name="photofile" type="text"  readonly="readonly"><input type="button" name="photo" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','photofile',this,'true');"><span class="fileLogo"></span></li>
	                    <li class="seach25 seach33"><p>身份证反面：</p><input name="idcardbackfile" type="text"  readonly="readonly"><input type="button" name="idcardback" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','idcardbackfile',this,'true');"><span class="fileLogo"></span></li>
	                    <p class="kong"></p>
	                    <li class="seach25 seach33"><p>户口本：</p><input name="residencefile" type="text"  readonly="readonly"><input type="button" name="residence" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','residencefile',this,'true');"><span class="fileLogo"></span></li>
	                    <li class="seach25 seach33"><p>其　它：</p><input name="otherfile" type="text"  readonly="readonly"><input type="button" name="other" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','otherfile',this,'true');"><span class="fileLogo"></span></li> 
	                    <p class="kong"></p>
            ...
            <c:choose>
	            <c:when test="${isForYouJia eq true and isLoose eq true and isSettlement eq true}">
                    ...
                    <div class="tourist-right">
                        ...
                        <li>
                        	<label>单人房差：</label>${singleDiffCurrencyMark}<span>0</span><input type="hidden" name="singleDiff" class="traveler" value="0" >/间 x<input type="text" class="ipt4" name="sumNight" maxlength="3" onkeyup="changeSumNight(this)"  onafterpaste="this.value=this.value.replace(/[^\d\+\-]/g,'')">晚
                        </li>
                    </ul>
                            </div>
                            	...
                            	<!-- 265需求，针对鼎鸿假，将返佣字段改为宣传费 -->
                            		<c:choose>
						         	<c:when test="${fns:getUser().company.uuid eq '049984365af44db592d1cd529f3008c3'}"> 
										<label class="ydColor1 ydLable2">预计个人宣传费：</label>
									</c:when>
						            <c:otherwise>
						                 <label class="ydColor1 ydLable2">预计个人返佣：</label>
						             </c:otherwise>
						         </c:choose>   
                                    ...
                                    <span class="discount">
                                        <span class="adultDiscount" style="display: none">
                                            <span class="activityDiscountCurrencyMark">
                                                ${fns:getCurrencyNameOrFlag(discountMap.adult.currencyId, '0')}
                                            </span>
                                            <input type="hidden" name="activityDiscountCurrencyId" value="${discountMap.adult.currencyId}"/>
                                            <span class="activityDiscountAmount">
                                                ${discountMap.adult.amount}
                                            </span>
                                        </span>
                                        <span class="childDiscount" style="display: none">
                                            <span class="activityDiscountCurrencyMark">
                                                ${fns:getCurrencyNameOrFlag(discountMap.child.currencyId, '0')}
                                            </span>
                                            <input type="hidden" name="activityDiscountCurrencyId" value="${discountMap.child.currencyId}"/>
                                            <span class="activityDiscountAmount">
                                                ${discountMap.child.amount}
                                            </span>
                                        </span>
                                        <span class="specialDiscount" style="display: none">
                                            <span class="activityDiscountCurrencyMark">
                                                ${fns:getCurrencyNameOrFlag(discountMap.special.currencyId, '0')}
                                            </span>
                                            <input type="hidden" name="activityDiscountCurrencyId" value="${discountMap.special.currencyId}"/>
                                            <span class="activityDiscountAmount">
                                                ${discountMap.special.amount}
                                            </span>
                                        </span>
                                    </span>
                                    <input type="hidden" name="orgDiscountPrice" value="">
                            	</div>
                                <div class="traveler-rebatesDiv-discount-limit">
                                    <label class="ydLable2 ydColor1">优惠金额：</label>
                                    <input type="text" name="discount" class="discount-limit" <c:if test='${isDiscountApplied}'>disabled="disabled"</c:if> maxlength="9" onblur="changeInnerDiscount(this)" onafterpaste="changeInnerDiscount(this)">
                                    <input type="hidden" name="disCurrencyMark" >
                                    <input type="hidden" name="disCurrencyName" >
                                    <input type="hidden" name="disCurrencyId" >
                                    <input type="hidden" name="reviewedDiscount" >
                                    <input type="hidden" name="fixedDiscount" value="0">
                                    <input type="hidden" name="org_fixedDiscount" >
                                </div>
                            </div>
							<div class="yd-line"></div>
                            ...
                            <div class="clearfix">
                                <div class="traveler-rebatesDiv-discount-limit">
                                    <p>
                                    	<label for="">同行价：</label>
                                    	<em><span name="settlementPrice"></span></em>
                                    </p>
                                    <p class="ydLable2">
                                        <label>单房差小计：</label>${singleDiffCurrencyMark}
                                        <span name="singleDiffSubtotal">0</span>
                                    </p>
                                    <p class="ydLable2">
                                    	<label for="">同行结算价：</label>
                                    	<em><span name="settlementClearPrice"></span></em>
                                    	<em>（优惠减<span name="totalDiscount"></span>）</em>
                                    </p>
                                    <p class="ydLable2">
                                    	<label for="">其他费用总计：</label>
                                    	<em><span name="totalOtherCostPrice"></span></em>
                                    </p>
                                    <input type="hidden" name="srcPrice" class="traveler">
                                    <input type="hidden" name="srcPriceCurrency" class="traveler" >
                                    <input type="hidden" name="srcPriceCurrencyMark" class="traveler" >
                                    <input type="hidden" name="srcPriceCurrencyName" class="traveler" >
                                    <input type="hidden" name="jsPrice" class="traveler" >
                                    <input type="hidden" name="jsPriceJson" class="traveler" >
                                </div>
                            </div>
                            <div class="yd-line"></div>
                            <div class="yd-line"></div>
                            ...
                            <div class="yd-total clearfix">
                                <div class="traveler-rebatesDiv-discount-limit">
                                    <p class="ydLable2 total">
                                        ...
                                        <input type="hidden" name="travelerClearPriceJson" class="traveler" >
                                    </p>
                                </div>
                            </div>
                        </div>
		            ...
		            <div class="tourist-right">
		                <div class="bj-info">
		                    ...
		                    <div class="clearfix">
		                        <ul class="tourist-info2">
		                            ...
		                            <li>
		                            	<label class="ydLable2">单房差小计：</label>${singleDiffCurrencyMark}<span class="ydFont1"></span>
		                            </li>
		                        </ul>
		                    </div>
		                    ...
		                    <div class="clearfix">
								<div class="traveler-rebatesDiv">
									...
									<select name="rebatesCurrency" >
			               			...
			               			<input type="text" class="ipt-rebates"  name="rebatesMoney"  maxlength="9" onafterpaste="checkRebatesValue(this, 50)" onkeyup="checkRebatesValue(this, 50)"/>
								</div>
							</div>
		                ...
		                <div class="yd-total clearfix">
		                  <div class="fr" style="white-space:nowrap;<c:if test="${priceType eq 2}">display: none</c:if>">
		                    <label class="ydLable2"><c:if test="${priceType eq 2}">成人价</c:if><c:if test="${priceType eq 1}">直客价</c:if><c:if test="${priceType ne 1 and priceType ne 2 }">同行价</c:if>：</label><span
                                  name="innerJsPrice" class="ydFont2"></span>
		                    <input type="hidden" name="srcPrice" class="traveler">
		                    <input type="hidden" name="srcPriceCurrency" class="traveler" >
		                    <input type="hidden" name="jsPrice" class="traveler">
		                  </div>
		                ...
		                <c:if test="${productorder.orderStatus ne 10 }">
			                ...
			                <div>
			                    <span>价格备注：</span>
			                    <textarea rows="4" cols="10" style="width: 160px" name="priceRemark" onafterpaste="checkMaxLength(this)" onKeyUp="checkMaxLength(this)"></textarea>
			                </div>
		                </c:if>
		                </shiro:hasPermission>
		                <!--299-end-->
        ...
        <!--保存按钮开始-->
        <div class="rightBtn">
           <input type="button" name="saveBtn" class="btn btn-primary" onclick="saveTraveler(this,this.form,${productorder.orderStatus})" value="保存">
