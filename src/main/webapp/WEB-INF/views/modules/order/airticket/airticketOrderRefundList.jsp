...
function createNewRecord(orderId){
	document.location= g_context + "/order/refund/createRefund?orderId=" + orderId;
}
...
function cancelRefund(reviewId){
    ...
    $.ajax({
	   type: "POST",
		  url: g_context + "/order/refund/cancelRefund",
		  dataType: "json",
		  ...
		  success:function(msg){
			  if(msg){
				  ...
				  document.location = g_context + "/order/refund/viewList?orderId=" + orderId;
			  }
		  }
   });
...
<!-- <div class="ydbz_tit orderdetails_titpr">退款记录<a class="ydbz_x" id="newRefund">申请退款</a> -->
</div>
     ...
     <c:forEach items="${viewMap }" var="map">
         <c:forEach items="${map.value}" varStatus="s1" var="bean">
	      <tr>
			...
			<td>${bean.remark }</td>
			<td class="${bean.statusClassDesc }">
			<!--   ${bean.statusDesc }
			   <c:if test="${bean.statusDesc eq null}">
			   	已取消
			   </c:if>  -->${fns:getNextReview(bean.reviewId) }
            </td>
			<td class="p0">
				<dl class="handle">
					...
					<dd class="">
						<p>
							...
							<a href="${ctx}/order/refund/refundPrice/${orderId}/${bean.reviewId}">退款详情</a>
						    ...
						    <c:if test="${bean.status == 1}">
						        <a href="javascript:void(0)" onclick="cancelRefund('${bean.reviewId}')">取消申请</a>	
						    </c:if>									
						</p>
					</dd>
				</dl>						
