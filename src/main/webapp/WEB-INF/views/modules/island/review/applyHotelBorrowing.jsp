...
//			}		 			
	   ...
	   if(flag){
		   if (submitNum == 1) {
			   ...
			   $.ajax({
	                type: "POST",
	                url: "${ctx}/orderCommon/manage/saveHotelBorrowing",
	                data: {
	                ...
	                success: function(msg){
		                if(msg.sign == 1){
				            	window.location.href ="${ctx}/island/review/viewHotelBorrowingList?productType=11&orderId=${orderId}&orderUuid=${orderUuid}&flowType=19";
				            }else{
				            	submit_times = 0;
				            	submitNum--;
                    ...
                    <table id="contentTable" class="table activitylist_bodyer_table activitylist_bodyer_table_new">
                        ...
                        <c:forEach items="${travelerList}" var="travelerRebates" varStatus="s">
		  					...
		  					<td class="table_borderLeftN"><input type="checkbox" data-group="refund" name="checkSign" /></td>
								...
								<td class="tr">${fns:getBorrowPayMoneyTravelByOrderType(productOrder.uuid,travelerRebates.uuid,11)}</td>
