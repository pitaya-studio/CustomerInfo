		...
		<dd class="">
			<p>
				...
				<shiro:hasPermission name="islandOrder:operation:view">
					<a href="${ctx}/islandOrder/islandOrderDetail/${orders.orderUuid}"  target="_blank">详情</a>
				</shiro:hasPermission>
				
				<!-- 如果团期已锁定或订单已锁定，则不能有已下操作 -->
				<c:if test="${orders.settleLockStatus != 1 &&  orders.lockStatus != 1}">
					<!-- 修改 -->
					<shiro:hasPermission name="islandOrder:operation:edit">
						<!-- 如果订单已确认或已取消，则不能修改订单 -->
						<c:if test="${orders.orderStatus == 1}">
							<a href="${ctx}/islandOrder/toIslandOrderUpdatePage?orderUuid=${orders.orderUuid}&isTransfer=false"  target="_blank">修改</a>
							<a href="${ctx}/islandOrder/toIslandOrderUpdatePage?orderUuid=${orders.orderUuid}&isTransfer=true"  target="_blank">转报名</a>
						</c:if>
					</shiro:hasPermission>
					
					<c:if test="${orders.orderStatus == 2 }">
						<!-- 改价 -->
						<shiro:hasPermission name="islandOrder:operation:changeprice">
							<%-- <a href="javascript:void(0)" onClick="viewIslandChangePriceList('${orders.id}','${orders.orderUuid}')"> 改价</a> --%>
							<a href="javascript:void(0)" onClick="viewIslandChangePriceListNew('${orders.id}','${orders.orderUuid}')"> 改价</a>
						</shiro:hasPermission>
						
						<!-- 返佣 -->
						<shiro:hasPermission name="islandOrder:operation:returnCommission">
							<a href="javascript:rebatesInfo.rebatesOrder('${orders.id}','12','${orders.orderUuid }');">返佣</a>
						</shiro:hasPermission>
						
						<!--申请退团：已删除与已取消不能退团-->
						<shiro:hasPermission name="islandOrder:operation:toapplyLeague">
							<a href="javascript:void(0)" onClick="viewExitGroup('${orders.orderUuid}','${orderStatus}')">退团</a>
						</shiro:hasPermission>
						
						<!-- 转团 -->
						<shiro:hasPermission name="islandOrder:operation:changeGroup">
							<a href="javascript:void(0)"  onClick="changeGroup('${orders.orderUuid}')">转团</a>
						</shiro:hasPermission>
						
						<!-- 转款 -->
						<c:if test="${orders.transferMoneyCheck==1}">
							<shiro:hasPermission name="islandOrder:operation:transfersMoney">
								<a href="javascript:void(0)" onclick="transfersMoney(${orders.id})">转款</a>
							</shiro:hasPermission>
						</c:if>
						<!-- 借款 -->
						<shiro:hasPermission name="islandOrder:operation:borrowingList">
								<a href="javascript:void(0)" onClick="viewBorrowingList('${orders.id}','12','${orders.orderUuid }')">借款</a>
						</shiro:hasPermission>
						
						<!-- 退款-->
						<c:if test="${not empty orders.accountedMoney && orders.accountedMoney!='¥0.00'}">
							<shiro:hasPermission name="islandOrder:operation:refund">
				...
				<shiro:hasPermission name="islandOrder:operation:cancle">
				...
				<!-- 订单锁死或解锁 -->
				<shiro:hasPermission name="islandOrder:operation:lock">
				    <c:if test="${orders.settleLockStatus != 1 && orders.lockStatus == 1 }">
				...
				<!-- 上传订单资料 -->
				<shiro:hasPermission name="islandOrder:upload">
					<a href="javascript:void(0)" onclick="uploadFiles('${orders.orderUuid}', this)">上传资料</a>
				</shiro:hasPermission>
                           
				<shiro:hasPermission name="islandOrder:operation:invokeOrder">
				...
				<c:if test="${(not empty orders.accountedMoney) and (orders.usePrice >0 ) and (orders.createStatus eq 1 or empty orders.createStatus) or (orders.verifyStatus eq 2)}">
					<shiro:hasPermission name="islandOrder:saler:invoice">
						<a href="javascript:void(0)" onClick="makinvoice(${orders.gruopId},${orders.id},'${orders.invoiceid}',${orders.orderCompany})">发票申请</a>
					</shiro:hasPermission>
				</c:if>
				<shiro:hasPermission name="islandOrder:saler:invoice">
					<a href="javascript:viewInvoiceInfo('${orders.id}','${orders.orderNum }','${orderStatus }')" class="jtk">发票申请</a>
	...
	<dl class="handle">
		...
		<dd class="">
			<p <c:if test="${operation==false && fns:getUser().id != orders.createBy}">style="display: none;"</c:if>>
				<span></span>
				<shiro:hasPermission name="islandOrder:download">
	...
	<dl class="handle" <c:if test="${(operator==3&&orders.roleType!=3) || (operator==4&&orders.roleType!=3&&orders.roleType!=4)}">style="display: none;"</c:if>>
		...
		<dd class="">
			<p>
				...
				<shiro:hasPermission name="islandOrder:operation:topay">
