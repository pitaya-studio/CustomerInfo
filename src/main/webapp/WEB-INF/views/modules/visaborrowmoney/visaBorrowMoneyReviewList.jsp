...
<script type="text/javascript">
	...
	function payedConfirmNew(ctx,orderid,agentid,obj){
	      var str="";
			...
			$.jBox("iframe:"+ctx+"/orderPay/returnMoneyConfirm?orderid="+orderid+"&agentid="+agentid+"&chosenNum="+chosenNum,{		
				...
				width:830,
		   		...
		   		submit: function(v,h,f){
				      ...
				      $.ajax({ 
				          type:"POST",
				          url:ctx+"/visa/workflow/borrowmoney/batchReviewVisaBorrowMoney",
				          dataType:"json",
				          data:dataparam,
				          success:function(data){
				        	//if('ok'==data.flag){
...
<body>
			    ...
			    <div class="message_box">
            	 ...
            	 <!--右侧内容部分开始-->
                 <form id="searchForm" action="${ctx}/visa/workflow/borrowmoney/visaBorrowMoneyReviewList" method="post">
					...
					<div class="supplierLine">
						...
						<c:if test="${userid == '68'}">
							<p class="main-right-topbutt">
							   <a style="font-size:14px;padding:10px;margin:-20px;margin-right: 10px;" onclick="batchreviewlist();">原签证借款审核列表</a>
							</p>
						</c:if>
						-->
					</div>
                    ...
                    <!-- 遍历显示结果 start -->
                            ...
                            <td class="p0">
                                <dl class="handle">
                                    ...
                                    <p>
                                        ...
                                        <a href="${ctx}/visa/workflow/borrowmoney/visaBorrowMoneyReviewDetail?orderId=${returnReviewInfo.orderid}&travelerId=${returnReviewInfo.tid}&flag=0&revid=${returnReviewInfo.revid}&flowType=${returnReviewInfo.flowType}&nowLevel=${returnReviewInfo.curlevel}&flowType=${conditionsMap.flowType}">审批</a><br />
                                        </c:if>
										<a href="${ctx}/visa/workflow/borrowmoney/visaBorrowMoneyReviewDetail?orderId=${returnReviewInfo.orderid}&travelerId=${returnReviewInfo.tid}&flag=1&revid=${returnReviewInfo.revid}&flowType=${returnReviewInfo.flowType}&nowLevel=${returnReviewInfo.curlevel}&flowType=${conditionsMap.flowType}">查看</a>
										<!-- 处理是否显示打印按钮 -->
										<c:if test="${returnReviewInfo.revstatusforprint == '2'}">
                                             <a target="_blank"  href="${ctx}/visa/workflow/borrowmoney/visaBorrowMoneyFeePrint?orderId=${returnReviewInfo.orderid}&travelerId=${returnReviewInfo.tid}&flag=1&revid=${returnReviewInfo.revid}&flowType=${returnReviewInfo.flowType}&nowLevel=${returnReviewInfo.curlevel}&flowType=${conditionsMap.flowType}"">打印</a><br />
                                        </c:if>
                        ...
                        <tr class="checkalltd">
													<td colspan='19' class="tl">
														...
														<a target="_blank" id="piliang_o_${result.orderId}" onclick="javascript:payedConfirmNew('${ctx}',1,1,this);" >批量审批</a>
													</td>
												</tr>
												</c:if> 
                    </tbody>
