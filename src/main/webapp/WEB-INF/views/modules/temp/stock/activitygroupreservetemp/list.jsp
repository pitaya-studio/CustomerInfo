...
<script type="text/javascript">
        ...
        $(document).ready(function () {
           ...
           /*  $(".closeOrExpand").click(function () {
            ...
            $('[name="batchDelete"]').on('click',function(){
                ...
                var activitygroupreserveTemps=getUuids($(this));
                var delBatchData=JSON.stringify(activitygroupreserveTemps);
                if(!$selected.length) {
                    $.jBox.tip('您未选择团期，请选择！');
                    return;
                }
                $.jBox.confirm('确定删除所有选择的数据吗？','系统提示',function(v,h,f){
                    if(v==='ok'){
                        $selected.each(function(){
                            ...
                            $.ajax({
				                	type:"POST",
				                	url:"${ctx}/activitygroupreserveTemp/delete",
				                	data:{
            ...
            $('[name="confirm"]').on('click',function(){
                ...
                $.ajax({
            		type: "POST",
            	   	url: "${ctx}/activitygroupreserveTemp/confirmReverse",
            	   	async: false,
            	   	data: {
        ...
        $(document).on('click', '[name="save"]',function () {
        	dute();
        	if(scan){
        		$.jBox.tip('切位人数请输入正整数！','error');
                return false;
        	}
        	trub();
        	if(sy){
        		$.jBox.tip('订金格式不对！','error');
        		return false;
        	} 
            var $selected = $('#contentTable tbody tr input[type="checkbox"]:checked');
            $selected.each(function(){
                ...
                var activitygroupreserveTemps = getSelectedRows($(this));
                var updateData=JSON.stringify(activitygroupreserveTemps);
...
//                    }
        ...
        //下载框
        /* function showPayOrderWin(obj,hasPermission,agentId,activityGroupId){
		    $.jBox('iframe:${ctx}/activitygroupreserveTemp/query?agentId='+agentId+'&activityGroupId='+activityGroupId,{
		        title:'凭证管理',
		        width:500,
		        height:400,
		        buttons:{'关闭':true},
		        loaded:function(h){
		           ...
		           if(hasPermission){
		                var $content = $(h.find("iframe")[0].contentDocument);
		                $content.find('li').append('<em class="ico-del" title="删除" onclick="deleteAttachment(this)"></em>');
		                $content.find('ul').append('<input class="btn btn-primary" value="上传支付凭证" style="margin:10px 0px">');
		            } 
		        }
		    })
			} */
      	...
      	function showPayOrderWin(obj,hasPermission,uuid,createBy){
      			$.ajax({
				type:"POST",
				url:"${ctx}/activitygroupreserveTemp/query",
				asyn:true,
				data:{
					"uuid":uuid
				},
		...
		function deleteAttachment(obj,uuid){
		    $.jBox.confirm('确定删除该文件么','系统提示', function (v, h, f){
		        if(v=='ok'){
		            ...
		            $.ajax({
		            	type:"POST",
		            	url:"${ctx}/activitygroupreserveTemp/deleteFile",
		            	data:{
		            		"docId":docId,
		            		"uuid":uuid
		            	},
                    ...
                    obj.parents('.toggle:first').find('#contentTable tbody tr input[type="checkbox"]:checked').each(function () {
                        if ($(this).attr('name') == 'groupId') {
                            ...
                            if ($row.attr('name') != "grouprow") {
                                var $tds = $row.find('td');
                                var activitygroupreserveTemp = {};
                                var cIndex = 0;
                                //产品Id
                                activitygroupreserveTemp.id = $tds.eq(cIndex).children().eq(0).val();
                                //产品名称
                                cIndex++;
                               ...
                               cIndex++;
                                ...
                                //订金
                                cIndex++;
                                activitygroupreserveTemp.frontMoney = $tds.eq(cIndex).children().eq(1).val();
                                
                                //切位人数
                                cIndex++;
                                activitygroupreserveTemp.payReservePosition = $tds.eq(cIndex).children().eq(1).val();
                                //余位
                                cIndex++;
                               
                                //预定人
                                cIndex++;
                                activitygroupreserveTemp.reservation =  $tds.eq(cIndex).children().eq(1).val();
                                //成人
                                cIndex++;
                               ...
                               //预收
                                ...
                                //支付方式
                                cIndex++;
                                activitygroupreserveTemp.payType =  $tds.eq(cIndex).children().eq(1).val();
                                //支付凭证
                                cIndex++;
                                //备注
                                cIndex++;
                                activitygroupreserveTemp.remark =  $tds.eq(cIndex).children().eq(1).val();
                                activitygroupreserveTemps.push(activitygroupreserveTemp);
                               
                            }
                        }
                    });
                    
                    return activitygroupreserveTemps;
                }
                        ...
                        if ($(this).attr('name') == 'groupId') {
                            ...
                            if ($row.attr('name') != "grouprow") {
                                ...
                                var cIndex = 0;
                                 ...
                                 activitygroupreserveTemp.id = $tds.eq(cIndex).children().eq(0).val();
                                //产品uuid
                                 activitygroupreserveTemp.uuid = $tds.eq(cIndex).children().eq(1).val();
                                 activitygroupreserveTemps.push( activitygroupreserveTemp);
                            }
                         }        
                     });
	...
	function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/activitygroupreserveTemp/list?status=0&showType=1");
			$("#searchForm").submit();
	    }
      ...
      /*  function down(agentId,activityGroupId){
       		window.open("${ctx}/activitygroupreserveTemp/downLoad?agentId="+agentId+"&activityGroupId="+activityGroupId);
       }  */
       //正则表达式验证切位人数格式
     function dute(){	
...
//			alert("您安装了flash,当前flash版本为: "+fls.v+".x");
	...
	function flashChecker()
	{
	 ...
	 function commenFunction(obj,fileIDList,fileNameList,filePathList,uuid) {
			...
			$.ajax({
				type:"POST",
				url:"${ctx}/activitygroupreserveTemp/saveFile",
				data:{
					"uuid":uuid,
					"docIds":fileIDList
				},
...
<body>
	...
	<form:form action="${ctx }/activitygroupreserveTemp/list?status=0&showType=1" method="post"
		id="searchForm">
		...
		<div class="activitylist_bodyer_right_team_co_bgcolor inventory_management">
			<div class="activitylist_bodyer_right_team_co2 pr wpr20">
				<label>搜索：</label> <input style="width:260px;"
					class="txtPro inputTxt inquiry_left_text" id="wholeSalerKey"
					name="acitivityName" value="${acitivityName }" /> <span class="ipt-tips"
					style="display: block;">输入产品名称，支持模糊匹配</span>
			</div>
			<div class="form_submit">
				<input class="btn btn-primary" type="submit" value="搜索">
				&nbsp; <input class="btn btn-primary" type="button" value="重置查询"
					onclick="formReset();">
			</div>
			...
			<div class="ydxbd">
				<div class="activitylist_bodyer_right_team_co1">
					...
					<input value="${groupCode }" type="text" name="groupCode">
				</div>
					...
					<select name="agentName">
						...
						<c:forEach items="${agentinfoList}" var="agentInfo">
							<option value="${agentInfo.id}"
								${agentInfo.id==agentName?"selected":''}>${agentInfo.agentName}</option>
						</c:forEach>
					</select>
				</div>
				<div class="activitylist_bodyer_right_team_co2">
					<label>出团日期：</label> <input id="groupOpenDate"
						class="inputTxt dateinput" name="groupOpenDatefront" value='<f:formatDate value="${groupOpenDatefront }" pattern="yyyy-MM-dd"/>'
						style="margin-left: -3px; width: 122px;"
						...
						至</span> <input id="groupCloseDate" class="inputTxt dateinput" name="groupOpenDateAfter"
						value='<f:formatDate value="${groupOpenDateAfter }" pattern="yyyy-MM-dd"/>' style="width: 122px;" onClick="WdatePicker()" readonly />
				</div>
				</div>
			</div>
	</form:form>
	...
	<div class="toggle" style="margin-top: 10px">
		<form id="groupform" name="groupform" action="" method="post">
			...
			<table id="contentTable"
				...
				<thead>
					<tr>
						 <th width="6%" rowspan="2"><input name="checkAll" value="" type="checkbox" style="width:auto">团号</th>
                                    <th width="4%" rowspan="2">产品名称</th>
                                    <th width="4%" rowspan="2">出团日期</th>
				...
				<tbody>
					<c:forEach items="${page.list }" var="p" >
						<tr class="grouprow">
							...
							<td class="tc">${p.acitivityName }</td>
							<td class="tc">${p.groupOpenDate }</td>
							<td class="tc">${p.agentName }</td>
							<td class="tc">${p.name }</td>
							<td class="tc"><span>${p.frontMoney }</span> <input 
								name="deposit" maxlength="10" onblur="trub()"
								onkeyup=" value=value.replace(/[\a-zA-Z\u4E00-\u9FA5]/g,'')"
								type="text" class="edit-input"></td>
							<td class="tc"><span>${p.payReservePosition }</span> <input 
								class="valid edit-input" name="cutCount"
								onkeyup=" value=value.replace(/[\a-zA-Z\u4E00-\u9FA5]/g,'')"
								value="1" maxlength="10" type="text" onblur="dute()"></td>
							<td class="tc">${p.freePosition }</td>
							<td class="tc"><span>${p.reservation }</span> <input maxlength="26"
								name="reservationer" type="text" class="edit-input" ></td>
							<td class="tc">
								<%-- <c:if test="${p.settlementAdultPrice!=null }"> --%>
									
                                   ${fns:getCurrencyInfo(p.currencyType,0,'mark')}   		
                                <%--  </c:if> --%>
								${p.settlementAdultPrice }
							</td>

							<td class="tc">
								<%-- <c:if test="${p.settlementcChildPrice!=null }"> --%>
									
                                   ${fns:getCurrencyInfo(p.currencyType,1,'mark')}     		
                                <%--  </c:if> --%>
								${p.settlementcChildPrice }
							</td>
							<td class="tc">
								<%-- <c:if test="${p.settlementSpecialPrice!=null }"> --%>
                                   ${fns:getCurrencyInfo(p.currencyType,2,'mark')}         		
                                <%--  </c:if> --%>
								${p.settlementSpecialPrice }
							</td>
							<td class="tc">${p.planPosition }</td>
							<td class="tc">${fns:getCurrencyInfo(p.currencyType,7,'mark')}${p.singleDiff }</td>
							<td class="tc"><f:formatDate value="${p.createDate }"
							...
							<td class="tl"><span>${p.remark }</span> <input name="memo" 
								maxlength="200" type="text" class="edit-input"></td>
						</tr>
					</c:forEach>
						<tr>
                              <td colspan="18">
