			...
			$("#inputForm").validate({
				rules:{
					label:{
						...
						remote: {
							type: "POST",
							url: "${ctx}/airticketactivityreserveTemp/check?type="+$('#type').val()+"&uuid="+$('#uuid').val()
								}
						},
					sort:{
						required:true,
				...
				submitHandler: function(form){
					var url = "";
					if($("#uuid").val()=='') {
						url="${ctx}/airticketactivityreserveTemp/save";
					} else {
...
<body>
	...
	<form:form method="post" modelAttribute="airticketactivityreserveTempInput" action="" class="form-horizontal" id="inputForm" novalidate="">
		...
		<div class="maintain_add">
			...
			<p>
				<label><em class="xing"></em>机票产品信息表ID外键：</label>
			...
			<p>
				<label><em class="xing"></em>售出切位人数：</label>
			...
			<p>
				...
				<input readonly onclick="WdatePicker()" value="<fmt:formatDate value="${airticketactivityreserveTemp.createDate}" pattern="yyyy-MM-dd" />" name="createDate" class="inputTxt dateinput" id="createDate" />
			</p>
			<p>
			...
			<p>
				...
				<input readonly onclick="WdatePicker()" value="<fmt:formatDate value="${airticketactivityreserveTemp.updateDate}" pattern="yyyy-MM-dd" />" name="updateDate" class="inputTxt dateinput" id="updateDate" />
			</p>
			<p>
