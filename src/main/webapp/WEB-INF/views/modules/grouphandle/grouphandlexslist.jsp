   ...
   <%--t2改版 去掉重复引用的样式 bootstrap huanqiu jh modified by Tlw--%>
    ...
    <script type="text/javascript">
		...
		function page(n,s){
			...
			$("#youkePageNo").val(n);
			$("#youkePageSize").val(s);
			$("#searchForm").attr("action","${ctx}/grouphandle/grouphandlexslist");
			$("#searchForm").submit();
	    }
    ...
    <body>
        ...
        <div class="activitylist_bodyer_right_team_co_bgcolor" style="float:left;width: 100%"> 
          ...
          <!-- 搜索查询 --> 
          <!-- 查询form -->
          <form id="searchForm" name="searchForm" action="${ctx}/grouphandle/grouphandlexslist"  method="post" >  
            ...
            <div class="activitylist_bodyer_right_team_co">
                <div class="activitylist_bodyer_right_team_co2 pr">
	                <input id="orderNumOrGroupCode" name="commonCode" class="inputTxt searchInput inputTxtlong"
						   value="${searchForm.commonCode}"  placeholder="输入团号、产品名称"
              ...
              <div class="supplierLine"> 
               ...
               <%-- <p class="main-right-topbutt"><a href="${ctx}/grouphandle/visagroupBatchEditListNew" target="_blank" id="piliangcaozuo">批量操作</a></p> --%>
		                      ...
		                      <c:forEach items="${result.groupHandleTravellers}" var="groupHandleTraveller" varStatus="statusindex">
			                    ...
			                    <tr>
			                       ...
			                       <td class="tc" >
			                         ...
			                         <td class="tc">
											<c:if test="${groupHandleTraveller.passportStatus == '1'}">借出</c:if>
											<c:if test="${groupHandleTraveller.passportStatus == '2'}">销售已领取</c:if>
											<c:if test="${groupHandleTraveller.passportStatus == '4'}">已还</c:if>
											<c:if test="${groupHandleTraveller.passportStatus == '5'}">已取出</c:if>
											<c:if test="${groupHandleTraveller.passportStatus == '6'}">未取出</c:if>
											<c:if test="${groupHandleTraveller.passportStatus == '8'}">计调领取</c:if>
			                          </td>
	         ...
	         <c:forEach items="${travelPageList}" var="travelList" varStatus="status">	   
	            <tr class="toptr">
	                ...
	                <td class="tc" >
					                    ...
					                    <tbody>
					                    	<c:forEach items="${travelList.groupHandleTravllerVisas}" var="groupHandleTravllerVisa">
					                           <tr>
											      ...
											      <td class="tc">${groupHandleTravllerVisa.visa_consulardistric_name}</td>
											      <td class="tc">${groupHandleTravllerVisa.visa_handle_unit}</td>
											      <%-- <td class="tc">${groupHandleTravllerVisa.visa_stauts}</td> --%>
											      <td class="tc">
					                            	 <c:forEach items="${visaStatusList}" var="visaStatus">
					                            	       <c:if test="${groupHandleTravllerVisa.visa_stauts ==visaStatus.value }">${visaStatus.label}</c:if>
													 </c:forEach>
	                        ...
	                        <!--E 147悬浮-->
	                <td class="tc">
	                    <c:if test="${travelList.passportStatus == '1'}">借出</c:if>
			            <c:if test="${travelList.passportStatus == '2'}">销售已领取</c:if>
			            <c:if test="${travelList.passportStatus == '4'}">已还</c:if>
			            <c:if test="${travelList.passportStatus == '5'}">已取出</c:if>
			            <c:if test="${travelList.passportStatus == '6'}">未取出</c:if>
			            <c:if test="${travelList.passportStatus == '8'}">计调领取</c:if>
	                </td>
	         </tr>
