...
function switchSalerAndCreate(){
	...
	function cashConfirm(obj,rid) {
		$.ajax({
		        type: "POST",
		        url: "${ctx}/refundmentReview/cashConfirm",
		        data: {
		        	rId : rid
		        },
	...
	function printRefund(obj, productType, agentId, orderId, reviewId, groupCode){
	...
	function payRefund(orderType, orderId, travelerId, currencyId, payPrice, reviewId){
		$.ajax({
		        type: "POST",
		        url: "${ctx}/refundmentReview/refundPay",
		        data: {
		        	orderType : orderType,
		        	orderId : orderId,
...
<body>
       <page:applyDecorator name="refundment_review_head">
   			...
   			<div class="message_box_li">
	   			<c:if test="${conditionsMap.userJobId == userJob.id}">
	   				<div class="message_box_li_a curret ydbz_x">
						<a href="${ctx}/refundmentReview/refundReviewList?headPrd=${conditionsMap.headPrd}&userJobId=${userJob.id}"><span><font style="color:white">${fns:abbrs(userJob.deptName,userJob.jobName,40)}</font></span>
				...
				<c:if test="${conditionsMap.userJobId != userJob.id}">
					<div class="message_box_li_a ydbz_x gray">
						<a style="none" href="${ctx}/refundmentReview/refundReviewList?headPrd=${conditionsMap.headPrd}&userJobId=${userJob.id}"><span><font style="color:white">${fns:abbrs(userJob.deptName,userJob.jobName,40)}</font></span>
						<c:if test ="${userJob.count gt 0}">
							<div class="message_box_li_em" >${userJob.count}</div>
						</c:if>
						<c:if test="${fns:getStringLength(userJob.deptName,userJob.jobName) gt 37}">
...
<!--    		<c:forEach items="${userJobs}" var="userJob"> -->
<!--    			<c:if test="${conditionsMap.userJobId == userJob.id}"> -->
<!-- 				<a class="ydbz_x" href="${ctx}/refundmentReview/refundReviewList?userJobId=${userJob.id}">${userJob.deptName}_${userJob.jobName}</a> -->
<!-- 			</c:if>  -->
<!-- 			<c:if test="${conditionsMap.userJobId != userJob.id}"> -->
<!-- 				<a class="ydbz_x gray" href="${ctx}/refundmentReview/refundReviewList?userJobId=${userJob.id}">${userJob.deptName}_${userJob.jobName}</a> -->
<!-- 			</c:if>  -->
<!-- 		</c:forEach> -->
<!--     </div> -->
	<form id="searchForm" action="${ctx}/refundmentReview/refundReviewList"
		method="post">
			...
			<div class="ydxbd">
				...
				<div class="activitylist_bodyer_right_team_co1">
					...
					<select name='print' id = "print">
						<option value="0" <c:if test="${conditionsMap.print==0}">selected="selected"</c:if>>未打印</option>
                        <option value="1" <c:if test="${conditionsMap.print==1}">selected="selected"</c:if>>已打印</option>
						<option value="" <c:if test="${empty conditionsMap.print}">selected="selected"</c:if>>不限</option>
					</select>
				</div>
...
<div class="activitylist_bodyer_right_team_co2">
				<label>退款金额：</label><input id="refundPriceStart" maxlength="7" class="inputTxt" name="refundPriceStart" value="${conditionsMap.refundPriceStart}" onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')"/>
				<span style="font-size:12px;font-family:'宋体';"> 至</span> 
				<input id="refundPriceEnd" maxlength="7" class="inputTxt" name="refundPriceEnd" value="${conditionsMap.refundPriceEnd}" onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')"/>
         	</div>
          ...
          <div class="activitylist_bodyer_right_team_co1">
					<label>款项：</label><input id="refundName" maxlength="7" class="inputTxt" name="refundName" value="${conditionsMap.refundName}" />
				</div>
			<c:if
		...
		<tbody>
			...
			<c:forEach items="${page.list}" var="refundReviewInfo">
				<tr>
					...
					<td>
						...
						<div class="chanpin_cen qtip" title="JHGJLXS5674--切位">
							<c:if test="${refundReviewInfo.prdtype ==7}"><!--  -->
								<a href="${ctx}/airTicket/actityAirTickettail/${refundReviewInfo.chanpid}" target="_blank">${fns:getProductName(refundReviewInfo.chanpid, refundReviewInfo.prdtype)}</a>
							</c:if>
							<c:if test="${refundReviewInfo.prdtype !=7 && refundReviewInfo.prdtype !=6}">
								<a href="${ctx}/activity/manager/detail/${refundReviewInfo.chanpid}" target="_blank">${fns:getProductName(refundReviewInfo.chanpid, refundReviewInfo.prdtype)}</a>
							</c:if>
							<c:if test="${refundReviewInfo.prdtype ==6}">
								<a href="${ctx}/visa/preorder/visaProductsDetail/${refundReviewInfo.chanpid}" target="_blank">${fns:getProductName(refundReviewInfo.chanpid, refundReviewInfo.prdtype)}</a>
							</c:if>
						</div></td>
					...
					<td>${fns:getOrderTotal(refundReviewInfo.orderid,conditionsMap.headPrd)}</td>
                    <td><div class="yfje_dd">
					...
					<td>
						...
						<c:if test="${refundReviewInfo.printFlag==1}"><span title="首次打印时间：<fmt:formatDate value="${refundReviewInfo.printTime}" pattern="yyyy-MM-dd"/>"><font color=green>已打印</font></span></c:if>
					</td>
					<c:if test="${isConfirm eq '1' }">
						<td>
...
<!-- 											<a href="${ctx}/refundmentReview/refundReviewDetail?orderid=${refundReviewInfo.orderid}&revid=${refundReviewInfo.revid}&prdType=${refundReviewInfo.prdtype}&flag=0&nowlevel=${refundReviewInfo.curlevel}">审批</a><br /> -->
<!-- 										</c:if> -->
										<a href="${ctx}/refundmentReview/refundReviewDetail?orderid=${refundReviewInfo.orderid}&revid=${refundReviewInfo.revid}&prdType=${refundReviewInfo.prdtype}&flag=0&nowlevel=${refundReviewInfo.curlevel}" target="_blank">审批</a><br />
									</c:if>
									<c:if test="${refundReviewInfo.prdtype == 7 || refundReviewInfo.prdtype == 6}">
									...
									<c:if test="${refundReviewInfo.prdtype != 7 && refundReviewInfo.prdtype != 6&& refundReviewInfo.prdtype != 8&& refundReviewInfo.prdtype != 9}">
										...
										<a href="${ctx}/refundmentReview/refundReviewDetail?orderid=${refundReviewInfo.orderid}&revid=${refundReviewInfo.revid}&prdType=${refundReviewInfo.prdtype}&flag=1&nowlevel=${refundReviewInfo.curlevel}" target="_blank">查看</a><br />
									<c:if test="${refundReviewInfo.prdtype != 8 && refundReviewInfo.prdtype != 9}">
...
<!--  										审核通过的才能付款  
										<c:if test="${(refundReviewInfo.revstatus=='2' || refundReviewInfo.revstatus=='3') && userJob.jobType == 6}">
											<a href="${ctx}/refundmentReview/refundPay?orderType=${refundReviewInfo.prdtype}&orderId=${refundReviewInfo.orderId}&travelerId=${refundReviewInfo.travelerId}&currencyId=${refundReviewInfo.currencyId}&payPrice=${refundReviewInfo.refundPrice}&reviewId=${refundReviewInfo.revid}&agentId=${refundReviewInfo.agentid}">付款 </a>
										</c:if>-->
									</c:if>
									...
									<c:if test="${(refundReviewInfo.revstatus=='2' || refundReviewInfo.revstatus=='3') && (refundReviewInfo.prdtype == 11 || refundReviewInfo.prdtype == 12)}">
										<a href="${ctx }/refundmentReview/refundTable?reviewId=${refundReviewInfo.revid }">请款单</a>
									</c:if>
								</p>
							</dd>
						</dl></td>
	...
	<!--右侧内容部分结束-->
	<form id="refundRevPrintRefundForm" action="${ctx}/refundmentReview/refundReviewInfo" method = "post" target="_blank">
