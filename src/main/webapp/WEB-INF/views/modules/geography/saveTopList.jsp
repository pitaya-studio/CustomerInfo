...
function onSubmit(){
	...
	$.ajax({
		type : "POST",
		url : g_context_url+"/geography/saveTopList",
...
$(document).ready(function(){
   ...
   //未用  
	...
	// 默认选择节点
	var ids = $("[name='crossSection']").val().split(",");
	for(var i=0; i<ids.length; i++) {
		var node = tree.getNodeByParam("id", ids[i]);
	try{tree.checkNode(node, true, true);
	    onCheck('',ids[i],node);}catch(e){}
	}
	// 默认展开全部节点
...
<body>
           <page:applyDecorator name="sys_geographyAddTopList" ></page:applyDecorator>  
             <input id="treeData" type="hidden" name="treeData"  value="${treeData}"></input> 
				<!--右侧内容部分开始-->
                <div class="produceDiv">
    				<form method="post"  id="inputForm"   action="${ctx}/geography/saveTopList">
                     	<div class="sysdiv sysdiv_coupon">
                        	<p>
                                <label>上级区域：</label>
                                <span>
                                	<%--  ${parentName}--%>
                                	<c:if test="${lable=='guonei'}">中国</c:if>
                                	<c:if test="${lable=='guoji'}">无</c:if>
                                </span>
                            </p>
                        	<p>
                                <label><em class="xing">*</em>中文名称：</label>
                                <span>
                                	<input type="text"  name="nameCn" id="nameCn" value="" maxlength="75"  onblur="value=value.replace(/[^\u4E00-\u9FA5]/g,'')" ></input>
                                </span>
                            </p> 
                        	<p>
                                <label>中文缩写：</label>
                                <span>
                                	<input type="text" name="nameShortCn"  maxlength="75"  onblur="value=value.replace(/[^\u4E00-\u9FA5]/g,'')"  ></input>
                                </span>
                            </p>
                            <p>
                                <label>中文全拼：</label>
                                <span>
                                	<input type="text"  name="namePinyin"  id="namePinyin"  maxlength="150" onkeyup="value=value.replace(/[^\a-\z\A-\Z]/g,'')"></input>
                                </span>
                            </p> 
                            <p>
                                <label>全拼缩写：</label>
                                <span>
                                	<input type="text"  name="nameShortPinyin"  maxlength="150" onkeyup="value=value.replace(/[^\a-\z\A-\Z]/g,'')"></input>
                                </span>
                            </p> 
                            <p>
                                <label>英文名称：</label>
                                <span>
                                	<input type="text"  name="nameEn"  maxlength="150" onkeyup="value=value.replace(/[^\a-\z\A-\Z]/g,'')"></input>
                                </span>
                            </p> 
                            <p>
                                <label>英文缩写：</label>
                                <span>
                                	<input type="text"  name="nameShortEn"  maxlength="150" onkeyup="value=value.replace(/[^\a-\z\A-\Z]/g,'')" ></input>
                                </span>
                            </p> 
                            <p>
                                <label>排序：</label>
                                <span>
                                    <input type="text" class="w50"  name="sort" value="200"  onblur="this.value=this.value.replace(/\D/g,'');"   onkeyup="this.value=this.value.replace(/\D/g,'');"  onafterpaste="this.value=this.value.replace(/\D/g,'');"></input>
                                </span>
                            </p> 
                            <div class="coverarea">
                            ...
                            <div class="role">
                                <div class="role-left">
                                    <div class="role-leftTop">
                                        <input id="key" name="key" type="text"  maxlength="122"></input>
                                    </div>
							<div class="role-leftCen mCustomScrollbar _mCS_1 mCS_no_scrollbar">
								<div tabindex="0" id="mCSB_1"
									class="mCustomScrollBox mCS-light mCSB_vertical mCSB_inside">
									<div id="mCSB_1_container"
										class="mCSB_container mCS_y_hidden mCS_no_scrollbar_y"
										style="position:relative; top:0; left:0;" dir="ltr">
										<ul id="menuTree" class="ztree"
											style="margin-top: 3px; -moz-user-select: none;">
										</ul>
										<input id="menuIds" name="crossSection" type="hidden" maxlength="122" ></input>
									</div>
                             ...
                             <p class="maintain_pfull">
                            	<label>栏目描述：</label>
                                <textarea class="madintain_text"  name="description"></textarea>
                         	 </p> 
                         	  <input id="lable"   name="lable"  value="${lable}"  type="hidden"></input>
                        </div> 
                            </form>
                        <div class="release_next_add">
                            <button id="submitButton"  onclick="onSubmit()"  class="btn btn-primary" >确定</button>
                          <a href="${ctx}/geography/list?lable=${lable}" target="_self"><input type="button" class="btn btn-primary gray"  value="返回"></input></a>
                        </div>
                </div>
                <!--右侧内容部分结束-->     
</body>
