	...
	<script type="text/javascript">
	   function printdiv(printpage)
		...
		function downLoadPrintRefund(productType, agentId, orderId, reviewId, groupCode, payId, option){
			...
			$.ajax({
		        type: "POST",
		        url: "${ctx}/refundReview/updatePrint",
		        data: {
		        	reviewId : reviewId
		        },
			...
			<table class="dayinzy">
				...
				<tbody>
					<tr>
						...
						<td class="f3" width="100" colspan="2"><div style="float:left;width:128px;white-space: normal;word-break: break-word;"><c:if test = "${refundInfo.groupCodePrint == null || refundInfo.groupCodePrint == ''}">&nbsp;</c:if><c:if test="${refundInfo.groupCodePrint != null && refundInfo.groupCodePrint != ''}">${refundInfo.groupCodePrint}</c:if></div> <div style="width:40px;border-left:1px solid #000;height:30px;float:left;">人数</div></td>
<!-- 						<td class="f2 fr" width="80">人数：</td> -->
		...
		<div class="dbaniu">
			...
			<input id="" name="" type="button" class="ydbz_s" value="下载" onclick="downLoadPrintRefund('${refundInfo.productType}', '${refundInfo.agentId}', '${refundInfo.orderId}', '${refundInfo.reviewId}', '${refundInfo.groupCodePrint}', '${refundInfo.payId}', '${refundInfo.option}');"/>
		</div>
	<form id="refundRevPrintRefundDownLoadForm" action="${ctx}/refundReview/downloadList" method = "post">
