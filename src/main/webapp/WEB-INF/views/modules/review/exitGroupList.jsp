...
<body>
<page:applyDecorator name="exitGroup_review_head">
								...
								<tr>
									<td>
										...
										<c:if test="${reviewVO.reviewStatus==1 }">
										 	<input type="checkbox" value="${reviewInfos.userLevel}@${reviewInfos.rid}" name="activityId" >
										</c:if>
										<%=n++%>
									</td>
									...
									<td>
										<span class="order-saler onshow">${fns:getUserNameById(reviewInfos.saler)}</span>
										<span class="order-picker">${fns:getUserNameById(reviewInfos.picker)}</span>
									</td>
							...
							<c:if test="${reviewVO.reviewStatus==1 }">
								<tr class="checkalltd">
									<td colspan='19' class="tl">
										...
										<a target="_blank" id="piliang_o_${result.orderId}" onclick="javascript:payedConfirmNew('${ctx}',1,1,'${reviewVO.userLevel }','/orderReview/manage/batchReviewExitGroupInfo');" >批量审批</a>
									</td>
								</tr>
							</c:if>
 						</tbody>
