...
window.open("${ctx}/orderReview/manage/viewChangeGroupReviewInfo?orderId=" + orderId + "&rid="+rid+"&roleId="+roleId+"&userLevel=${reviewVO.userLevel}");
}
//审核转团申请
function reviewExitGroup(roleId,orderId,rid){
window.open("${ctx}/orderReview/manage/reviewChangeGroup?orderId=" + orderId + "&rid="+rid+"&roleId="+roleId+"&userLevel=${reviewVO.userLevel}");
}
//同一审核流程切换审核角色查询
...
<body>
<page:applyDecorator name="changeGroup_review_head">
	...
	<!--右侧内容部分开始-->	
				...
				<form id="searchForm" action="${ctx}/orderReview/manage/getChangeGroupReviewList"
						method="post">
							...
							<c:if test="${reviewVO.reviewStatus==1 }">
								<tr class="checkalltd">
									<td colspan='19' class="tl">
										...
										<a target="_blank" id="piliang_o_${result.orderId}" onclick="javascript:payedConfirmNew('${ctx}',1,1,'${reviewVO.userLevel}','/orderReview/manage/batchReviewChangeGroupInfo');" >批量审批</a>
									</td>
								</tr>
							</c:if>
						</tbody>
