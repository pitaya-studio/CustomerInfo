...
function page(n,s){
	$("#pageNo").val(n);
	$("#pageSize").val(s);
	$("#searchForm").attr("action","${ctx}/message/findMyCreateMsgList");
	$("#searchForm").submit();
}
//展开收起
	...
	$.ajax({
		...
		success : function(html){
			...
			if(json.res == "success"){
				...
				location.href = contextPath + "/message/findMyCreateMsgList";	// 重新刷新列表页
			}
		}
	});
}
...
function jbox_isRead(id){
	...
	jBox.confirm("确定已读？","已读",submitRead);
}
// 批量消息已读
function jbox_isReadAll(){
	jBox.confirm("确定已读？","已读",submitRead);
}
var submitRead = function (v, h, f) {
    if (v == true){
    	read_message();
    	 $.jBox.tip("正在修改数据...", 'loading');
    	 window.setTimeout(function () { $.jBox.closeTip('修改成功。', 'success'); }, 8000);
    }else
        jBox.tip("取消已读", 'info');
...
function goToMessageInfo(msgId){
	window.open(contextPath + "/message/goToSaveMessageInfo/"+msgId);	// 跳转到详情页
	location.href = contextPath + "/message/findMyCreateMsgList";	// 重新刷新列表页
}
...
<body>
	    <page:applyDecorator name="message_my_create" >
	    	<page:param name="current">mycreatemsglist</page:param>
	    </page:applyDecorator>
    	<!--右侧内容部分开始-->
        <div class="noticeDetails">
            ...
            <div class="filterbox-foot">
            	<c:forEach items="${departmentSet }" var="depart">
            		<label><input type="checkbox"  name="msgNoticeType"  value="${depart.id }" checked="checked"/>${depart.name }</label>
            	</c:forEach>
            </div>
             -->
            <!--查询结果筛选条件排序结束-->
            ...
            <table class="activitylist_bodyer_table tableNotice" id="contentTable">
                <thead >
                    <tr>
                        ...
                        <th width="17%">操作
                        		<a href="#"  onclick="jbox_isdelAll()">批量删除</a>　
                        </th>
                    </tr>
                </thead>
                <tbody>
                	...
                	<c:forEach items="${page.list }"  var="msg">
	                	<tr>
	                		...
	                		<td class="tc">
	                			<c:if test="${msg.status==0 }">草稿</c:if>
	                			<c:if test="${msg.status==1 }">已发布</c:if>
	                			<c:if test="${msg.status==2 }">已删除</c:if>
	                			<c:if test="${msg.status==3 }">已过期</c:if>
	                		</td>
	                        ...
	                        <td class="tc">
	                       <c:if test="${msg.status>0 && msg.status<4 }">
		                        <!-- 
