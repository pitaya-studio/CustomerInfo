...
function expand(child,obj,srcActivityId){
    ...
    function page(n,s){
      $("#pageNo").val(n);
      $("#pageSize").val(s);
      $("#searchForm").attr("action","${ctx}/cost/review/island/${reviewLevel}");
      $("#searchForm").submit();
      }
            ...
            <div class="message_box_li">
              <c:choose>
                <c:when test="${userJobId==role.id}">
                      <a  href="${ctx}/cost/review/island/${reviewLevel}?userJobId=${role.id}">
...
<!--   <div class="activitylist_bodyer_right_team_co_bgcolor">-->
   <form:form id="searchForm" modelAttribute="activityIsland" action="${ctx}/cost/review/island/${reviewLevel}" method="post" >
    ...
    <div class="ydxbd">       
       <div class="activitylist_bodyer_right_team_co3">
        ...
        <input id="groupCloseDate" class="inputTxt dateinput" name="groupEndDate" value='<fmt:formatDate value="${activityIslandGroup.groupEndDate }" pattern="yyyy-MM-dd"/>' style="width: 122px;" onClick="WdatePicker()" readonly/>
          </div>
          <div class="kong"></div>   
  ...
  <table id="contentTable" class="table activitylist_bodyer_table" >
    <thead >    
              <tr>              
                                ...
                                <th width="6%">岛屿名称</th>
                                <th width="6%">酒店名称</th>
                                <th width="6%">余位数/预收数</th>                              <th width="5%">审核发起人</th>                             
                                <th width="6%">审核状态</th>
                                <th width="5%">审核</th>
                                <th >操作</th>
        ...
        <tr id="parent${s.count}">      
          ...
          <td class="tc">${activity.groupCode}</td>
                  <td class="tc">${activity.activityName}</td>
                    ...
                    <td class="tc">
                    ${fns:getNextCostIslandReview(activity.id)}   
              </td>   
              <td class="tc">
                 <c:if test="${ activity.nowLevel == reviewLevel && activity.review==1}"> 
              <a target="_blank" href="${ctx}/cost/review/islandDetail/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}/${reviewCompanyId}?from=operatorPre">审核</a>               
         </c:if>   
              </td>           
              <td class="p00">            
         <a target="_blank" href="${ctx}/cost/review/islandRead/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}?from=operatorPre">查看</a>&nbsp;
