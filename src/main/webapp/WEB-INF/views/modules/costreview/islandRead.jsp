     ...
     var submit = function (v, h, f) {            
            ...
            if(remarkLength > 200){
                top.$.jBox.tip('项目备注不能大于200字','error' ,{ focusId: 'comment' }); return false;
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/review/denyactivitycost",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
                            remark : remark,id: id},
                        success: function(array){
                            window.location.href = "${ctx}/cost/review/list/${typeId}/${reviewLevel}";
                           // window.location.reload();                          
                        },
                        error : function(e){
...
function passActivityCost(dom,id,review){ 
  $.ajax({
                     type: "POST",                      
                     url: "${ctx}/cost/review/passactivitycost",                      
                     cache:false,
                     dataType:"json",
                     async:false,
...
<body>
     	...
     	<c:choose>
     		<c:when test="${flag eq '1' }">
     			<page:applyDecorator name="new_cost_review_head">
			    	<page:param name="showType">island</page:param>
			    </page:applyDecorator>
     		</c:when>
     		<c:otherwise>
    ...
    <!-- <div class="mod_nav"><c:if test="${empty title}">财务 > 财务审核 > 成本审核 > 海岛游成本审核</c:if><c:if test="${!empty title}">${title}</c:if></div> -->
            ...
            <div class="mod_information_dzhan_d mod_details2_d">
                ...
                <table border="0" width="90%">
                	<tr>
                        ...
                        <td class="mod_details2_d2">${fns:getUserNameById(activityIsland.createBy) }</td>
                        <td class="mod_details2_d1">下单人：</td>
                        <td class="mod_details2_d2">
							<c:forEach items="${islandOrders}" var="islandOrder">
                        		${fns:getUserNameById(islandOrder.createBy) }
                        	</c:forEach>
						</td>
                    ...
                    <tr>
                        <td class="mod_details2_d1">岛屿名称：</td>
                        <td class="mod_details2_d2">${island.islandName }</td>
                        <td class="mod_details2_d1">酒店：</td>
                        <td class="mod_details2_d2">${hotel.nameCn}</td>
                        <td class="mod_details2_d1">星级：</td>
                        <td class="mod_details2_d2">
						<span class="y_xing"> 
                        ...
                        <td class="mod_details2_d2"><fmt:formatDate value="${activityGroup.groupOpenDate }" /></td>
                    </tr>
                    <tr>
                        ...
                        <td class="mod_details2_d2">${fns:getAirlineByGroupUuid(activityGroup.uuid) }</td>
                        ...
                        <td class="mod_details2_d2"><fmt:formatDate value="${activityIslandGroupAirline.departureTime }" pattern="HH:mm" /></td>
                    </tr>
                    <tr>
               ...
               <table   class="table activitylist_bodyer_table_new contentTable_preventive"  style="width:60% !important;">
					...
					<tbody>
						<c:forEach items="${rms}" var="rm">
							<tr>
								<td class="tc">${rm.room_name }*${rm.nights }晚</td><td class="tc">${rm.mlabel }</td><td class="tc">${rm.mrlabel } ${fns:getCurrencyNameOrFlag(rm.currency_id, '0') }${rm.price }/人</td>
							</tr>
						</c:forEach>
					</tbody>
				  </table>
        ...
        <table class="activitylist_bodyer_table mod_information_dzhan" id="contentTable">
          ...
          <c:forEach items="${orderList}" var="groupOrder">
            <tr >
              ...
              <td class="tc"><div class="groupCode">${groupOrder.groupCode}</div>  <div  style="display:none;" class="acitivityName">${groupOrder.activityName}</div></td>
              <td class="tc">              
             ...
             ${groupOrder.orderPersonNum}
              ...
              <td>
              	<c:choose>
					<c:when test="${groupOrder.orderStatus == 1}">
						待确认报名
					</c:when>
					<c:when test="${groupOrder.orderStatus == 2}">
						已确认报名
					</c:when>
					<c:otherwise>
						已取消
              ...
              <td class="tc">${fns:getUserNameById(groupOrder.createBy)}</td>
             <td class="tc">
