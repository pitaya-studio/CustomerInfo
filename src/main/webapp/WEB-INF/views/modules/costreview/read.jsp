...
<head>
   ...
   $(function(){
        ...
        if(names.length == 0){
        	$("#targetAreaName").text(names);
        	$("#targetAreaName").attr('title',names);
        }else{
        	$("#targetAreaName").text(names.substring(0,names.length-1));
        	$("#targetAreaName").attr('title',names.substring(0,names.length-1));
        }
        /* 
...
<body>
     	...
     	<c:choose>
     		<c:when test="${flag eq '1' }">
     			<page:applyDecorator name="new_cost_review_head">
		             <page:param name="showType"><c:choose><c:when test="${typeId==1}">single</c:when><c:when test="${typeId==2}">loose</c:when><c:when test="${typeId==3}">study</c:when><c:when test="${typeId==5}">free</c:when><c:when test="${typeId==4}">bigCustomer</c:when><c:when test="${typeId==10}">cruise</c:when></c:choose></page:param>
		       </page:applyDecorator>
     		</c:when>
     		<c:otherwise>
    ...
    <div class="mod_nav"><c:if test="${empty title}">财务 > 财务审核 > 成本审核 > ${typename}录入审核</c:if><c:if test="${!empty title}">${title}</c:if></div>
