...
function denyCost(dom,id,reviewLevel){
  ...
  $.ajax({
                     type: "POST",                      
                     url: "${ctx}/cost/review/denyIsland",                      
                     cache:false,
                     dataType:"json",
                     async:false,
  ...
  var passCosts = function(id, classType){
            ...
            //if (v == 'ok') {
                $.ajax({
                    type: "POST",
                    url: "${ctx}/cost/manager/passCosts",
                    cache:false,
                    async:false,
                    data:{id : id,
                        type : classType},
...
function passCost(dom,id,reviewLevel){
  ...
  $.ajax({
                     type: "POST",                      
                     url: "${ctx}/cost/review/passIsland",                      
                     cache:false,
                     dataType:"json",
                     async:false,
                     data:{  format:"json",id : id, reviewLevel:reviewLevel, reviewCompanyId :${reviewCompanyId},orderType:${typeId} },
...
function tosales()
 ...
 function saveCheckBox(id){   
    ...
    $.ajax({
                        type: "POST",
                        url: "${ctx}/cost/review/saveCostIslandList",
                        cache:false,
                        dataType:"json",
                        async:false,
                        data:{ 
...
<body>
     ...
     <div class="mod_nav">财务 > 财务审核 > 成本审核 > 海岛审核</div>
        ...
        <table class="activitylist_bodyer_table mod_information_dzhan" id="contentTable">
          ...
          <ul class="cost-ul" data-total="income">
                  ...
                  <ul style="margin: 0px;float: right;">
                    <input class="btn-primary" type="button" value="成本审核通过"  onclick="saveCheckBox('budgetId')" />                        
                    </ul>               
        </div>
          ...
          <ul class="cost-ul" data-total="income">
                  ...
                  <ul class="cost-ul" data-total="profit">
                      ...
                      <li>实际总毛利：￥<fmt:formatNumber  type="currency" pattern="#,##0.00" value="${fns:getSum(incomeList,'accountedMoney')-fns:getSum(actualCost,'cost')-actualrefund }"/>
                        ...
                        <ul style="margin: 0px;float: right;">
                    <input class="btn-primary" type="button" value="成本审核通过"  onclick="saveCheckBox('actualId')" />                        
                    </ul>  
               </div>   
