...
function expand(child,obj,srcActivityId){
    ...
    function page(n,s){
      $("#pageNo").val(n);
      $("#pageSize").val(s);
      $("#searchForm").attr("action","${ctx}/cost/review/hotel/${reviewLevel}");
      $("#searchForm").submit();
      }
            ...
            <div class="message_box_li">
              <c:choose>
                <c:when test="${userJobId==role.id}">
                      <a  href="${ctx}/cost/review/hotel/${reviewLevel}?userJobId=${role.id}">
...
<!--   <div class="activitylist_bodyer_right_team_co_bgcolor">-->
   <form:form id="searchForm" modelAttribute="activityHotel" action="${ctx}/cost/review/hotel/${reviewLevel}" method="post" >
    ...
    <input id="reviewLevel"  type="hidden"  name="reviewLevel"  value="${reviewLevel}">
     <div class="activitylist_bodyer_right_team_co">       
       <div class="activitylist_bodyer_right_team_co2 pr wpr20">
          <label>团号：</label><input class="txtPro inputTxt inquiry_left_text" id="wholeSalerKey" name="groupCode" value="${activityHotelGroup.groupCode }"/>
    ...
    <div class="ydxbd">       
       <div class="activitylist_bodyer_right_team_co3">
        ...
        <input id="groupCloseDate" class="inputTxt dateinput" name="groupEndDate" value='<fmt:formatDate value="${activityHotelGroup.groupEndDate }" pattern="yyyy-MM-dd"/>' style="width: 122px;" onClick="WdatePicker()" readonly/>
          </div>
          <div class="kong"></div>
        ...
        <tr id="parent${s.count}">      
          ...
          <td class="tc">${activity.groupCode}</td>
                   ...
                   <!-- <td>${activity.createByName}</td>  -->   
                    <td class="tc">
                     ${fns:getNextCostHotelReview(activity.id)} 
              </td>   
              <td class="tc">
                 <c:if test="${ activity.nowLevel == reviewLevel && activity.review==1}"> 
              <a target="_blank" href="${ctx}/cost/review/hotelDetail/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}/${reviewCompanyId}?from=operatorPre">审核</a>               
         </c:if>   
              </td>           
              <td class="p00">            
         <a target="_blank" href="${ctx}/cost/review/hotelRead/${activity.productUuid}/${activity.activityUuid}/${reviewLevel}?from=operatorPre">查看</a>&nbsp;
