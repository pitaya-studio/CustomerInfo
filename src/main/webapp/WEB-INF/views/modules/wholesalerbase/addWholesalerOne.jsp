...
<title>
	<c:if test="${newORold==0 }">批发商管理-新增第1步-基本信息</c:if>
	<c:if test="${newORold==1 }">批发商管理-修改第1步-基本信息</c:if>
</title>
...
function ajaxContact(the_param){
	$.ajax({
		type : "POST",
		url : contextPath+"/manage/saler/addNewContact",
		data : the_param,
		dataType : "text",
...
function subForm(){
	...
	$.ajax({
		type : "POST",
		url : contextPath+"/manage/saler/addWholeOfficeOne",
		data : the_param,
		dataType : "text",
		success:function(html){
			var json;
			...
			if(json.res=="success"){
				...
				if(saveOrnext==0){
					// 进入下一步
					location.href =contextPath + "/manage/saler/gotoAddWholeOfficeTwo/${office.id}/${newORold}";
...
<body>
        ...
        <form class="form-horizontal" id="subWholeForm"  name="subForm">
          <div class="sysdiv sysdiv_coupon qdgl-cen">
			...
			<p>
            	<label>境内境外：</label>
	           	<select class="domesticOverseas required" onchange="changePosition(this,'country');">
					<option value="1" selected="selected">境内</option>
            ...
            <p>
              <label><em class="xing">*</em>批发商名称：</label>
              <span>
	              <c:if test="${newORold eq 0}">
	              		<input type="text" maxlength="50" name="name"  value="">
	              </c:if>
              	  <c:if test="${newORold eq 1 }">
