...
<body>
            	<!--右侧内容部分开始-->
            	<form:form id="searchForm" modelAttribute="sysModuleConfig" action="${ctx}/modules/sysmoduleconfig/updateSysModuleConfig?id=" method="post" class="form-search">
               	<input type="hidden" id = "sysid" name = "sysid" value="${sysModule.id}">
                <div class="sysdiv sysdiv_coupon">
							...
							<select id="companyId" name="companyId">
								<option value="">请选择</option>
						      		<c:forEach items="${officelist}" var="office">
			                	    	<c:choose>
									  		<c:when test="${sysModule.companyId==office.id}"><option value="${office.id}"  selected="selected">${office.name}</option></c:when>
					...
					<p>
						...
						<span>
							<select id="fmoduleId" name="fmoduleId" onchange="getModules(this)">
	                				...
	                				<c:choose>
									<c:when test="${sysModule.fmoduleId==module[0]}"><option value="${module[0]}"  selected="selected">${module[1]}</option></c:when>
						...
						<span>
							...
							<option value="">请选择</option>
						      <c:forEach items="${modulelist}" var="smodule">
		                	    <c:choose>
								  <c:when test="${sysModule.moduleId==smodule[0]}"><option value="${smodule[0]}"  selected="selected">${smodule[1]}</option></c:when>
					...
					<p>
						<label>页面名称：</label>
						<span>
							<input type="text" id = "pageName" name = "pageName" maxlength="50" value="${sysModule.pageName}" />
						</span>
					</p>
					<p>
						<label>页面路径：</label>
						<span>
							<select class="selectinput" name="path" id="path">
                                ...
                                <c:forEach items="${modulePathlist}" var="sysmodulepath">
			                        <c:choose>
				                		<c:when test="${sysModule.path ==sysmodulepath.modulepath}"><option value="${sysmodulepath.modulepath }" selected="selected">${sysmodulepath.modulepath}</option></c:when>
					...
					<p>
						<label>预览路径：</label>
						<span>
							<textarea id = "prepath" name = "prepath" rows="3" rows="3" class="input-xlarge" maxlength="200">${sysModule.prePath }</textarea>
						</span>
					</p>
					<p>
						<label>&nbsp;</label>
