...
function isSearchConditionNotUsed(){
	...
	if(""==startTime&&""==endTime&&" "==meter&&""==channnel&&""==saler){
		return true;
	}else{
		return false;
	}
	...
	function batchreviewlist(){
		window.location.href = g_context_url+"/visa/workflow/returnreceipt/visaReturnReceiptBatchReviewList?flowType=4";
	}
	...
	function  reviewGoback(revid){
		...
		$.jBox(html, { 
			...
			if (v=="1"){
		        ...
		        $.ajax({ 
			          type:"POST",
			          url:g_context_url+"/visa/workflow/returnreceipt/returnReceiptCancelAjax",
			          dataType:"json",
			          data:{'revid':revid},
	...
	//批量审核按钮 操作
	function multiReviewVisaReturnReceipt(ctx,orderid,agentid,obj){
	    var str="";
		...
		$.jBox("iframe:"+ctx+"/orderPay/returnMoneyConfirm?orderid="+orderid+"&agentid="+agentid+"&chosenNum="+chosenNum,{		
				...
				width:830,
		   		...
		   		submit: function(v,h,f){
				      ...
				      $.ajax({ 
				          type:"POST",
				          url:ctx+"/visa/workflow/returnreceipt/multiReviewVisaReturnReceipt",
				          dataType:"json",
				          data:dataparam,
				          success:function(data){
				               $("#searchForm").submit();
...
<body>
            	 ...
            	 <!--右侧内容部分开始-->
                 <form id="searchForm" action="${ctx}/visa/workflow/returnreceipt/visaReturnReceiptReviewList4CW" method="post">
                                ...
                                <dl class="handle">
                                    ...
                                    <p>
										...
										<c:if test="${returnReviewInfo.revstatusforprint == '2'||returnReviewInfo.revstatusforprint == '3'}">
                                             <a target="_blank"  href="${ctx}/visa/workflow/returnreceipt/visaReturnMoneyFeePrint?orderId=${returnReviewInfo.orderid}&travelerId=${returnReviewInfo.tid}&flag=1&revid=${returnReviewInfo.revid}&flowType=${returnReviewInfo.flowType}&nowLevel=${returnReviewInfo.curlevel}&flowType=${conditionsMap.flowType}"">打印</a><br />
                                        </c:if>
                        ...
                        <c:if test="${conditionsMap.statusChoose=='1' }"> 
	                        <tr class="checkalltd">
								<td colspan='19' class="tl">
									...
									<a target="_blank" id="piliang_o_${result.orderId}" onclick="javascript:multiReviewVisaReturnReceipt('${ctx}',1,1,this);" >批量审批</a>
								</td>
							</tr>
						</c:if> 
                    </tbody>
