...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click','[name="openGroupList"]',function(){
            ...
            $(this).parents('.channel-cut:first').find("table tr:visible input[name=productId]").each(function(){
            	selectedProductArr.push($(this).val());
            });
        	
            var agentId = $(this).parents('.channel-cut:first').attr("channel-code");
            
            var $pop=  $.jBox("iframe:${ctx}/stock/manager/airticket/getProductGroupPage?source=isReturn&agentId="+agentId+"&selectedProducts=" + selectedProductArr.join(','),{
                title: "产品列表",
                width: 1100,
                height: 500,
                buttons: {'提交':1,'取消':0},
                        ...
                        $content.find('input[type="checkbox"]:checked').each(function(){
                            ...
                            if($row.length>0 && $this.attr('disabled')!='disabled'){
                                ...
                                var product = {
                                    ...
                                    cutCount: $($tds.get(6)).text(),
                                    productId: $($tds.get(8)).find('input[name=id]').val(),
                                    soldPayPosition: $($tds.get(8)).find("input[name=soldPayPosition]").val()
                                }
        ...
        function cloneRows(obj,products){
            ...
            for(var i= 0,len=products.length;i<len;i++){
                ...
                $tds.eq(cIndex).find("input[name=productId]").val(product.productId);
                //团号
                cIndex++;
    ...
    <script type="text/javascript">
	    ...
	    function getReturnReserveList($obj){
			...
			$($returnTbody).find("tr[name=product]").each(function(){
					...
					returnReserveInfo.productId = $(this).find("input[name=productId]").val();//散拼团期ID或者机票产品ID
						...
						<table class="table table-striped table-bordered table-condensed table-mod2-group">
							...
							<tbody class="returnTbody">
								<tr class="grouprow" style="display:none" name="groupList">
									...
									<td class="tc">
										<input type="text" name="returnRemark" />
									</td>
								</tr>
							</tbody>
						</table>
