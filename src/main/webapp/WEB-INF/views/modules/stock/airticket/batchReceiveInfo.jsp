            ...
            function saveInDraftboxState() {
                ...
                $(document).on('change', 'input[type="checkbox"]', function () {
                    ...
                    $('input[type="checkbox"][name="groupCheck"]:checked').each(function(){
                        var isHidden = $(this).parents("[name='groupList']").is(":hidden");
                        if(isHidden) {
                    ...
                    $('div.channel-cut').each(function(){
                    	var checkLength = $(this).find("tr[name=reserveInfoTr] input[name='groupCheck']:checked").length;
                    	var allLength = $(this).find("tr[name=reserveInfoTr] input[name='groupCheck']").length;
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $(document).on('click', '.visa_copy', function () {
                    ...
                    $(this).parents('table:first').find('tbody tr:visible').each(function () {
                    ...
                    $.jBox.confirm('确定删除所有选择的数据吗？', '系统提示', function (v, h, f) {
                        if (v === 'ok') {
                            $checks.each(function () {
                                ...
                                removeItem(channellist[$area.attr('channel-code')],decodeURIComponent($checkRow.attr('product-no')));
                                $checkRow.remove();
                                $detail.remove();
                            });
                            resetIndex($area);
                ...
                function getSelectedRows(obj) {
                    var products = [];
                    obj.parents('.channel-cut:first').find('table input[type="checkbox"]:checked').each(function () {
                        if ($(this).attr('name') == 'groupCheck') {
                            ...
                            if ($row.attr('name') != "groupList") {
                                ...
                                //团号
                                cIndex++;
                                product.groupNo = $tds.eq(cIndex).text();
                                //产品名称
                                cIndex++;
                                product.productNo = $tds.eq(cIndex).html();
                                //出团日期
                                cIndex++;
                                ...
                                //成人
                                cIndex++;
                                product.adultNo = $tds.eq(cIndex).html();
                                //儿童
                                cIndex++;
                                product.childrenNo = $tds.eq(cIndex).html();
                                //特殊人群
                                cIndex++;
                                product.specNo = $tds.eq(cIndex).html();
                                //预收
                                cIndex++;
                                ...
                                //备注
                                cIndex++;
                                product.remark = $tds.eq(cIndex).children(':first').val();
                ...
                function copyGroups(channels, products) {
                    ...
                    if (isProductExist(channels, products)) {
                        $.jBox.confirm('目标渠道已包含所选产品,复制后可能覆盖已录入的信息,确定要复制么?', '系统提示', function (v, h, f) {
                            if (v == 'ok') {
                ...
                //校验批量复制的产品是否已在目标渠道中存在
                function isProductExist(channels, products) {
                    for (var i = 0, len = channels.length; i < len; i++) {
                        var selectedProduct = channellist[channels[i].channelCode];
                        if (selectedProduct) {
                            for (var m = 0, len = selectedProduct.length; m < len; m++) {
                                for (var n = 0, len = products.length; n < len; n++) {
                                    if (selectedProduct[m] == products[n].productNo) {
                                        return true;
                                    }
                                }
                            }
                ...
                $(document).on('click', '[name="confirm"]', function () {
                    ...
                    $channelInfo.find('table tbody tr[name=reserveInfoTr]').each(function(){
                        var $tr= $(this);
                    ...
                    //渠道切位信息
                    var reserveInfos = [];
                    var reserveInfoList = getReserveList($channelInfo, false);
                    ...
                    $.ajax({
                		type: "POST",
                	   	url: "${ctx}/stock/manager/airticket/batchReceive",
                	   	async: false,
                	   	data: {
              ...
              //根据渠道切位信息组装切位json数据,checkFlag为true时需要进行校验，checkFlag为false时不需要进行校验
                function getReserveList($obj, checkFlag){
                	var reserveInfoList = [];
            		var $reserveTbody = $obj.find('tbody.reserveTbody');
            		var channelCode = $obj.find("span[name=channelCode]").text();
            		
            		$($reserveTbody).find("tr[name=reserveInfoTr]").each(function(){
            			if(checkFlag) {
	            			if($(this).find("input[name=groupCheck]").attr("checked") == 'checked') {
	            		    	var reserveInfo = {};
	            				reserveInfo.activityId = $(this).find("input[name=groupCheck]").val();//团期id
                ...
                $(document).on('click', '[name="confirmAll"]', function () {
                    ...
                    $('#modForm').find('div.channel-cut table tbody tr[name=reserveInfoTr]').each(function(){
                        var $tr= $(this);
                            var cutCount = $tr.find('input[name=cutCount]').val();
                            var surplusCount = $tr.find('td[name=surplusCount]').text();
                    ...
                    $.ajax({
                		...
                		dataType: "json",
                	   	success: function(data){
                	   		if(data.result=="1"){
                	   		 	window.setTimeout('window.location.href = "${ctx}/stock/manager/airticket/?agentId="', 300);
                	   			$.jBox.tip(data.message, 'success');
                	   		} else if(data.result=="2") {
                	   			$.jBox.tip(data.message, 'fail');
                	   		} else if(data.result=="3") {
                ...
                $(document).on('click', '[name="saveInDraftbox"]', function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/airticketactivityreserveTemp/batchSave2Draftbox",
                        async: false,
                        data: {
                            "reserveJsonData":reserveJsonData,
                            "uploadJsonData":uploadJsonData
                        ...
                        success: function(data){
                            if(data.result=="success"){
                                $.jBox.tip(data.message, 'success');
                                window.setTimeout('window.location.href = "${ctx}/airticketactivityreserveTemp/list?status=0&showType=2"',300);
                ...
                $(document).on('click', '[name="delete"]', function () {
                    ...
                    $.jBox.confirm('确定要删除数据吗？', '系统提示', function (v, h, f) {
                        if (v === 'ok') {
                            $tr.remove();
                            removeItem(channellist[$area.attr('channel-code')],decodeURIComponent($tr.attr('product-no')));
                            $detail.remove();
                            resetIndex($area);
                ...
                $(document).on('click', '[name="openGroupList"]', function () {
                    var $this = $(this);
                    //var selectedProducts = (channellist[$(this).parents('.channel-cut:first').attr('channel-code')]).join(',');
                    ...
                    var $pop = $.jBox("iframe:${ctx}/stock/manager/airticket/getProductGroupPage?source=isReserve&selectedProducts=" + selectedProductArr.join(','), {
                        title: "机票产品列表",
                        width: 1100,
                        height: 500,
                        buttons: {'提交': 1, '取消': 0},
                                ...
                                $content.find('input[type="checkbox"]:checked').each(function () {
                                    ...
                                    if ($row.length > 0 && $this.attr('disabled') != 'disabled') {
                                        ...
                                        var product = {
                                            ...
                                            productNo: $($tds.get(3)).text(),
                                            surplusCount: $($tds.get(5)).text(),
                                            startDate: $($tds.get(8)).find("input[name=startingDate]").val(),
                                            reservation: '',
                                            adultNo: $($tds.get(8)).find("input[name=settlementAdultPrice]").val(),
                                            childrenNo: $($tds.get(8)).find("input[name=settlementcChildPrice]").val(),
                                            specNo: $($tds.get(8)).find("input[name=settlementSpecialPrice]").val(),
                                            advanceCount: $($tds.get(8)).find("input[name=reservationsNum]").val(),
                                            payTime: '<fmt:formatDate pattern="yyyy-MM-dd" value="${currDate}"/>',
                                            payType: '',
                                            remark: '',
                                            id: $($tds.get(8)).find("input[name=id]").val()
                                        }
                                        products.push(product);
                                    }
                                });
                                if (products.length == 0) {
                                    $.jBox.tip('没有选择产品', 'error');
                                    return false;
                                }
                ...
                function cloneRows(obj, products) {
                    ...
                    for (var i = 0, len = products.length; i < len; i++) {
                        ...
                        //根据产品编号获取列,若存在则直接删除,批量复制时会重新写入
                        var $selectedGroup = obj.parents('.channel-cut').find('tr[product-no="' + encodeURIComponent(product.productNo) + '"]');
                        if ($selectedGroup) {
                        	$selectedGroup.next('[name="detail"]').remove();
                            $selectedGroup.remove();
                            removeItem(channellist[channelCode], product.productNo);
                        }
                        ...
                        $tr.show();
                        $tr.attr("name", "reserveInfoTr");
                        $tr.attr("product-no", encodeURIComponent(product.productNo));
                        var $tds = $tr.find("td");
                        ...
                        //产品编号
                        cIndex++;
                        $tds.eq(cIndex).html(product.productNo);
                        //出团日期
                        cIndex++;
                        ...
                        //成人
                        cIndex++;
                        $tds.eq(cIndex).html(product.adultNo);
                        //儿童
                        cIndex++;
                        $tds.eq(cIndex).html(product.childrenNo);
                        //特殊人群
                        cIndex++;
                        $tds.eq(cIndex).html(product.specNo);
                        //预收
                        cIndex++;
                        $tds.eq(cIndex).text(product.advanceCount);
                        //支付时间
                        cIndex++;
                        $tds.eq(cIndex).html(product.payTime);
                        cIndex++;
                        //备注
                        cIndex++;
                        $tds.eq(cIndex).children(':first').attr('value', product.remark);
                        ...
                        channellist[channelCode].push(product.productNo);
                        //支付方式
                        obj.parents('.channel-cut').find('tr[product-no="' + encodeURIComponent(product.productNo) + '"]').find('select[name="payMethod"]').val(product.payType);
                    }
			...
			function resetIndex($obj){
			    $obj.find('[name="reserveInfoTr"]').each(function(index){
					...
					<div class="channel-cut">
						...
						<div class="toggle" style="display: none;margin-top: 10px;">
							<table class="table table-striped table-bordered table-condensed table-mod2-group ">
								<thead>
									<tr>
										...
										<th rowspan="2" width="8%">团号</th>
										<th rowspan="2" width="8%">产品编号</th>
										<th rowspan="2" width="6%">出发日期</th>
										<th rowspan="2" width="5%">订金(元) 
											<span class="tc"></span>
											<input type="text" name="reservation2" id="reservation2" maxlength="10" onkeyup="value=value.replace(/[\a-zA-Z\u4E00-\u9FA5]/g,'')" onafterpaste="value=value.replace(/[\a-zA-Z\u4E00-\u9FA5]/g,'')" onblur="value=value.replace(/[\a-zA-Z\u4E00-\u9FA5]/g,'')"> 
											<input value="复制" name="cutAmout" class="visa_copy" type="button"> 
										</th>
										<th rowspan="2" width="5%"><span class="xing">*</span>
											切位人数
											<span class="tc"> 
										...
										<th rowspan="2" width="6%">预订人
											<span class="tc"> 
												<input name="reservation4" id="reservation4" type="text"> <br>
										...
										<th rowspan="2" width="6%">备注
											<span class="tc"> 
												<input name="reservation5" id="reservation5" type="text"> <br>
								...
								<tbody class="reserveTbody">
									<tr group-no="21231" style="display:none" name="groupList">
										<td class="tc">
											<input name="groupCheck" value="" type="checkbox" style="width: auto">
											<span></span>
										</td>
										<td class="tc"></td>
										<td class="tc"></td>
										...
										<td class="tc"><input name="memo" type="text">
										</td>
										<td class="p0">
											<dl class="handle">
												<dt>
												...
												<dd class="">
													<p>
														<span></span> <a href="javascript:void(0)" name="clear">清空</a>
														<a href="javascript:void(0)" name="showDetail">
															查看已切位明细 </a> <a href="javascript:void(0)" name="delete">删除
														</a>
													</p>
												</dd>
											</dl></td>
							...
							<div style="text-align: left">
								<strong>上传切位支付凭证：</strong> <input type="button" name="passport" class="btn btn-primary" value="上传" onclick="uploadFiles('${ctx}','',this,'false')">
