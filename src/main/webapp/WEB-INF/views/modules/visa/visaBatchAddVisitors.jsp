...
<title>订单-签务签证订单-选择游客批量加入批次</title>
<!--[if lte IE 6]>
...
function page(n,s){
	...
	$("#pageNo").val(n);
	$("#pageSize").val(s);
	$("#searchForm").attr("action","${ctx}/visa/order/visaBatchAddVisitors?batchNo="+batchNo+"&refundDate="+refundDate);
	$("#searchForm").submit();
}
...
function batchJk4activiti(orderId){
	...
	var travelerIds ="";
	var visaIds ="";
	//---0064需求,游客加入批次-s-批发商环球行-加入批次号--//
	var joinBatchNo='${batchNo}';
	var joinRefundDate='${refundDate}';
	//alert("----"+joinRefundDate);
	//---0064需求,游客加入批次-e--//
	//游客界面
	if(orderId==null){
		$("input[type=checkbox][type=checkbox][class=tdCheckBox]").each(function(){
	...
	//---0064需求,游客加入批次-s-批发商环球行-后来同产品确认,该批次已经有的游客无需查询,所以注释掉-//
	   //从表batch_traveler_relation查询当前批次已有的traveler_id和visa_id并拼接到travelerIds和visaIds
	   //发送ajax请求进行查询
	/*  $.ajax({
		  url:"${ctx}/visa/order/getInfoByBatchNo",
		  type:"POST",
		  async:false,
		  data:{
			  "joinBatchNo":joinBatchNo
		  },
		  success:function(info){
			  debugger;
			  if(info!=null||""!=info){
				 for(var i=0;i<info.length;i++){
					 travelerIds+=info[i].traveler_id+",";
					 visaIds+=info[i].visa_id+",";
				 }
			  }
		  }
	   });  */
	//alert("-----"+travelerIds+"---"+visaIds);
	//---0064需求,游客加入批次-e--//
	
	$.ajax({
			cache: true,
			type: "POST",
			url:g_context_url+ "/visa/hqx/borrowmoney/checkBatchJkHqx?joinRefundDate="+joinRefundDate,//放入还款日期-0064
			data:{
				"orderId":orderId,
				"travelerIds":travelerIds,
				"visaIds":visaIds
...
function batchJk1activiti(msg){
	var rightList =msg.rightList;
	var errList =msg.errList;
	//---0064需求,游客加入批次-s-批发商环球行-加入批次号,还款日期--//
	var joinBatchNo='${batchNo}';
	var tempJoinRefundDate=msg.joinRefundDate;
	//alert("+++++"+tempJoinRefundDate);
	//---0064需求,游客加入批次-e--//
	var html = '<div style="margin-top:20px;padding:0 10px;overflow:scroll;height:500px;" id="borrowWin">';
	//---0064需求,游客加入批次-s-批发商环球行-加入批次号--//
	html+='<p style="font-weight:bold;">批次号：'+joinBatchNo+'</p>';
	//---0064需求,游客加入批次-e--//
	html += '<p>不满足条件用户：</p>';
	if(errList.length==0){
		html += ' (无)';
	...
	//TODO-270需求,临时性屏蔽,以后上线用
	  if(msg.refundDateOption=='1'){ //refundDateOption:1-必填,0-非必填--0064需求,还款日期的展示,所以删除了必填标识
	    html+='<p><span style="color:red;display:none;">*</span><span>还款日期：<input id="refundDate4BatchJK" value="'+tempJoinRefundDate+'" onclick="WdatePicker()" name="refundDate4BatchJK" type="text" style="width:100px;" disabled="disabled"/> </span></p>';
	 }else{
		html+='<p><span>还款日期：<input id="refundDate4BatchJK" value="'+tempJoinRefundDate+'" onclick="WdatePicker()" name="refundDate4BatchJK" type="text" style="width:100px;" disabled="disabled"/> </span></p>'; 
	 } 
	//***270需求--添加"还款日期"项-e****//
	if(rightList.length==0){
		html += ' (无)';
		html += '</div>';
		$.jBox(html, { title: "批量加入批次",buttons:{'取消':0}, submit:function(v, h, f){
		},height:'auto',width:680});
	}else{
		//因为0064需求,产品要求在满足条件用户,不展示"借款日期",为了避免可能产生的影响,采用给相应元素添加了hidden属性-2016-05-06
		html +='<table class="activitylist_bodyer_table t-type-jbox"><thead><tr><th width="8%">游客</th><th  width="10%">护照号</th><th width="8%">签证费用</th><th width="12%" hidden>借款日期</th><th width="10%">借款人</th><th width="12%">借款金额</th><th width="8%">借款状态</th><th width="10%">备注</th></tr></thead><tbody>';
		for(var i = 0,len = rightList.length; i<len; i++){
			html += '<tr><td><input visaId="'+rightList[i].visaId+'" travelerId ="'+rightList[i].tid+'" orderId ="'+rightList[i].orderId+'" travelerName="'+rightList[i].tname+'" type="checkbox" disabled="disabled" checked="checked"/>'+rightList[i].tname+'</td>';
			html += '<td>'+rightList[i].passportCode+'</td><td>'+rightList[i].visaFee+'</td><td hidden><input type="text" onclick="WdatePicker()" id="borrowWinDate_'+rightList[i].visaId+'" value="'+rightList[i].curDate+'" disabled/></td>';
			html +='<td><input type="text"  id="borrowWinPerson_'+rightList[i].visaId+'" value="'+rightList[i].borrowPerson+'" disabled /></td>';
			html +='<td><dl class="huanjia"><dt><input type="text" onkeyup="clearNoNum(this)" id="borrowWinMoney_'+rightList[i].visaId+'" defaultvalue="" value="" name="lendPrice"/></dt>';
			html +='<dd ><div class="ydbz_x" flag="appAll" >应用全部</div><div class="ydbz_x gray" flag="reset">清空</div></dd></dl></td>';
			html +='<td>'+rightList[i].jiekuanStatus+'</td><td><input id="borrowWinOther_'+rightList[i].visaId+'" type="text" /></td></tr>';
		...
		html += '</tbody></table>';
		html += '</div>';
		$.jBox(html, { title: "批量加入批次",buttons:{'保存': 1}, submit:function(v, h, f){
		if (v!="0"){
			var travelerIds="";
			var visaIds="";
			var dates="";
		    ...
		    var refundDate=$("#refundDate4BatchJK").val();
			 //关于还款日期是否需要校验,规则:必填配置为1且还款日期为空时,0064需求仅仅做的是静态展示,所以这里不进行相关校验了!
		   /*  if((msg.refundDateOption=='1')&&(refundDate==null||refundDate=="")){
			...
			//--0064需求,游客加入批次-s-批发商环球行-点击"后的校验" --s//
			$.ajax({
				url:"${ctx}/visa/order/checkTravelerJoinBatch",
				type:"POST",
				async:false,
				data:{
					"travelerIds":travelerIds,
				...
				success:function(result){
					var tempErrList=result.errList; //不满足条件用户集合
					var arrSubmit=result.submitFlag;//批次是否提交数组
					var tempSubmitFlag=null; //批次是否提交的标志
					debugger;
					for(var i=0;i<arrSubmit.length;i++){
						if("2"==arrSubmit[i]){ //'2'表示批次已提交
							tempSubmitFlag=true;
						}
					}
					var tempHTML="";
					
					if(tempErrList.length>0){ //游客状态发生变化
						tempHTML+='<div style="text-align:center;padding-top:100px;font-size:20px;">'
						tempHTML+='有游客借款状态发生变化!';
						tempHTML+='</div>';
						 $.jBox(tempHTML, { title: "提示",buttons:{'确定': true}, submit:function(v, h, f){
						        if (v==true){
						          top.$.jBox.close(true);//关闭父jBox窗口
						          batchJk4activiti(null); 
						          return;
						        }
						    },height:300,width:300});
						 
					}else{
						if(tempSubmitFlag){ //批次已提交的情况
							tempHTML=""
								tempHTML+='<div style="text-align:center;padding-top:100px;font-size:20px;">'
									tempHTML+='此批次已提交审批 , 无法增加游客 !';
									tempHTML+='</div>';
									 $.jBox(tempHTML, { title: "提示",buttons:{'确定': true}, submit:function(v, h, f){
									        if (v==true){
									          top.$.jBox.close(true);//关闭父jBox窗口
						...
						debugger;
						var tempBatchNo='${batchNo}';
							$.ajax({
									cache: true,
									type: "POST",
									url:"${ctx}/visa/order/batchJk4activiti",
									data:{
										"type":v,
										"travelerIds":travelerIds,
										"visaIds":visaIds,
									    ...
									    "batchNo":tempBatchNo},//270-新增还款日期-TODO-临时性屏蔽 */
										"async": false,
										success: function(msg){
										if(msg.msg!=null&&msg.msg!=""){
											top.$.jBox.tip(msg.msg,'warning');
...
<body>
	...
	<div style="width: 100%" class="activitylist_bodyer_right_team_co_bgcolor">
		<form method="post" action="${ctx}/visa/order/visaBatchAddVisitors?batchNo=${batchNo }&refundDate=${refundDate}" id="searchForm" name="searchForm">	  
	...
	<table class="activitylist_bodyer_table">
		<tbody>
			...
			<td colspan='15' class="tl">
				...
				<p class="main-right-topbutt" style=""><a onclick="batchJk4activiti(null)" style="top:-28px;">加入批次</a></p>
			</td>
			</tr>
		</tbody>
	</table>
...
<input type="hidden" value="${traveler.agentinfoId }" id="activityId_agentId_${traveler.id}">
													...
													<td width="7%" class="tc">
														...
														<div class="dzje_dd">
															<span class="fbold" > 
																<span id="traveler_startOut">${traveler.startOut}</span>
													...
													<td width="7%" class="tc">
														...
														<div class="dzje_dd">
															<span class="fbold" > 
																<span id="traveler_contract">${traveler.contract}</span>
													...
													<td width="7%">
														<c:choose>
															<c:when test="${traveler.visaStatus == 0}"><span><p align="center">未送签</p></span></c:when>
															<c:when test="${traveler.visaStatus == 1}"><span><p align="center">送签</p></span></c:when>
															<c:when test="${traveler.visaStatus == 2}"><span><p align="center">已约签</p></span></c:when>
															<c:when test="${traveler.visaStatus == 3}"><span><p align="center">通过</p></span></c:when>
															<c:when test="${traveler.visaStatus == 4}"><span><p align="center">未约签</p></span></c:when>
															<c:when test="${traveler.visaStatus == 5}"><span><p align="center">已撤签</p></span></c:when>
															<c:when test="${traveler.visaStatus == 7}"><span><p align="center">拒签</p></span></c:when>
															<c:when test="${traveler.visaStatus == 8}"><span><p align="center">调查</p></span></c:when>
															<c:when test="${traveler.visaStatus == 9}"><span><p align="center">续补资料</p></span></c:when>
															<c:otherwise><span></span></c:otherwise>
														</c:choose> 
														<!-- 0:未送签,1送签，2约签，3出签，4申请撤签，5撤签成功，6撤签失败，7拒签' -->
														<%-- <span><p align="center"> ${traveler.visaStatus} </p></span> --%>
														...
														<select id="traveler_visaStatus_${traveler.id }" name="" style="width:100%;">
																<c:choose>
																	<c:when test="${visaStatus.value == 2}">
																		...
																		<c:otherwise>
																					<span value="${visaStatus.value}">${visaStatus.label}</span>
																			<c:choose>
																				 <c:when test="${visaStatus.value eq traveler.visaStatus}">
																					<option selected="selected" value="${visaStatus.value}">${visaStatus.label}</option>  
																				</c:when>
													...
													<td width="7%"><span>
														<c:choose>
															<c:when test="${ '1' eq traveler.passportStatus}"><span><p align="center">借出</p></span></c:when>
															<c:when test="${ '2' eq traveler.passportStatus}"><span><p align="center">销售已领取</p></span></c:when>
															<c:when test="${ '3' eq traveler.passportStatus}"><span><p align="center">未签收</p></span></c:when>
															<c:when test="${ '4' eq traveler.passportStatus}"><span><p align="center">已还</p></span></c:when>
															<c:when test="${ '5' eq traveler.passportStatus}"><span><p align="center">已取出</p></span></c:when>
															<c:when test="${ '6' eq traveler.passportStatus}"><span><p align="center">未取出</p></span></c:when>
															<c:when test="${ '7' eq traveler.passportStatus}"><span><p align="center">走团</p></span></c:when>
															<c:when test="${ '1' eq traveler.passportStatus}"><span><p align="center">计调领取</p></span></c:when>
														</c:choose>
													...
													<!-- 担保-->
													<td width="6%"><label>${guaranteeStatus }</label>
														<c:choose>
															<c:when test="${ '1' eq traveler.guaranteeStatus}"><span><p align="center">担保</p></span></c:when>
															<c:when test="${ '2' eq traveler.guaranteeStatus}"><span><p align="center">担保+押金</p></span></c:when>
															<c:when test="${ '3' eq traveler.guaranteeStatus}"><span><p align="center">押金</p></span></c:when>
														</c:choose>
<%-- <select id="traveler_guaranteeStatus_${traveler.id }" onchange="xuanze('${traveler.visaId}','${traveler.id}','traveler_guaranteeStatus_${traveler.id }','traveleryingshouyajin_${traveler.id}')" name="" style="width:100%;">
														<c:choose>
													...
													<td width="5%" style="text-align:center;">
														<span align="center"><a target="_Blank" href="${ctx}/visa/order/goUpdateVisaOrder?visaOrderId=${traveler.visaorderId}&details=1" 
																	 onclick="">
																	 	查看
																	 </a>
														</span>
