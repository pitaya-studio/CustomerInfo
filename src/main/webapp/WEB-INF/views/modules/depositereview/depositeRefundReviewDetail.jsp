...
<script type = "text/javascript">
	//驳回
	function jbox_bohui_refund(travelerId,revId,nowlevel,denyReason,orderId,orderTypeSub,moneyAmount,currencyId,flag){
		...
		$.jBox(html, { title: "驳回原因",buttons:{"提 交":"1"}, submit:function(v, h, f){
			if (v=="1"){
				$.ajax({
	            	type : "post",
					url : "${ctx}/deposite/depositeReview",
					data:{ 
	...
	//审核通过
	function review(travelerId,revId,nowlevel,denyReason,orderId,orderTypeSub,moneyAmount,currencyId,flag){
					...
					<div class="dbaniu">
						<form id="searchForm" action="${ctx}/deposite/depositeReview" method="post">
							...
							<a class="ydbz_s gray" href = "javascript:window.close();">返回</a> <a class="ydbz_s"
								onclick="jbox_bohui_refund(${reviewdetail.travelerId},${reviewdetail.id},${nowlevel},'',${orderDetail.orderid},${orderDetail.prdtype},${reviewdetail.refundPrice},${reviewdetail.depositPriceCurrency},${flag});">驳回</a> <input type="button" value="审核通过" onclick="review(${reviewdetail.travelerId},${reviewdetail.id},${nowlevel},'',${orderDetail.orderid},${orderDetail.prdtype},${reviewdetail.refundPrice},${reviewdetail.depositPriceCurrency},${flag})"
