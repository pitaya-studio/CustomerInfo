                ...
                <form id="modForm" action="/trekiz_wholesaler/a/activity/manager/modsave?proId=" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mod_information_dzhan">
                        <div class="mod_information_dzhan_d mod_details2_d">
                            ...
                            <div class="title_samil">
                                                                         第${flightInfo.number}段：<c:if test="${flightInfo.ticket_area_type eq '1'}">内陆</c:if>
                    ...
                    <div class="mod_information_dzhan">
                        ...
                        <table width="90%" border="0">
                            <tbody><tr>
                             ...
                             <tr>
                                ...
                                <c:if test="${empty airticket.invoiceTax}"><!-- 发票税为空的情况 -->
                                <td class="mod_details2_d2">0.00&nbsp;%</td>
                                </c:if>
                                <c:if test="${not empty airticket.invoiceTax}"><!-- 发票税不为空的情况 -->
                      ...
                      <c:forEach items="${airticket.flightInfos}" var="flightInfo" varStatus="s2">
                        ...
                        <div class="title_samil">第${flightInfo.number}段：${flightInfo.paraMap.leaveAirport}-${flightInfo.paraMap.destinationAirpost}
                        </div>
                        <table width="90%" border="0">
                            <tbody><tr>
                                <td class="mod_details2_d1">币种选择：</td>
                                <td class="mod_details2_d2"> <c:forEach items="${curlist}" var="currency">
                                             <c:if test="${currency.id eq flightInfo.currency_id}">
                                 ...
                                 <c:if test="${airticket.taxamt != '0.00'}">
	                                <td class="mod_details2_d2">
                                          <c:forEach items="${curlist}" var="currency">
                                             ...
                                             </c:if></c:forEach> ${flightInfo.taxamt}</td>	
                                </c:if>
                                ...
                                <td class="mod_details2_d2"><c:forEach items="${curlist}" var="currency">
                                             <c:if test="${currency.id eq flightInfo.currency_id}">
                                             ${currency.currencyMark} 
                                             </c:if></c:forEach>${flightInfo.settlementAdultPrice}</td>
                             </tr>
                             <tr>
                                ...
                                <td class="mod_details2_d2"><c:forEach items="${curlist}" var="currency">
                                             <c:if test="${currency.id eq flightInfo.currency_id}">
                                             ${currency.currencyMark} 
                                             </c:if></c:forEach>${flightInfo.settlementcChildPrice}</td>
                                <td class="mod_details2_d1">特殊人群同行价：</td>
                                <td class="mod_details2_d2"><c:forEach items="${curlist}" var="currency">
                                             <c:if test="${currency.id eq flightInfo.currency_id}">
                                             ${currency.currencyMark} 
                                             </c:if></c:forEach>${flightInfo.settlementSpecialPrice}</td>
                                 <td class="mod_details2_d1">特殊人群备注：</td>
                                <td class="mod_details2_d2"><p style="width:200px; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;" 
                                title="${flightInfo.remark}">${flightInfo.remark}</p></td>
                             </tr>
                            </tbody></table>
