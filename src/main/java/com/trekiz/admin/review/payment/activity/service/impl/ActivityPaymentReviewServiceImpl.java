package com.trekiz.admin.review.payment.activity.service.impl;
...
/**
 * 单团类产品付款申请的Service实现类
 * @author shijun.liu
 *
 */
@Service
public class ActivityPaymentReviewServiceImpl implements
		IActivityPaymentReviewService {

	private static final Logger log = Logger.getLogger(ActivityPaymentReviewServiceImpl.class);
	...
	public void apply(String item) throws PaymentException{
		...
		//查询团期数据信息
		ActivityGroup activityGroup = activityGroupService.findById(costRecord.getActivityId());
		Long deptId = activityGroup.getTravelActivity().getDeptId();
		Long operatorId = activityGroup.getCreateBy().getId();//计调
		...
		variables.put(ReviewVariableKey.REVIEW_VARIABLE_KEY_GROUP_ID, costRecord.getActivityId());//团期ID
		variables.put(ReviewVariableKey.REVIEW_VARIABLE_KEY_GROUP_CODE, activityGroup.getGroupCode());//团号
		variables.put(ReviewVariableKey.REVIEW_VARIABLE_KEY_PRODUCT_ID, activityGroup.getTravelActivity().getId());//产品ID
		variables.put(ReviewVariableKey.REVIEW_VARIABLE_KEY_PRODUCT_NAME, activityGroup.getTravelActivity().getAcitivityName());//产品名称
