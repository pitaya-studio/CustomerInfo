...
public class FileUtils extends org.apache.commons.io.FileUtils {
	 ...
	 * 复制字节流到上传的文件目录
	 * @param in 源文件字节流
	 * @param descPath 上传文件路径
	 * @param descFileName 上传文件名称
	 * @return
	 */
	public static boolean copyFileByStream(InputStream in,String descPath,String descFileName){
		
		boolean flag = false;
		try {
			FileUtils.copyInputStreamToFile(in, new File(descPath, descFileName));
			flag = true;
		} catch (Exception e) {		
			e.printStackTrace();
			log.error("文件上传失败");
	...
	/**
	 * 上传文件,生成唯一文件名称,并创建 根路径/年/月 的相对路径
	 * 并上传文件
	 *     
	 * 创建人：liangjingming   
	 * 创建时间：2014-1-27 下午3:57:25 
	 * @param basePath 文件绝对根路径
	 * @param in 上传源文件字节流
	 * @param srcFileName 上传源文件名称
	 * @return relaFilePath 返回带有文件名称的相对路径
	 * @version    
	 *
	 */
	public static String uploadFile(InputStream in,String srcFileName){
		...
		//文件更目录下以年为单位的目录
		//年目录下的月份目录
		String yearFolder = String.valueOf(cal.get(Calendar.YEAR));
		String monthFolder = String.valueOf(cal.get(Calendar.MONTH)+1);
		//月目录下的日目录
		String dayFolder = String.valueOf(cal.get(Calendar.DATE));
		//拼装绝对路径
		String filePath = basePath+File.separator+File.separator+fileFolder+File.separator+yearFolder+File.separator+monthFolder+File.separator+dayFolder+File.separator;
		//创建文件路径
		createDirectory(filePath);
		//文件相对路径
		String relaFilePath = fileFolder+File.separator+yearFolder+File.separator+monthFolder+File.separator+dayFolder+File.separator;
		
		String suffix = "";
		if(srcFileName.lastIndexOf(".")>0) {
			suffix=srcFileName.substring(srcFileName.lastIndexOf("."),srcFileName.length());
		}
		String destName = String.valueOf(System.currentTimeMillis())+suffix;		
		//上传文件
		copyFileByStream(in, filePath, destName);
		
		return relaFilePath + destName;
	}
	
	/**
	 * 根据文件名称，生成文件路径
	 * 生产的路径包括日目录
	 * 返回List.get(0) = 相对路径 List.get(1) = 绝对路径
	 */
	public static List<String> getUploadFilePath(){
	...
	public static String getTempDir(){
		//文件绝对路径根目录
		String basePath = Global.getBasePath();
		String pathPrefix = basePath + File.separator + "TEMP" + File.separator;
	 ...
	 * @param delay		单位是秒,并且大约0的整数,如果传入负数，默认为10分钟。
	 * @param files		要删除的文件
	 * @author	shijun.liu
	 * @date	2016.07.07
     */
	public static void timingDeleteFile(int delay, File... files){
		final File[] fileList = files;
		if(delay < 0){
			delay = 60 * 10;
		}
	 ...
	 * @date	2016.07.07
	 */
	public static void timingDeleteFile(int delay, List<File> files){
