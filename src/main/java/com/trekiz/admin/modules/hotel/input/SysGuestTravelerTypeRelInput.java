...
public class SysGuestTravelerTypeRelInput  extends BaseInput {
	//自定义拆分规则定义
	...
	public SysGuestTravelerTypeRelInput(){
	}
	//数据库映射bean转换成表单提交bean
	public SysGuestTravelerTypeRelInput(SysGuestTravelerTypeRel obj ){
		BeanUtil.copySimpleProperties(this, obj);
	}
	//表单提交的bean转换成数据库映射的bean
	public SysGuestTravelerTypeRel getSysGuestTravelerTypeRel() {
		dataObj = new SysGuestTravelerTypeRel();
	...
	public boolean validateInput(){
		...
		if(this.getSysGuestTypeUuid().length() > 32) {
			return false;
		}
		if(this.getSysTravelerTypeUuid().length() > 32) {
			return false;
		}
